...
static void ospf6_dbdesc_recv_master (struct ospf6_header *oh, struct ospf6_neighbor *on)
{
    ...
    case OSPF6_NEIGHBOR_EXSTART:
        ...
        {
#ifdef OSPF6_DEBUG
            zlog_debug("[%d]%s() execute negotiation_done, neighbor ipv6 address:%s", __LINE__, __func__, ipv6_string);
#endif
...
#ifdef OSPF6_DEBUG
            zlog_debug("[%d]%s() thread add event seqnumber mismatch, neighbor ipv6 address:%s", __LINE__, __func__, ipv6_string);
#endif
    ...
    case OSPF6_NEIGHBOR_FULL:
        ...
        zlog_debug("[%d]%s() thread add event seqnumber mismatch", __LINE__, __func__);
        thread_add_event (master, seqnumber_mismatch, on, 0);
        return;
    ...
    {
#ifdef OSPF6_DEBUG
        zlog_debug("[%d]%s() thread exec exchange done:%s", __LINE__, __func__, ipv6_string);
#endif
...
#ifdef OSPF6_DEBUG
    //sangmeng add debug
    zlog_debug("[%d]%s() memcpy dbdesc to debesc_lsat, neighbor ipv6 address:%s", __LINE__, __func__, ipv6_string);
#endif
...
#ifdef OSPF6_DEBUG
        if (on->leaving_exchange_state == 1)
            zlog_debug("[%d]This neighbor is Leaving---->Exchange:%s, on->summary_list->count:%d", __LINE__, ipv6_string, on->summary_list->count);
        else
            zlog_debug("This neighbor is Exstart---->Exchange:%s", ipv6_string);
#endif
        if (!memcmp (dbdesc, &on->dbdesc_last, sizeof (struct ospf6_dbdesc)))
...
#ifdef OSPF6_DEBUG
            zlog_debug("[%d]%s() thread add event seqnumber mismatch, neighbor ipv6 string:%s", __LINE__, __func__, ipv6_string);
#endif
            thread_add_event (master, seqnumber_mismatch, on, 0);
            return;
        }
...
static void ospf6_lsack_recv (struct in6_addr *src, struct in6_addr *dst, struct ospf6_interface *oi, struct ospf6_header *oh)
{
    ...
    for (p = (char *) ((caddr_t) oh + sizeof (struct ospf6_header)); p + sizeof (struct ospf6_lsa_header) <= OSPF6_MESSAGE_END (oh); p += sizeof (struct ospf6_lsa_header))
    {
		...
		zlog_debug("%s()%d will call ospf6_decrement_retrans_count.\n", __func__, __LINE__);
...
int ospf6_dbdesc_send (struct thread *thread)
{
    ...
    if (CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_IBIT))
    {
        zlog_debug("[%d]%s() IBIT set, state:%s, neighbor ipv6 address:%s ", __LINE__, __func__, ospf6_neighbor_state_str[on->state], ipv6_string);
    }
#endif
...
static int ospf6_print_lsa_prefix (struct ospf6_prefix *current, unsigned length, const u_int32_t req_num_pfxs)
{
    ...
    while (length)
    {
        ...
        zlog_debug ("[%d]%s() *******lsa header type is OSPF6_LSTYPE_INTRA_PREFIX, prefix:%s/%d", __LINE__, __func__, buf, current->prefix_length);
...
int ospf6_print_lsa_header_type (struct ospf6_lsa_header *lsah)
{
    ...
    zlog_debug ("[%d]%s()>>>print lsa header<<<<\n", __LINE__, __func__);
    ...
    case OSPF6_LSTYPE_AS_EXTERNAL:	/* RFC5340 A.4.7, same as A.4.8. */
    case OSPF6_LSTYPE_TYPE_7:
        zlog_debug ("[%d]%s() lsa header type is OSPF6_LSTYPE_AS_EXTERNAL or OSPF6_LSTYPE_TYPE_7", __LINE__, __func__);
        as_external_lsa = (struct ospf6_as_external_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
        exp_length = OSPF6_LSA_HEADER_SIZE + OSPF6_AS_EXTERNAL_LSA_MIN_SIZE;
        /* To find out if the last optional field (Referenced Link State ID) is
         * assumed in this LSA, we need to access fixed fields of the IPv6
        ...
        break;
    case OSPF6_LSTYPE_LINK:
        zlog_debug ("[%d]%s() lsa header type is OSPF6_LSTYPE_LINK", __LINE__, __func__);
        link_lsa = (struct ospf6_link_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
        ...
        break;
    case OSPF6_LSTYPE_INTRA_PREFIX:
        zlog_debug ("[%d]%s() lsa header type is OSPF6_LSTYPE_INTRA_PREFIX", __LINE__, __func__);
        intra_prefix_lsa = (struct ospf6_intra_prefix_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
        ret =
...
static int ospf6_compare_lsa_prerfix_2 (struct in6_addr *in6_in_lsdb, u_int8_t prefix_length_in_lsdb, u_int32_t adv_router_in_lsdb, struct ospf6_lsa_header *lsah)
{
    ...
    while (length)
    {
        ...
        inet_ntop (AF_INET6, &in6, buf, sizeof (buf));
        zlog_debug ("[%d]%s() *******lsah , prefix:%s/%d", __LINE__, __func__, buf, current->prefix_length);
...
static int ospf6_compare_lsa_prefix (struct ospf6_prefix *current, unsigned length, const u_int32_t req_num_pfxs, u_int32_t adv_router, struct ospf6_lsa_header *lsah)
{
    ...
    while (length)
    {
        ...
        inet_ntop (AF_INET6, &in6, buf, sizeof (buf));
        zlog_debug ("[%d]%s() *******lsa header  prefix:%s/%d", __LINE__, __func__, buf, current->prefix_length);
