...
#define INIT_CIRCLE_PERIOD 200

unsigned char conf_debug_ospf6_neighbor = 0;
//sangmeng add here for OSPF+
...
#ifdef OSPF6_DEBUG
    //sangmeng add debug
    zlog_debug("[%d]%s thread off connection_period_timer, circle_period_timer", __LINE__, __func__);
#endif
    //sangmeng add
...
hello_received (struct thread *thread)
{
    ...
    //zlog_debug("[%d]%s() recv hello packet, now neighbor state:%s, ipv6 addr:%s", __LINE__, __func__, ospf6_neighbor_state_str[on->state], ipv6_string);
#endif
    if (on->state == OSPF6_NEIGHBOR_LEAVING)
    {
            ...
            if (!memcmp (&p->stl_msg_all.neighbor_ipv6_address, &on->linklocal_addr, sizeof (struct in6_addr)))
            {
                ...
                else
                {
                    ...
                    * */
                    ...
                    {
                        zlog_debug ("[%d]%s()on->first_hello_state is:%d", __LINE__, __func__, on->first_hello_state);
...
#ifdef OSPF6_DEBUG
        zlog_debug("[%d]%s() summary_list_count:%d", __LINE__, __func__, on->summary_list->count);
#endif
    ...
    {
#ifdef OSPF6_DEBUG
        zlog_debug("[%d]%s()ospf6 neighbor state change to INIT, neighbor ipv6 address:%s", __LINE__, __func__, ipv6_string);
#endif
...
twoway_received (struct thread *thread)
{
    ...
    {
#ifdef OSPF6_DEBUG
        zlog_debug("[%d]%s()ospf6 neighbor state change to TWOWAY, neighbor ipv6 address:%s", __LINE__, __func__, ipv6_string);
#endif
...
#ifdef OSPF6_DEBUG
    zlog_debug("[%d]%s()ospf6 neighbor state change to EXSTART, neighbor ipv6 address:%s", __LINE__, __func__, ipv6_string);
#endif
...
inactivity_timer (struct thread *thread)
{
    ...
    if (on->state == OSPF6_NEIGHBOR_FULL)
    {
        ...
        /*sangmeng add, when neighbor is leaving, del route*/
#if 1/*sangmeng add here*/
...
//sangmeng change for OSPF+ 17.2.14
