...
void ospf6_lsa_originate (struct ospf6_lsa *lsa)
{
    ...
    lsa->refresh = thread_add_timer (master, ospf6_lsa_refresh, lsa, LS_REFRESH_TIME);

    if (IS_OSPF6_DEBUG_LSA_TYPE (lsa->header->type) || IS_OSPF6_DEBUG_ORIGINATE_TYPE (lsa->header->type))
    {
    ...
    if (old)
	{
		zlog_debug("%s()%d will call ospf6_flood_clear.", __func__, __LINE__);
...
void ospf6_lsa_originate_interface (struct ospf6_lsa *lsa, struct ospf6_interface *oi)
{
    lsa->lsdb = oi->lsdb;
    ospf6_lsa_originate (lsa);
}
...
void ospf6_lsa_purge (struct ospf6_lsa *lsa)
{
    ...
    lsdb_self = ospf6_get_scoped_lsdb_self (lsa);
    self = ospf6_lsdb_lookup (lsa->header->type, lsa->header->id, lsa->header->adv_router, lsdb_self);
    if (self)
    {
...
void ospf6_increment_retrans_count (struct ospf6_lsa *lsa)
{
	...
	zlog_debug("%s()%d increment orig->retrans_count:%d.\n", __func__, __LINE__, lsa->retrans_count);
    lsa->retrans_count++;
}
...
void ospf6_decrement_retrans_count (struct ospf6_lsa *lsa)
{
    ...
    orig = ospf6_lsdb_lookup (lsa->header->type, lsa->header->id, lsa->header->adv_router, lsdb);
    if (orig)
    {

		zlog_debug("%s() decrement orig->retrans_count:%d.\n", __func__, orig->retrans_count);
...
void ospf6_install_lsa_for_leaving (struct ospf6_lsa *lsa)
{
    struct ospf6_lsa *old;
    struct timeval now;
	zlog_debug("%s()%d enter ospf6_install_lsa_for_leaving.", __func__, __LINE__);
...
#if 0
    //sangmeng add debug
    zlog_debug ("[%d]%s()>>>>>>>>>Install LSA: %s", __LINE__, __func__, lsa->name);
    ospf6_lsa_header_print (lsa);
#endif
...
#if 0
        zlog_debug ("[%d]%s()>>>>>>>>>>>>>>>>old LSA: %s", __LINE__, __func__, lsa->name);
#endif
    ...
    {
#ifdef OSPF6_DEBUG
        zlog_debug ("[%d]%s()>>>>>>>>>maxage, delete LSA: %s, and return", __LINE__, __func__, lsa->name);
#endif
        ospf6_lsa_delete (lsa);
        return;
    }
...
static void ospf6_flood_interface (struct ospf6_neighbor *from, struct ospf6_lsa *lsa, struct ospf6_interface *oi)
{
    ...
    if (IS_OSPF6_DEBUG_FLOODING || IS_OSPF6_DEBUG_FLOOD_TYPE (lsa->header->type))
    {
        is_debug++;
    ...
    for (ALL_LIST_ELEMENTS (oi->neighbor_list, node, nnode, on))
    {
        ...
        //if ((on->state < OSPF6_NEIGHBOR_EXCHANGE) || (on->state == OSPF6_NEIGHBOR_LEAVING) || (on->leaving_exchange_state == 1))
        //if ((on->state < OSPF6_NEIGHBOR_EXCHANGE) || (on->leaving_exchange_state == 1))
        if (on->state < OSPF6_NEIGHBOR_EXCHANGE)
        {
            if (is_debug)
                zlog_debug ("Neighbor state less than ExChange or state is Leaving, next neighbor");
        ...
        if (on->state != OSPF6_NEIGHBOR_FULL)
        {
            ...
            req = ospf6_lsdb_lookup (lsa->header->type, lsa->header->id, lsa->header->adv_router, on->request_list);
            if (req == NULL)
            {
                if (is_debug)
		...
		zlog_debug("%s()%d will call ospf6_increment_retrans_count.\n", __func__, __LINE__);
    ...
    if (from && from->ospf6_if == oi && (from->router_id == oi->drouter || from->router_id == oi->bdrouter))
    {
        if (is_debug)
...
#if 1
    //sangmeng add
    /*first judge the other interface has leaving neighbor
     *then install lsas to other interface's new lsas list
     */
    if (oi->reference_count > 0)
    {

        struct ospf6_lsa *lsa_bak;
        lsa_bak = ospf6_lsdb_lookup (lsa->header->type, lsa->header->id, lsa->header->adv_router, oi->area->lsdb_bak);
        if (lsa_bak == NULL)
        {
            /*check lsa maxage,if the age is 3600,then check the lsa prefix is the oi interface */
            if (ospf6_check_lsa_maxage_prefix (oi, lsa) == 0)
            {
                u_int16_t lsatype;
                lsatype = ntohs (lsa->header->type);
                if (lsatype != OSPF6_LSTYPE_INTRA_PREFIX)
                {
#ifdef OSPF6_DEBUG
                    zlog_debug ("%s()%d The interface %s has %d neighbor in leaving, then install lsas to new lsas list, type:%u", __func__, __LINE__, oi->interface->name, oi->reference_count, lsatype);
#endif
                    newlsa = ospf6_lsa_copy (lsa);
                    newlsa->lsdb = oi->new_lsas_list;
                    ospf6_install_lsa_for_leaving (newlsa);
                }
            }
            else
                return;
        ...
        {
#ifdef OSPF6_DEBUG
            zlog_debug ("%s()%d in area->lsdb_bak find lsa is:%s", __func__, __LINE__,lsa_bak->name);
#endif
        }
    }
#endif
    ...
    {
#ifdef OSPF6_DEBUG
        zlog_debug ("[%d]%s() ifname:%s", __LINE__, __func__, oi->interface->name);
#endif
    ...
    else
    {
        ...
        for (ALL_LIST_ELEMENTS (oi->neighbor_list, node, nnode, on))
        {
            ...
            {
#ifdef OSPF6_DEBUG
                zlog_debug ("[%d]%s() flip on->leaving_exchange_state to 0, __LINE__, __func__");
#endif
    ...
    {
#ifdef OSPF6_DEBUG
        zlog_debug ("[%d]%s() The interface %s has %d neighbor in leaving, then install lsas to new lsas list", __LINE__, __func__, oi->interface->name, oi->reference_count);
#endif
        newlsa = ospf6_lsa_copy (lsa);
        newlsa->lsdb = oi->new_lsas_list;
        ospf6_install_lsa_for_leaving (newlsa);
...
static void ospf6_flood_area (struct ospf6_neighbor *from, struct ospf6_lsa *lsa, struct ospf6_area *oa)
{
    struct listnode *node, *nnode;
    ...
    {
        if (OSPF6_LSA_SCOPE (lsa->header->type) == OSPF6_SCOPE_LINKLOCAL && oi != OSPF6_INTERFACE (lsa->lsdb->data))
            continue;

#if 0
        if (OSPF6_LSA_SCOPE (lsa->header->type) == OSPF6_SCOPE_AS && ospf6_is_interface_virtual_link (oi))
            continue;
#endif /*0 */
...
static void ospf6_flood_process (struct ospf6_neighbor *from, struct ospf6_lsa *lsa, struct ospf6 *process)
{
    struct listnode *node, *nnode;
    ...
    {
        if (OSPF6_LSA_SCOPE (lsa->header->type) == OSPF6_SCOPE_AREA && oa != OSPF6_AREA (lsa->lsdb->data))
            continue;
        if (OSPF6_LSA_SCOPE (lsa->header->type) == OSPF6_SCOPE_LINKLOCAL && oa != OSPF6_INTERFACE (lsa->lsdb->data)->area)
            continue;

        if (ntohs (lsa->header->type) == OSPF6_LSTYPE_AS_EXTERNAL && IS_AREA_STUB (oa))
            continue;
...
static void ospf6_flood_clear_interface (struct ospf6_lsa *lsa, struct ospf6_interface *oi)
{
    struct listnode *node, *nnode;
    struct ospf6_neighbor *on;
    ...
    for (ALL_LIST_ELEMENTS (oi->neighbor_list, node, nnode, on))
    {
        rem = ospf6_lsdb_lookup (lsa->header->type, lsa->header->id, lsa->header->adv_router, on->retrans_list);
        if (rem && !ospf6_lsa_compare (rem, lsa))
        {
            if (IS_OSPF6_DEBUG_FLOODING || IS_OSPF6_DEBUG_FLOOD_TYPE (lsa->header->type))
                zlog_debug ("Remove %s from retrans_list of %s", rem->name, on->name);
...
static void ospf6_acknowledge_lsa_bdrouter (struct ospf6_lsa *lsa, int ismore_recent, struct ospf6_neighbor *from)
{
    struct ospf6_interface *oi;
    int is_debug = 0;
    ...
    if (ismore_recent < 0)
    {
        if (oi->drouter == from->router_id)
        {
            ...
            if (oi->thread_send_lsack == NULL)
                oi->thread_send_lsack = thread_add_timer (master, ospf6_lsack_send_interface, oi, 3);
        }
        else
        {
            if (is_debug)
    ...
    if (CHECK_FLAG (lsa->flag, OSPF6_LSA_DUPLICATE) && CHECK_FLAG (lsa->flag, OSPF6_LSA_IMPLIEDACK))
    {
        if (oi->drouter == from->router_id)
        {
            if (is_debug)
...
static void ospf6_acknowledge_lsa_allother (struct ospf6_lsa *lsa, int ismore_recent, struct ospf6_neighbor *from)
{
    struct ospf6_interface *oi;
    int is_debug = 0;
...
static void ospf6_acknowledge_lsa (struct ospf6_lsa *lsa, int ismore_recent, struct ospf6_neighbor *from)
{
    struct ospf6_interface *oi;
...
static int ospf6_is_maxage_lsa_drop (struct ospf6_lsa *lsa, struct ospf6_neighbor *from)
{
    struct ospf6_neighbor *on;
    struct ospf6_interface *oi;
    struct ospf6_area *oa;
    ...
    if (ospf6_lsdb_lookup (lsa->header->type, lsa->header->id, lsa->header->adv_router, lsa->lsdb))
        return 0;
...
void ospf6_receive_lsa_for_leaving (struct ospf6_neighbor *from, struct ospf6_lsa_header *lsa_header)
{
    ...
    /*the new->lsdb is from->ospf6_if->new_lsa_list */
...
#ifdef OSPF6_DEBUG
        //sangmeng add debug
        zlog_debug ("[%d]%s() will eneter ospf6 install new lsa to interface:%s", __LINE__, __func__, from->ospf6_if->interface->name);
#endif
...
void ospf6_receive_lsa (struct ospf6_neighbor *from, struct ospf6_lsa_header *lsa_header)
{
    ...
    if (ospf6_lsdb_lookup (new->header->type, new->header->id, new->header->adv_router, from->request_list))
    {
    ...
    if (ismore_recent == 0)
    {
        ...
        rem = ospf6_lsdb_lookup (new->header->type, new->header->id, new->header->adv_router, from->retrans_list);
        if (rem)
        {
            if (is_debug)
            {
    ...
    /* (8) previous database copy is more recent */
    {
        ...
        if (OSPF6_LSA_IS_MAXAGE (old) && old->header->seqnum == htonl (MAX_SEQUENCE_NUMBER))
        {
            if (is_debug)
            {
        ...
        /* Otherwise, Send database copy of this LSA to this neighbor */
        {
            ...
            if (from->thread_send_lsupdate == NULL)
                from->thread_send_lsupdate = thread_add_event (master, ospf6_lsupdate_send_neighbor, from, 0);
            ospf6_lsa_delete (new);
            return;
        }
        return;
...
DEFUN (debug_ospf6_flooding, debug_ospf6_flooding_cmd, "debug ospf6 flooding", DEBUG_STR OSPF6_STR "Debug OSPFv3 flooding function\n")
{
    OSPF6_DEBUG_FLOODING_ON ();
    return CMD_SUCCESS;
}

DEFUN (no_debug_ospf6_flooding, no_debug_ospf6_flooding_cmd, "no debug ospf6 flooding", NO_STR DEBUG_STR OSPF6_STR "Debug OSPFv3 flooding function\n")
{
