...
static void update_neighbor_period (struct in6_addr *ipv6_addr)
{
    ...
    int ifindex_array[INTERFACE_NUM];
    int i = 0, j, k;
    for (k = 0; k < INTERFACE_NUM; k++)
        ifindex_array[k] = -1;
    //FIXME: how to get ifindex
#if	1
...
static inline void recv_local (void)
{
    ...
    int lo_sd;
    int lo_newsd;
    struct sockaddr_in6 lo_addr, raddr;
    socklen_t raddr_len;
    char ipstr[STRSIZE];
    char info[BUFSIZE];
    ...
    lo_sd = socket (AF_INET6, SOCK_STREAM, 0);
    printf ("lo_sd=%d\n", lo_sd);
    if (lo_sd < 0)
    {
        perror ("socket()");
    ...
    sockopt_reuseaddr (lo_sd);
    sockopt_reuseport (lo_sd);

    lo_addr.sin6_family = AF_INET6;
    lo_addr.sin6_port = htons (atoi (LOCALPORT));
    lo_addr.sin6_addr = in6addr_any;

    if (bind (lo_sd, (void *) &lo_addr, sizeof (lo_addr)) < 0)
    {
        perror ("bind()");
        return ;
    }

    if (listen (lo_sd, 200) < 0)
    {
        perror ("listen()");
        return ;
    }
    while (1)
    {
        ...
        lo_newsd = accept (lo_sd, (void *) &raddr, &raddr_len);
        if (lo_newsd < 0)
        {
            perror ("accept()");
            continue;
        ...
        if (recv (lo_newsd, info, BUFSIZE, 0) < 0)
        {
