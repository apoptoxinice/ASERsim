...
struct ospf6_neighbor
{
    ...
    u_char connection_period_timer_state;/**< when connection period timer set, flip it to 1 */
    u_char leaving_exchange_state;/*when the state is leaving--->exchange, flip it to 1*/
    struct thread *connection_period_timer; /**< connection period timer*/
    ...
    u_char first_hello_state; /**< when first recv hello packet ,filp it to 1*/
    float  first_hello_time; /**< when first recv hello packet, record the time has gone to morning*/
};
...
extern int connection_period_timer(struct thread *);
