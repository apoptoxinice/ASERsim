    ...
    if (ifp->connected)
    {
        for (ALL_LIST_ELEMENTS (ifp->connected, node, next, ifc))
        {
            ...
            if (p->family == AF_INET)
            {
                ...
                if (ret < 0)
                {
                    zlog_debug("[%d]%s()__LINE__, __func__Can't unset interface IP address: %s.\n",
...
int ipv4_address_delete_from_kernel(struct interface *ifp)
{
    int ret;
    struct connected *connected;
    struct listnode *node;
...
int ipv6_address_delete_from_kernel(struct interface *ifp)
{
    int ret;
    struct connected *connected;
    struct listnode *node;
                    ...
                    const char *label)
{
    ...
    ret = get_net_device_link_detected(ifp->name);
    if (ret == 0)
    {
        if_unset_flags (ifp, IFF_RUNNING);
        if_refresh(ifp);
    }
            ...
            && CHECK_FLAG (ifp->status, ZEBRA_INTERFACE_ACTIVE))
    {
        ...
        //sangmeng mark here for check interface is link up
        if (if_is_running (ifp))
        {
            ret = if_set_prefix (ifp, ifc);
            if (ret < 0)
        ...
        {
            ipv4_address_delete_from_kernel(ifp);
        }
        /* Add to subnet chain list (while marking secondary attribute). */
        if_subnet_add (ifp, ifc);
        ...
        if (if_is_operative(ifp))
        {
//	vty_out(vty, "[%d]%s()ifname:%s is operative\n", __LINE__, __func__, ifp->name);
            if (if_is_running (ifp))
            {
//		vty_out(vty, "[%d]%s()ifname:%s link is up\n", __LINE__, __func__, ifp->name);
                connected_up_ipv4 (ifp, ifc);
            }
        }
    }
                      ...
                      const char *label)
{
    ...
    //sangmeng mark here for check if the interface is link down
    /* This is real route. */
    if (if_is_running (ifp))
    {
        ret = if_unset_prefix (ifp, ifc);
        if (ret < 0)
        {
            vty_out (vty, "%% [%d]%s()Can't unset interface IP address: %s.%s",
                     __LINE__, __func__ , safe_strerror(errno), VTY_NEWLINE);
            return CMD_WARNING;
        }
    }
#if 1
...
//end add
            ...
            && CHECK_FLAG (ifp->status, ZEBRA_INTERFACE_ACTIVE))
    {
        ...
        {
            ipv6_address_delete_from_kernel(ifp);
        }
        /* IP address propery set. */
        SET_FLAG (ifc->conf, ZEBRA_IFC_REAL);
