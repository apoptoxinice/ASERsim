...
#if 1
    char buf[64];
    const char *compare_str = "fe80::";
    memset(buf, 0, sizeof(buf));
    socklen_t size = sizeof(buf);

#if 0
    inet_ntop(p->family, p->prefix.s6_addr, buf, size);
    fprintf(stdout, "ipv6:%s/%d\n", buf, p->prefixlen);

    if (gate)
    {
        memset(buf, 0, sizeof(buf));
        inet_ntop(AF_INET6, gate->s6_addr, buf, size);
        fprintf(stdout, "next hop:%s\n", buf);
    }
#endif
    /*filter fe80::/64 via fe80::xxx*/
    if (p->prefixlen == 64)
    {
        inet_ntop(p->family, p->prefix.s6_addr, buf, size);
 ...
 *
 * */
static int getIp6Addr_use_lo(char *buf, char *if_name)
{
    ...
    while (ifAddrStruct!=NULL)
    {
        if ((ifAddrStruct->ifa_addr !=NULL)&&(ifAddrStruct->ifa_addr->sa_family==AF_INET6))
        {
            ...
            if(strcmp (ifAddrStruct->ifa_name, if_name) == 0)
            {
                ...
                //printf("[%d]%s get it!!!\n\n\n\n", __LINE__, ifAddrStruct->ifa_name);
                zlog_debug("[%d]%s get it!", __LINE__, ifAddrStruct->ifa_name);
                return 1;
            }
            memset(buf, 0xff, 16);
        }
...
int
zebra_4over6_nexthop_lookup(struct in6_addr *nexthop, char *tnl_name)
{
    struct zebra_4over6_tunnel_entry *pLast;
    struct zebra_4over6_tunnel_entry *pNext;
    int iRetVal = 0;
...
int
zebra_4over6_tunnel_create(struct in6_addr remote, char *tunnel_name)
{
    struct ifreq ifr;
    ...
    ret_lo = getIp6Addr_use_lo(&local, "lo");
    if (ret_lo == 0)
    {
        /* Lookup table.  */
        table = vrf_table (AFI_IP6, SAFI_UNICAST, 0);
        ...
        if (CHECK_FLAG(rib->nexthop->flags, NEXTHOP_FLAG_RECURSIVE))
            getIp6Addr(&local, rib->nexthop->rifindex);
        else
    ...
    zlog_notice("[%d]%s() tunnel_name: %s, local = %s", __LINE__, __func__, tunnel_name, buf);
...
int
zebra_4over6_nexthop_add_check(struct in6_addr nexthop)
{
    struct zebra_4over6_tunnel_entry *pLast;
    struct zebra_4over6_tunnel_entry *pNext;
...
int
rib_add_ipv4_4over6 (int type, int flags, struct prefix_ipv6 *p,
                     struct in6_addr *gate, unsigned int ifindex, u_int32_t vrf_id,
                     u_int32_t metric, u_char distance, safi_t safi)
{
    struct rib *rib;
...
/* XXX factor with rib_delete_ipv6 */
int
rib_delete_ipv4_4over6 (int type, int flags, struct prefix_ipv6 *p,
                        struct in6_addr *gate, unsigned int ifindex, u_int32_t vrf_id, safi_t safi)
{
    struct route_table *table;
    struct route_node *rn;
...
int
zread_ipv4_4over6_add (struct zserv *client, u_short length)
{
    int i;
    struct stream *s;
...
int
zebra_4over6_tunnel_delete(char *tunnel_name)
{
    struct ifreq ifr;
    int socketfd;
    int ret=0;
...
int
zebra_4over6_nexthop_del_check(struct in6_addr nexthop)
{
    struct zebra_4over6_tunnel_entry *pLast;
    struct zebra_4over6_tunnel_entry *pNext;
    int iRetVal = 0;
...
int
zread_ipv4_4over6_delete (struct zserv *client, u_short length)
{
    int i;
    struct stream *s;
    struct zapi_ipv6 api;
...
int
static_delete_ipv4_4over6 (struct prefix *p, u_char type, struct in_addr *gate,
                           const char *ifname, u_char distance, u_int32_t vrf_id)
{
    struct route_node *rn;
    struct static_ipv6 *si;
