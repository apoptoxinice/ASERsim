...
bgp_afi_safi_valid_indices (afi_t afi, safi_t *safi)
{
    ...
    case AFI_IP6:
        ...
        case SAFI_UNICAST:
        case SAFI_MULTICAST:
        case SAFI_4OVER6://added for 4over6 20130131
...
bgp_capability_mp (struct peer *peer, struct capability_header *hdr)
{
    ...
    //adde for 4over6 20130225
    if (mpc.safi == BGP_SAFI_4OVER6)
    {
        peer->afc_recv[AFI_IP][SAFI_4OVER6] = 1;

        if (peer->afc[AFI_IP][SAFI_4OVER6])
            peer->afc_nego[AFI_IP][SAFI_4OVER6] = 1;
        else
            return -1;
    }
    else
...
bgp_capability_orf_entry (struct peer *peer, struct capability_header *hdr)
{
    ...
    for (i = 0 ; i < entry.num ; i++)
    {
        ...
        if (!((afi == AFI_IP && safi == SAFI_UNICAST)
                ...
                || (afi == AFI_IP && safi == SAFI_4OVER6)//added for 4over6 20130131
...
peek_for_as4_capability (struct peer *peer, u_char length)
{
    ...
    while (stream_get_getp(s) < end)
    {
        ...
        zlog_debug("++++++opt_length = %d opt_type = %d",opt_length,opt_type);
        /* Option length check. */
...
bgp_open_option_parse (struct peer *peer, u_char length, int *mp_capability)
{
    ...
    while (stream_get_getp(s) < end)
    {
        ...
        case BGP_OPEN_OPT_CAP:
            ...
            break;
        case BGP_OPEN_OPT_REBOOT:
            bgp_peer_reboot_flag = 1;
            s->getp++;
            break;
        default:
            bgp_notify_send (peer,
            ...
            ! CHECK_FLAG (peer->flags, PEER_FLAG_OVERRIDE_CAPABILITY))
    {
        if (! peer->afc_nego[AFI_IP][SAFI_UNICAST]
                ...
                && ! peer->afc_nego[AFI_IP][SAFI_4OVER6]//added for 4over6 20130131
...
bgp_open_capability (struct stream *s, struct peer *peer)
{
    ...
    if (! peer->afc[AFI_IP][SAFI_MULTICAST]
            ...
            && ! peer->afc[AFI_IP][SAFI_4OVER6]
            && ! peer->afc[AFI_IP6][SAFI_UNICAST]
            && ! peer->afc[AFI_IP6][SAFI_MULTICAST]
            && ! CHECK_FLAG (peer->flags, PEER_CAP_REFRESH_ADV))
        return;
    ...
    /* IPv4 4over6. added for 4over6 20130131*/
    if (peer->afc[AFI_IP][SAFI_4OVER6])
    {
        peer->afc_adv[AFI_IP][SAFI_4OVER6] = 1;
        stream_putc (s, BGP_OPEN_OPT_CAP);
        stream_putc (s, CAPABILITY_CODE_MP_LEN + 2);
        stream_putc (s, CAPABILITY_CODE_MP);
        stream_putc (s, CAPABILITY_CODE_MP_LEN);
    ...
    if(neighbor_linklist_search(peer->host))
    {
        zlog_debug("input reboot");
        zlog_debug("position is %d",s->endp);
        stream_putc(s,BGP_OPEN_OPT_REBOOT);
        stream_putc(s,1);
        stream_putc(s,BGP_OPEN_REBOOT);
    }
    /* Total Opt Parm Len. */
...
/*
int router_reboot_open_process(struct peer *peer)
{
	struct bgp *bgp_entry;
	struct bgp_table *table;
	struct bgp_node *rn;
	struct bgp_adj_in *adj;
//  int reboot_flag = 1;
    int count = 0;
	bgp_entry = bgp_get_default();
	table = bgp_entry->rib[AFI_IP6][SAFI_UNICAST];
