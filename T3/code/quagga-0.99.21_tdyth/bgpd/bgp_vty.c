...
DEFUN (bgp_config_type,
       bgp_config_type_cmd,
       "bgp config-type (cisco|SpaceRouter6000)",
       BGP_STR
       "Configuration type\n"
       "cisco\n"
       "SpaceRouter6000\n")
...
/* "bgp 4over6 enable" configuration. */
DEFUN (bgp_4over6_enable,
       bgp_4over6_enable_cmd,
       "bgp 4over6 enable",
       ...
       "enable 4over6\n")
{
    ...
    if(CHECK_FLAG(bgp->table_config, BGP_TABLE_CONFIG_4OVER6))
        ...
        SET_FLAG (bgp->table_config, BGP_TABLE_CONFIG_4OVER6);
    ...
    for (rn = bgp_table_top (table); rn; rn = bgp_route_next (rn))
    {
        for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
        {
            if (peer->status == Established)
            {
                ...
                {
                    bgp_table_ipv4_to_4over6(peer, &rn->p);
                }
            }
        }
    }
...
DEFUN (no_bgp_4over6_enable,
       no_bgp_4over6_enable_cmd,
       "no bgp 4over6 enable",
       NO_STR
       "BGP specific commands\n"
       "config 4over6\n"
       "enable 4over6\n")
{
    ...
    bgp = vty->index;
    UNSET_FLAG (bgp->table_config, BGP_TABLE_CONFIG_4OVER6);

    for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
        if (peer->afc[AFI_IP][SAFI_4OVER6])
            ret = peer_4over6_deactivate (peer, AFI_IP, SAFI_4OVER6);
    }
...
{
    zlog_debug("neighbor is %s",peer_str);
    int ret;
    struct bgp *bgp;
    as_t as;
    union sockunion su;
...
DEFUN (address_family_ipv4_safi,
       ...
       "address-family ipv4 (unicast|multicast|4over6)",
       "Enter Address Family command mode\n"
       "Address family\n"
...
//added for 4over6 20130202
DEFUN (address_family_ipv4_4over6,
       ...
       "Exit from Address Family configuration mode\n")
{
    if (vty->node == BGP_IPV4_NODE
            || vty->node == BGP_IPV4M_NODE
            || vty->node == BGP_IP4OVER6_NODE//added for 4over6 20130202
            || vty->node == BGP_VPNV4_NODE
            || vty->node == BGP_IPV6_NODE
            || vty->node == BGP_IPV6M_NODE)
        vty->node = BGP_NODE;
...
afi_safi_print (afi_t afi, safi_t safi)
{
    ...
    //added for 4over6 20130131
    else if (afi == AFI_IP && safi == SAFI_4OVER6)
        return "IPv4 4over6";
...
bgp_show_peer (struct vty *vty, struct peer *p)
{
    ...
    if (p->status == Established)
    {
        if (p->cap
                ...
                //added for 4over6 20130131
                || p->afc_adv[AFI_IP][SAFI_4OVER6]
                || p->afc_recv[AFI_IP][SAFI_4OVER6]
#ifdef HAVE_IPV6
...
//added for 4over6 20130202
static struct cmd_node bgp_ipv4_4over6_node =
{
    BGP_IP4OVER6_NODE,
    "%s(config-router-af)# ",
    1,
...
bgp_vty_init (void)
{
    ...
    install_node (&bgp_ipv4_4over6_node, NULL);//added for 4over6 20130202
    ...
    install_default (BGP_IP4OVER6_NODE);//added for 4over6 20130202
    ...
    //added for 4over6 20130202
    /* "bgp 4over6 enable" commands */
    install_element (BGP_NODE, &bgp_4over6_enable_cmd);
    install_element (BGP_NODE, &no_bgp_4over6_enable_cmd);
    ...
    install_element (BGP_IP4OVER6_NODE, &neighbor_activate_cmd);//added for 4over6 20130202
    ...
    install_element (BGP_IP4OVER6_NODE, &no_neighbor_activate_cmd);//added for 4over6 20130202
    ...
    //added for 4over6 20130401
    install_element (BGP_IP4OVER6_NODE, &neighbor_route_reflector_client_cmd);
    install_element (BGP_IP4OVER6_NODE, &no_neighbor_route_reflector_client_cmd);
    ...
    install_element (BGP_IP4OVER6_NODE, &neighbor_default_originate_cmd);
    install_element (BGP_IP4OVER6_NODE, &no_neighbor_default_originate_cmd);
