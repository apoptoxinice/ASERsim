             ...
             as_t remote_as, afi_t afi, safi_t safi)
{
    ...
    //added for 4over6 20130327
    if	(su->sa.sa_family == AF_INET)
    {
        if (safi)
            peer->afc[AFI_IP][safi] = 1;
    }
    else if (su->sa.sa_family == AF_INET6)
    {
        if (safi)
            peer->afc[AFI_IP6][safi] = 1;
    }
...
peer_as_change (struct peer *peer, as_t as)
{
    ...
    if (peer_sort (peer) != BGP_PEER_IBGP)
    {
        ...
        UNSET_FLAG (peer->af_flags[AFI_IP][SAFI_MULTICAST],
                    PEER_FLAG_REFLECTOR_CLIENT);
        UNSET_FLAG (peer->af_flags[AFI_IP][SAFI_4OVER6],
                    PEER_FLAG_REFLECTOR_CLIENT);//added for 4over6 20130402
...
peer_activate (struct peer *peer, afi_t afi, safi_t safi)
{
    ...
    /*如果是EBGP邻居，则不建立4over6连接*/
//remove for ebgp 4over6
#if 0
    if(afi == AFI_IP && safi == SAFI_4OVER6 && peer_sort (peer) == BGP_PEER_EBGP)
        return 0;
#endif
...
//added for 4over6 20130228
int
peer_4over6_deactivate (struct peer *peer, afi_t afi, safi_t safi)
{
...
peer_group_active (struct peer *peer)
{
    if (peer->af_group[AFI_IP][SAFI_UNICAST]
            ...
            || peer->af_group[AFI_IP][SAFI_4OVER6]//added for 4over6 20130131
...
bgp_create (as_t *as, const char *name)
{
    ...
    //added for 4over6 20130303
    SET_FLAG (bgp->flags, BGP_FLAG_4OVER6_STARTED_AT_VIF);
...
peer_active (struct peer *peer)
{
    if (peer->afc[AFI_IP][SAFI_UNICAST]
            ...
            || peer->afc[AFI_IP][SAFI_4OVER6]//added for 4over6 20130131
...
bgp_config_write (struct vty *vty)
{
    ...
    for (ALL_LIST_ELEMENTS (bm->bgp, mnode, mnnode, bgp))
    {
        ...
        if (CHECK_FLAG (bgp->table_config, BGP_TABLE_CONFIG_4OVER6))
            vty_out(vty, " bgp 4over6 enable%s", VTY_NEWLINE);
        ...
        write += bgp_config_write_family (vty, bgp, AFI_IP, SAFI_4OVER6);
