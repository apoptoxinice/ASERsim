                      ...
                      struct prefix_rd *prd, u_char *tag)
{
    ...
    /* Nexthop attribute. added for 4over6 20130225*/
    if(safi != SAFI_4OVER6 )
    {
        if (p->family == AF_INET)
        {
            ...
            else if(safi == SAFI_UNICAST && attr->nexthop.s_addr == peer->local_id.s_addr)
            {
                stream_put_ipv4 (s, peer->nexthop.v4.s_addr);
            }
            else
    ...
    if (p->family == AF_INET6
            ||((p->family == AF_INET)&&(safi == SAFI_4OVER6)))//added for 4over6 20130226
    {
        unsigned long sizep;
        struct attr_extra *attre = attr->extra;
...
bgp_size_t
bgp_packet_attribute (struct bgp *bgp, struct peer *peer,
                      struct stream *s, struct attr *attr, struct prefix *p,struct in6_addr *nexthop,
                      afi_t afi, safi_t safi, struct peer *from,
                      struct prefix_rd *prd, u_char *tag)
{
    size_t cp;
            ...
            ||((p->family == AF_INET)&&(safi == SAFI_4OVER6)))//added for 4over6 20130226
    {
        ...
        if ((peer_sort (peer) == BGP_PEER_EBGP
                && from
                && peer_sort (from) == BGP_PEER_IBGP&&nexthop)||(peer_sort (peer) == BGP_PEER_IBGP
                        && from
                        && peer_sort (from) == BGP_PEER_EBGP&&nexthop)||(peer_sort (peer) == BGP_PEER_EBGP
                                && from
                                && peer_sort (from) == BGP_PEER_EBGP&&nexthop))
        {
            if (attre->mp_nexthop_len == 16)
            {
#if 1
                if((nexthop->s6_addr32[0] == 0) && (nexthop->s6_addr32[1] == 0) && (nexthop->s6_addr32[2] == 0) && (nexthop->s6_addr32[3] == 0))
                    if((attre->mp_nexthop_global.s6_addr32[0]==0)&&(attre->mp_nexthop_global.s6_addr32[1]==0)&&(attre->mp_nexthop_global.s6_addr32[2]==0)&&(attre->mp_nexthop_global.s6_addr32[3]==0))
