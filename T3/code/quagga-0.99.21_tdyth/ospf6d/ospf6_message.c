...
static const size_t ospf6_message_type_str_max = sizeof (ospf6_message_type_str) / sizeof (ospf6_message_type_str[0]);
...
static void ospf6_header_print (struct ospf6_header *oh)
{
    ...
    zlog_debug ("    OSPFv%d Type:%d Len:%hu Router-ID:%s", oh->version, oh->type, ntohs (oh->length), router_id);
    zlog_debug ("    Area-ID:%s Cksum:%hx Instance-ID:%d", area_id, ntohs (oh->checksum), oh->instance_id);
}
...
void ospf6_hello_print (struct ospf6_header *oh)
{
    ...
    zlog_debug ("    I/F-Id:%ld Priority:%d Option:%s", (u_long) ntohl (hello->interface_id), hello->priority, options);
    zlog_debug ("    HelloInterval:%hu DeadInterval:%hu", ntohs (hello->hello_interval), ntohs (hello->dead_interval));
    zlog_debug ("    DR:%s BDR:%s", drouter, bdrouter);

    for (p = (char *) ((caddr_t) hello + sizeof (struct ospf6_hello)); p + sizeof (u_int32_t) <= OSPF6_MESSAGE_END (oh); p += sizeof (u_int32_t))
    {
...
void ospf6_dbdesc_print (struct ospf6_header *oh)
{
    ...
    dbdesc = (struct ospf6_dbdesc *) ((caddr_t) oh + sizeof (struct ospf6_header));

    ospf6_options_printbuf (dbdesc->options, options, sizeof (options));

    zlog_debug ("    MBZ: %#x Option: %s IfMTU: %hu", dbdesc->reserved1, options, ntohs (dbdesc->ifmtu));
    zlog_debug ("    MBZ: %#x Bits: %s%s%s SeqNum: %#lx",
                dbdesc->reserved2,
                (CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_IBIT) ? "I" : "-"),
                (CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_MBIT) ? "M" : "-"), (CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_MSBIT) ? "m" : "s"), (u_long) ntohl (dbdesc->seqnum));

    for (p = (char *) ((caddr_t) dbdesc + sizeof (struct ospf6_dbdesc)); p + sizeof (struct ospf6_lsa_header) <= OSPF6_MESSAGE_END (oh); p += sizeof (struct ospf6_lsa_header))
...
void ospf6_lsreq_print (struct ospf6_header *oh)
{
    ...
    for (p = (char *) ((caddr_t) oh + sizeof (struct ospf6_header)); p + sizeof (struct ospf6_lsreq_entry) <= OSPF6_MESSAGE_END (oh); p += sizeof (struct ospf6_lsreq_entry))
    {
        ...
        zlog_debug ("    [%s Id:%s Adv:%s]", ospf6_lstype_name (e->type), id, adv_router);
    }
...
void ospf6_lsupdate_print (struct ospf6_header *oh)
{
    ...
    lsupdate = (struct ospf6_lsupdate *) ((caddr_t) oh + sizeof (struct ospf6_header));

    num = ntohl (lsupdate->lsa_number);
    zlog_debug ("    Number of LSA: %ld", num);

    for (p = (char *) ((caddr_t) lsupdate + sizeof (struct ospf6_lsupdate)); p < OSPF6_MESSAGE_END (oh) && p + OSPF6_LSA_SIZE (p) <= OSPF6_MESSAGE_END (oh); p += OSPF6_LSA_SIZE (p))
    {
        ospf6_lsa_header_print_raw ((struct ospf6_lsa_header *) p);
    }
...
void ospf6_lsack_print (struct ospf6_header *oh)
{
    ...
    for (p = (char *) ((caddr_t) oh + sizeof (struct ospf6_header)); p + sizeof (struct ospf6_lsa_header) <= OSPF6_MESSAGE_END (oh); p += sizeof (struct ospf6_lsa_header))
        ospf6_lsa_header_print_raw ((struct ospf6_lsa_header *) p);

    assert (p == OSPF6_MESSAGE_END (oh));
}

static void ospf6_hello_recv (struct in6_addr *src, struct in6_addr *dst, struct ospf6_interface *oi, struct ospf6_header *oh)
{
    struct ospf6_hello *hello;
    ...
    /* E-bit check */
    if (OSPF6_OPT_ISSET (hello->options, OSPF6_OPT_E) != OSPF6_OPT_ISSET (oi->area->options, OSPF6_OPT_E))
    {
        if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
...
static void ospf6_dbdesc_recv_master (struct ospf6_header *oh, struct ospf6_neighbor *on)
{
    struct ospf6_dbdesc *dbdesc;
    char *p;
#if 1
    ...
    case OSPF6_NEIGHBOR_EXSTART:
        ...
        if (!CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_MSBIT) && !CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_IBIT) && ntohl (dbdesc->seqnum) == on->dbdesc_seqnum)
        {
#if 0
            zlog_debug("%s()%d execute negotiation_done, neighbor ipv6 address:%s", __func__, __LINE__, ipv6_string);
#endif
    ...
    case OSPF6_NEIGHBOR_EXCHANGE:
        ...
        if (on->leaving_exchange_state == 1)
            zlog_debug("This neighbor is Leaving---->Exchange:%s", ipv6_string);
        else
            zlog_debug("This neighbor is Exstart---->Exchange:%s", ipv6_string);
#endif
...
#if 1
            zlog_debug("%s()%d thread add event seqnumber mismatch, neighbor ipv6 address:%s", __func__, __LINE__, ipv6_string);
#endif
        ...
        {
            if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
                zlog_debug ("Sequence number mismatch (%#lx expected)", (u_long) on->dbdesc_seqnum);
    ...
    case OSPF6_NEIGHBOR_FULL:
        ...
        if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
            zlog_debug ("Not duplicate dbdesc in state %s", ospf6_neighbor_state_str[on->state]);
#ifdef OSPF6_DEBUG
        zlog_debug("%s()%d thread add event seqnumber mismatch", __func__, __LINE__);
#endif
        thread_add_event (master, seqnumber_mismatch, on, 0);
        return;
    case OSPF6_NEIGHBOR_LEAVING:
        if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
            zlog_debug ("Neighbor state is leaving, ignore");
        return;


    default:
#ifdef OSPF6_DEBUG
        zlog_debug("now on->state:%d ", on->state);
#endif
        assert (0);
        break;
    }
    ...
    for (p = (char *) ((caddr_t) dbdesc + sizeof (struct ospf6_dbdesc)); p + sizeof (struct ospf6_lsa_header) <= OSPF6_MESSAGE_END (oh); p += sizeof (struct ospf6_lsa_header))
    {
        ...
        if (ntohs (his->header->type) == OSPF6_LSTYPE_AS_EXTERNAL && IS_AREA_STUB (on->ospf6_if->area))
        {
            if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
        ...
        mine = ospf6_lsdb_lookup (his->header->type, his->header->id, his->header->adv_router, lsdb);

#if 0 //only recv lsupdate for route
        //add for OSPF+ 16.4.21
        /*judge the reference count int ospf6_if greater than 0,
         *if greater than 0, it's mean the interface has a neighbor in the leaving state
         * began to save lsa header to new lsas list
         */
        if (on->ospf6_if->reference_count > 0)
        {
#ifdef OSPF6_DEBUG
            zlog_debug("%s()The interface has %d neighbor in the leaving state, save lsa header to new lsas list,ifname:%s", __func__, on->ospf6_if->reference_count,on->ospf6_if->interface->name);
#endif
            ospf6_receive_lsa_for_leaving (on, (struct ospf6_lsa_header *) p);
        }
#endif
    ...
    /* More bit check */
    if (!CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_MBIT) && !CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_MBIT))
    {
#ifdef OSPF6_DEBUG
        zlog_debug("%s()%d thread exec exchange done:%s", __func__, __LINE__, ipv6_string);
#endif
...
#ifdef OSPF6_DEBUG
    zlog_debug("%s()%d memcpy dbdesc to debesc_lsat, neighbor ipv6 address:%s", __func__, __LINE__, ipv6_string);
#endif
...
int ospf6_dbdesc_send_newone_for_leaving_to_exchange (struct thread *thread)
{
    ...
    if (!CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_MSBIT) &&	/* Slave */
            !CHECK_FLAG (on->dbdesc_last.bits, OSPF6_DBDESC_MBIT) && !CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_MBIT))
    {
        thread_add_event (master, exchange_done, on, 0);
    }
...
static void ospf6_dbdesc_recv_slave (struct ospf6_header *oh, struct ospf6_neighbor *on)
{
    struct ospf6_dbdesc *dbdesc;
    char *p;
    ...
    case OSPF6_NEIGHBOR_EXSTART:
        /* If the neighbor is Master, act as Slave. Schedule negotiation_done
           and process LSA Headers. Otherwise, ignore this message */
        if (CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_IBIT) &&
                CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_MBIT) && CHECK_FLAG (dbdesc->bits, OSPF6_DBDESC_MSBIT) && ntohs (oh->length) == sizeof (struct ospf6_header) + sizeof (struct ospf6_dbdesc))
        {
    ...
    case OSPF6_NEIGHBOR_EXCHANGE:
#if 1
        if (on->leaving_exchange_state == 1)
            zlog_debug("neighbor, Leaving to Exchange:%s, on->summary_list->count:%d",ipv6_string, on->summary_list->count);
        else
            zlog_debug("neighbor, Exstart to Exchange:%s", ipv6_string);
#endif
        if (!memcmp (dbdesc, &on->dbdesc_last, sizeof (struct ospf6_dbdesc)))
        {
...
#if 1
            zlog_debug("%s()%d thread add event seqnumber mismatch, neighbor ipv6 string:%s", __func__, __LINE__, ipv6_string);
#endif
            thread_add_event (master, seqnumber_mismatch, on, 0);
            return;
        }
        ...
        {
            if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
                zlog_debug ("Sequence number mismatch (%#lx expected)", (u_long) on->dbdesc_seqnum + 1);
#if 1
            zlog_debug("%s()%d thread add event seqnumber mismatch, neighbor ipv6 string:%s", __func__, __LINE__, ipv6_string);
#endif
            thread_add_event (master, seqnumber_mismatch, on, 0);
    ...
    for (p = (char *) ((caddr_t) dbdesc + sizeof (struct ospf6_dbdesc)); p + sizeof (struct ospf6_lsa_header) <= OSPF6_MESSAGE_END (oh); p += sizeof (struct ospf6_lsa_header))
    {
        ...
        if (OSPF6_LSA_SCOPE (his->header->type) == OSPF6_SCOPE_AS && IS_AREA_STUB (on->ospf6_if->area))
        {
            if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
...
static void ospf6_dbdesc_recv (struct in6_addr *src, struct in6_addr *dst, struct ospf6_interface *oi, struct ospf6_header *oh)
{
    struct ospf6_neighbor *on;
    struct ospf6_dbdesc *dbdesc;
    ...
    {
        if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
            zlog_debug ("Non-0 reserved field in %s's DbDesc, correct", on->name);
        dbdesc->reserved1 = 0;
        dbdesc->reserved2 = 0;
    }
...
static void ospf6_lsreq_recv (struct in6_addr *src, struct in6_addr *dst, struct ospf6_interface *oi, struct ospf6_header *oh)
{
    struct ospf6_neighbor *on;
    char *p;
    ...
    if (on->state != OSPF6_NEIGHBOR_EXCHANGE && on->state != OSPF6_NEIGHBOR_LOADING && on->state != OSPF6_NEIGHBOR_FULL)
    {
        if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
    ...
    for (p = (char *) ((caddr_t) oh + sizeof (struct ospf6_header)); p + sizeof (struct ospf6_lsreq_entry) <= OSPF6_MESSAGE_END (oh); p += sizeof (struct ospf6_lsreq_entry))
    {
        ...
        if (lsa == NULL)
        {
            ...
            if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
            {
                inet_ntop (AF_INET, &e->id, id, sizeof (id));
                inet_ntop (AF_INET, &e->adv_router, adv_router, sizeof (adv_router));
                zlog_debug ("Can't find requested [%s Id:%s Adv:%s]", ospf6_lstype_name (e->type), id, adv_router);
            }
    ...
    on->thread_send_lsupdate = thread_add_event (master, ospf6_lsupdate_send_neighbor, on, 0);
}

/* Verify, that the specified memory area contains exactly N valid IPv6
   prefixes as specified by RFC5340, A.4.1. */
static unsigned ospf6_prefixes_examin (struct ospf6_prefix *current,	/* start of buffer    */
                                       unsigned length, const u_int32_t req_num_pfxs	/* always compared with the actual number of prefixes */
                                      )
{
    ...
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
            zlog_debug ("%s: IPv6 prefix number mismatch (%u required, %u real)", __func__, req_num_pfxs, real_num_pfxs);
        return MSG_NG;
    }
    return MSG_OK;
}
...
static unsigned ospf6_lsa_examin (struct ospf6_lsa_header *lsah, const u_int16_t lsalen, const u_char headeronly)
{
    ...
    if (ltindex < OSPF6_LSTYPE_SIZE && ospf6_lsa_minlen[ltindex] && lsalen < ospf6_lsa_minlen[ltindex] + OSPF6_LSA_HEADER_SIZE)
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
    ...
    case OSPF6_LSTYPE_INTER_PREFIX:
        ...
        return ospf6_prefixes_examin
               ((struct ospf6_prefix *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE + OSPF6_INTER_PREFIX_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_INTER_PREFIX_LSA_MIN_SIZE, 1);
    ...
    case OSPF6_LSTYPE_TYPE_7:
        ...
        return ospf6_prefixes_examin ((struct ospf6_prefix *) ((caddr_t) as_external_lsa + OSPF6_AS_EXTERNAL_LSA_MIN_SIZE), lsalen - exp_length, 1);
    case OSPF6_LSTYPE_LINK:
        ...
        return ospf6_prefixes_examin ((struct ospf6_prefix *) ((caddr_t) link_lsa + OSPF6_LINK_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_LINK_LSA_MIN_SIZE, ntohl (link_lsa->prefix_num)	/* 32 bits */
                                     );
    case OSPF6_LSTYPE_INTRA_PREFIX:
        ...
        return ospf6_prefixes_examin ((struct ospf6_prefix *) ((caddr_t) intra_prefix_lsa + OSPF6_INTRA_PREFIX_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_INTRA_PREFIX_LSA_MIN_SIZE, ntohs (intra_prefix_lsa->prefix_num)	/* 16 bits */
                                     );
    ...
    case OSPF6_LSTYPE_E_INTRA_PREFIX:
        ...
        if (headeronly)
            break;
        intra_prefix_lsa = (struct ospf6_intra_prefix_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
        return ospf6_prefixes_examin ((struct ospf6_prefix *) ((caddr_t) intra_prefix_lsa + OSPF6_INTRA_PREFIX_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_INTRA_PREFIX_LSA_MIN_SIZE - 24, ntohs (intra_prefix_lsa->prefix_num)	//16 bits
                                     );
#endif
    }
...
static unsigned ospf6_lsaseq_examin (struct ospf6_lsa_header *lsah,	/* start of buffered data */
                                     ...
                                     const u_int32_t declared_num_lsas)
{
    ...
    while (length)
    {
        ...
        {
            if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
                zlog_debug ("%s: undersized (%zu B) trailing (#%u) LSA header", __func__, length, counted_lsas);
            return MSG_NG;
        }
        ...
        {
            if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
                zlog_debug ("%s: malformed LSA header #%u, declared length is %u B", __func__, counted_lsas, lsalen);
            return MSG_NG;
        }
        if (headeronly)
        {
            ...
            {
                if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
                    zlog_debug ("%s: anomaly in header-only %s LSA #%u", __func__, ospf6_lstype_name (lsah->type), counted_lsas);
                return MSG_NG;
            }
        ...
        else
        {
            ...
            {
                if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
                    zlog_debug ("%s: anomaly in %s LSA #%u: declared length is %u B, buffered length is %zu B", __func__, ospf6_lstype_name (lsah->type), counted_lsas, lsalen, length);
                return MSG_NG;
            }
            if (MSG_OK != ospf6_lsa_examin (lsah, lsalen, 0))
            {
                if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
                    zlog_debug ("%s: anomaly in %s LSA #%u", __func__, ospf6_lstype_name (lsah->type), counted_lsas);
                return MSG_NG;
            }
    ...
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
            zlog_debug ("%s: #LSAs declared (%u) does not match actual (%u)", __func__, declared_num_lsas, counted_lsas);
        return MSG_NG;
    }
    return MSG_OK;
}
...
static unsigned ospf6_packet_examin (struct ospf6_header *oh, const unsigned bytesonwire)
{
    ...
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
            zlog_debug ("%s: packet length error (%u real, %u declared)", __func__, bytesonwire, ntohs (oh->length));
        return MSG_NG;
    }
    /* version check */
    ...
    if (oh->type < OSPF6_MESSAGE_TYPE_ALL && ospf6_packet_minlen[oh->type] && bytesonwire < OSPF6_HEADER_SIZE + ospf6_packet_minlen[oh->type])
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
            zlog_debug ("%s: undersized (%u B) %s packet", __func__, bytesonwire, LOOKUP (ospf6_message_type_str, oh->type));
        return MSG_NG;
    }
    ...
    case OSPF6_MESSAGE_TYPE_HELLO:
        ...
        if (0 == (bytesonwire - OSPF6_HEADER_SIZE - OSPF6_HELLO_MIN_SIZE) % 4)
            return MSG_OK;
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
            zlog_debug ("%s: alignment error in %s packet", __func__, LOOKUP (ospf6_message_type_str, oh->type));
        return MSG_NG;
    case OSPF6_MESSAGE_TYPE_DBDESC:
        /* RFC5340 A.3.3, packet header + OSPF6_DB_DESC_MIN_SIZE bytes followed
           by N>=0 header-only LSAs. */
        test = ospf6_lsaseq_examin ((struct ospf6_lsa_header *) ((caddr_t) oh + OSPF6_HEADER_SIZE + OSPF6_DB_DESC_MIN_SIZE), bytesonwire - OSPF6_HEADER_SIZE - OSPF6_DB_DESC_MIN_SIZE, 1, 0);
        break;
    ...
    case OSPF6_MESSAGE_TYPE_LSUPDATE:
        ...
        test = ospf6_lsaseq_examin ((struct ospf6_lsa_header *) ((caddr_t) lsupd + OSPF6_LS_UPD_MIN_SIZE), bytesonwire - OSPF6_HEADER_SIZE - OSPF6_LS_UPD_MIN_SIZE, 0, ntohl (lsupd->lsa_number)	/* 32 bits */
                                   );
        break;
    case OSPF6_MESSAGE_TYPE_LSACK:
        /* RFC5340 A.3.6, packet header + N>=0 header-only LSAs. */
        test = ospf6_lsaseq_examin ((struct ospf6_lsa_header *) ((caddr_t) oh + OSPF6_HEADER_SIZE + OSPF6_LS_ACK_MIN_SIZE), bytesonwire - OSPF6_HEADER_SIZE - OSPF6_LS_ACK_MIN_SIZE, 1, 0);
        break;
    default:
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
...
static int ospf6_rxpacket_examin (struct ospf6_interface *oi, struct ospf6_header *oh, const unsigned bytesonwire)
{
    ...
    if (oh->area_id != oi->area->area_id)
    {
        if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
        {
            if (oh->area_id == BACKBONE_AREA_ID)
                ...
                zlog_debug
                ("%s: Area-ID mismatch (my %s, rcvd %s)", __func__, inet_ntop (AF_INET, &oi->area->area_id, buf[0], INET_ADDRSTRLEN), inet_ntop (AF_INET, &oh->area_id, buf[1], INET_ADDRSTRLEN));
        }
        return MSG_NG;
    }
...
static void ospf6_lsupdate_recv (struct in6_addr *src, struct in6_addr *dst, struct ospf6_interface *oi, struct ospf6_header *oh)
{
    struct ospf6_neighbor *on;
    struct ospf6_lsupdate *lsupdate;
    char *p;
    ...
    if (on->state != OSPF6_NEIGHBOR_EXCHANGE && on->state != OSPF6_NEIGHBOR_LOADING && on->state != OSPF6_NEIGHBOR_FULL && on->state != OSPF6_NEIGHBOR_LEAVING)
    {
        if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
            zlog_debug ("Neighbor state less than Exchange, ignore");
        return;
    ...
    if (on->state != OSPF6_NEIGHBOR_LEAVING)
    {
        ...
        if (!IN6_IS_ADDR_MULTICAST (dst) && (on->state == OSPF6_NEIGHBOR_EXCHANGE || on->state == OSPF6_NEIGHBOR_LOADING))
        {
...
static void ospf6_lsack_recv (struct in6_addr *src, struct in6_addr *dst, struct ospf6_interface *oi, struct ospf6_header *oh)
{
    struct ospf6_neighbor *on;
    char *p;
    struct ospf6_lsa *his, *mine;
    ...
    for (p = (char *) ((caddr_t) oh + sizeof (struct ospf6_header)); p + sizeof (struct ospf6_lsa_header) <= OSPF6_MESSAGE_END (oh); p += sizeof (struct ospf6_lsa_header))
    {
        ...
        mine = ospf6_lsdb_lookup (his->header->type, his->header->id, his->header->adv_router, on->retrans_list);
        if (mine == NULL)
        {
            if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
        ...
        if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
            zlog_debug ("Acknowledged, remove from %s's retrans-list", on->name);
...
int ospf6_receive (struct thread *thread)
{
    ...
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
            zlog_debug ("%s: Ignore message on passive interface %s", __func__, oi->interface->name);
        return 0;
    }
#if 1
    int i, index = -1;
    ...
    {
        if ((ostats[i].ifindex == 0) && (index == -1))
            index = i;

        if (ostats[i].ifindex == ifindex)
        {
            index = i;
            break;
        }
    }
    if (i == MAXINTERFACENUM)
        ostats[index].ifindex  = ifindex;
#endif
    ...
    if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
    {
        ...
        zlog_debug ("%s received on %s", LOOKUP (ospf6_message_type_str, oh->type), oi->interface->name);
    ...
    {
    case OSPF6_MESSAGE_TYPE_HELLO:
        ostats[index].hello_recv_nums++;
        ostats[index].hello_recv_bytes += len;
        ospf6_hello_recv (&src, &dst, oi, oh);
        break;

    case OSPF6_MESSAGE_TYPE_DBDESC:
        ostats[index].dbdesc_recv_nums++;
        ostats[index].dbdesc_recv_bytes += len;
        ospf6_dbdesc_recv (&src, &dst, oi, oh);
        break;

    case OSPF6_MESSAGE_TYPE_LSREQ:
        ostats[index].lsreq_recv_nums++;
        ostats[index].lsreq_recv_bytes += len;
        ospf6_lsreq_recv (&src, &dst, oi, oh);
        break;

    case OSPF6_MESSAGE_TYPE_LSUPDATE:
        ostats[index].lsupdate_recv_nums++;
        ostats[index].lsupdate_recv_bytes += len;
        ospf6_lsupdate_recv (&src, &dst, oi, oh);
        break;

    case OSPF6_MESSAGE_TYPE_LSACK:
        ostats[index].lsack_recv_nums++;
        ostats[index].lsack_recv_bytes += len;
...
static void ospf6_send (struct in6_addr *src, struct in6_addr *dst, struct ospf6_interface *oi, struct ospf6_header *oh)
{
    int len;
    char srcname[64], dstname[64];
    struct iovec iovector[2];
    ...
    if (IS_OSPF6_DEBUG_MESSAGE (oh->type, SEND))
    {
        ...
        zlog_debug ("%s send on %s", LOOKUP (ospf6_message_type_str, oh->type), oi->interface->name);
        zlog_debug ("    src: %s", srcname);
        zlog_debug ("    dst: %s", dstname);
    ...
    for (i = 0; i < MAXINTERFACENUM; i++)
    {
        ...
        if (ostats[i].ifindex == oi->interface->ifindex)
        {
            index = i;
            break;
        }
    ...
    if (i == MAXINTERFACENUM)
        ostats[index].ifindex  = oi->interface->ifindex;
#endif
    ...
    {
    case OSPF6_MESSAGE_TYPE_HELLO:
        ostats[index].hello_send_nums++;
        ostats[index].hello_send_bytes += len;
        break;

    case OSPF6_MESSAGE_TYPE_DBDESC:
        ostats[index].dbdesc_send_nums++;
        ostats[index].dbdesc_send_bytes += len;
        break;

    case OSPF6_MESSAGE_TYPE_LSREQ:
        ostats[index].lsreq_send_nums++;
        ostats[index].lsreq_send_bytes += len;
        break;

    case OSPF6_MESSAGE_TYPE_LSUPDATE:
        ostats[index].lsupdate_send_nums++;
        ostats[index].lsupdate_send_bytes += len;
        break;

    case OSPF6_MESSAGE_TYPE_LSACK:
        ostats[index].lsack_send_nums++;
        ostats[index].lsack_send_bytes += len;
        break;
...
int ospf6_hello_send (struct thread *thread)
{
    ...
    if (oi->state <= OSPF6_INTERFACE_DOWN)
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_HELLO, SEND))
            zlog_debug ("Unable to send Hello on down interface %s", oi->interface->name);
...
int ospf6_dbdesc_send (struct thread *thread)
{
    ...
    if (on->state < OSPF6_NEIGHBOR_EXSTART)
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_DBDESC, SEND))
            zlog_debug ("Quit to send DbDesc to neighbor %s state %s", on->name, ospf6_neighbor_state_str[on->state]);
        return 0;
    }

    /* set next thread if master */
    if (CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_MSBIT))
        on->thread_send_dbdesc = thread_add_timer (master, ospf6_dbdesc_send, on, on->ospf6_if->rxmt_interval);
    ...
    if (CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_IBIT))
    {
        zlog_debug("%s()%d IBIT set, state:%s, neighbor ipv6 address:%s ", __func__, __LINE__, ospf6_neighbor_state_str[on->state], ipv6_string);
    }
#endif
    ...
    if (!CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_IBIT))
    {
        for (lsa = ospf6_lsdb_head (on->dbdesc_list); lsa; lsa = ospf6_lsdb_next (lsa))
        {
    ...
    ospf6_send (on->ospf6_if->linklocal_addr, &on->linklocal_addr, on->ospf6_if, oh);
    return 0;
}


int ospf6_dbdesc_send_new_lsas_list (struct thread *thread)
{
    struct ospf6_neighbor *on;
    struct ospf6_lsa *lsa;
    unsigned int size = 0;
    ...
    for (lsa = ospf6_lsdb_head (on->summary_list); lsa; lsa = ospf6_lsdb_next (lsa))
    {
...
int ospf6_lsreq_send (struct thread *thread)
{
    ...
    if (on->state != OSPF6_NEIGHBOR_EXCHANGE && on->state != OSPF6_NEIGHBOR_LOADING)
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSREQ, SEND))
            zlog_debug ("Quit to send LSReq to neighbor %s state %s", on->name, ospf6_neighbor_state_str[on->state]);
        return 0;
    }
    ...
    /* set next thread */
    on->thread_send_lsreq = thread_add_timer (master, ospf6_lsreq_send, on, on->ospf6_if->rxmt_interval);
    ...
    for (lsa = ospf6_lsdb_head (on->request_list); lsa; lsa = ospf6_lsdb_next (lsa))
    {
        /* MTU check */
...
#if 1
int ospf6_lsupdate_send_neighbor_for_new_lsas (struct thread *thread)
{
    struct ospf6_neighbor *on;
    struct ospf6_header *oh;
    struct ospf6_lsupdate *lsupdate;
    ...
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSUPDATE, SEND))
            zlog_debug ("Quit to send (neighbor state %s)", ospf6_neighbor_state_str[on->state]);
        return 0;
    }

    /* if we have nothing to send, return */
    if (on->ospf6_if->new_lsas_list->count == 0)
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSUPDATE, SEND))
    ...
    for (lsa = ospf6_lsdb_head (on->ospf6_if->new_lsas_list); lsa; lsa = ospf6_lsdb_next (lsa))
    {
        /* MTU check */
        if ((p - sendbuf + (unsigned int) OSPF6_LSA_SIZE (lsa->header)) > ospf6_packet_max (on->ospf6_if))
        {
            break;
        }
    ...
    if (on->lsupdate_list->count != 0 || on->retrans_list->count != 0)
    {
        if (on->lsupdate_list->count != 0)
            ...
            on->thread_send_lsupdate = thread_add_timer (master, ospf6_lsupdate_send_neighbor, on, on->ospf6_if->rxmt_interval);
    }
#endif
...
int ospf6_lsupdate_send_neighbor (struct thread *thread)
{
    ...
    /* lsupdate_list lists those LSA which doesn't need to be
       retransmitted. remove those from the list */
    for (lsa = ospf6_lsdb_head (on->lsupdate_list); lsa; lsa = ospf6_lsdb_next (lsa))
    {
        /* MTU check */
        if ((p - sendbuf + (unsigned int) OSPF6_LSA_SIZE (lsa->header)) > ospf6_packet_max (on->ospf6_if))
        {
...
#if 1
static int ospf6_print_lsa_prefix (struct ospf6_prefix *current, unsigned length, const u_int32_t req_num_pfxs)
{
    u_char requested_pfx_bytes;
    struct in6_addr in6;
    char buf[128];
    ...
    while (length)
    {
        ...
        memcpy (&in6, OSPF6_PREFIX_BODY (current), OSPF6_PREFIX_SPACE (current->prefix_length));
        inet_ntop (AF_INET6, &in6, buf, sizeof (buf));
        zlog_debug ("%s()%d lsa header type is OSPF6_LSTYPE_INTRA_PREFIX, prefix:%s/%d", __func__, __LINE__, buf, current->prefix_length);
...
int ospf6_print_lsa_header_type (struct ospf6_lsa_header *lsah)
{
    int ret = 0;
    u_int16_t lsatype;
    u_int16_t lsalen;
    ...
    zlog_debug ("%s()%d print lsa header.\n", __func__, __LINE__);
    ospf6_lsa_header_print_raw (lsah);
    switch (lsatype)
    {
    case OSPF6_LSTYPE_ROUTER:
        zlog_debug ("lsa header type is OSPF6_LSTYPE_ROUTER");
        break;
    case OSPF6_LSTYPE_NETWORK:
        zlog_debug ("lsa header type is OSPF6_LSTYPE_NETWORK");
        break;
    case OSPF6_LSTYPE_INTER_PREFIX:
        zlog_debug ("lsa header type is OSPF6_LSTYPE_INTER_PREFIX");
        break;

    case OSPF6_LSTYPE_INTER_ROUTER:
        zlog_debug ("lsa header type is OSPF6_LSTYPE_INTER_ROUTER");
        break;
    case OSPF6_LSTYPE_AS_EXTERNAL:	/* RFC5340 A.4.7, same as A.4.8. */
    case OSPF6_LSTYPE_TYPE_7:
        zlog_debug ("lsa header type is OSPF6_LSTYPE_AS_EXTERNAL or OSPF6_LSTYPE_TYPE_7");
        as_external_lsa = (struct ospf6_as_external_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
        exp_length = OSPF6_LSA_HEADER_SIZE + OSPF6_AS_EXTERNAL_LSA_MIN_SIZE;
        /* To find out if the last optional field (Referenced Link State ID) is
         * assumed in this LSA, we need to access fixed fields of the IPv6
        ...
        ret = ospf6_print_lsa_prefix ((struct ospf6_prefix *) ((caddr_t) as_external_lsa + OSPF6_AS_EXTERNAL_LSA_MIN_SIZE), lsalen - exp_length, 1);
        break;
    case OSPF6_LSTYPE_LINK:
        zlog_debug ("lsa header type is OSPF6_LSTYPE_LINK");
        link_lsa = (struct ospf6_link_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
        ospf6_print_lsa_prefix ((struct ospf6_prefix *) ((caddr_t) link_lsa + OSPF6_LINK_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_LINK_LSA_MIN_SIZE,
                                ntohl (link_lsa->prefix_num) /* 32 bits */ );
        break;
    case OSPF6_LSTYPE_INTRA_PREFIX:
        zlog_debug ("lsa header type is OSPF6_LSTYPE_INTRA_PREFIX");
        intra_prefix_lsa = (struct ospf6_intra_prefix_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
        ret =
            ospf6_print_lsa_prefix ((struct ospf6_prefix *) ((caddr_t) intra_prefix_lsa + OSPF6_INTRA_PREFIX_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_INTRA_PREFIX_LSA_MIN_SIZE,
...
static inline int __ipv6_prefix_equal (const unsigned int *a1, const unsigned *a2, unsigned int prefixlen)
{
    unsigned pdw, pbi;
...
static int ospf6_examine_lsa_prefix (struct ospf6_prefix *current,  unsigned length, const u_int32_t req_num_pfxs, struct prefix_ipv6 *p)
{
    u_char requested_pfx_bytes;
    struct in6_addr in6;
    char buf[128];
    ...
    while (length)
    {
        ...
        memcpy (&in6, OSPF6_PREFIX_BODY (current), OSPF6_PREFIX_SPACE (current->prefix_length));
        inet_ntop (AF_INET6, &in6, buf, sizeof (buf));
        printf ("%s()%d lsa header type is OSPF6_LSTYPE_INTRA_PREFIX, prefix:%s/%d\n", __func__, __LINE__, buf, current->prefix_length);

        if (p->prefixlen == current->prefix_length)
        {
            if (ipv6_prefix_equal (&p->prefix, &in6, p->prefixlen) == 1)
            {
                printf ("%s()%d the address is same, return 1\n", __func__, __LINE__);
                return 1;
            }
        }
...
#ifdef TWOD
static int ospf6_compare_lsa_prerfix_2 (struct in6_addr *in6_in_lsdb, u_int8_t prefix_length_in_lsdb, u_int32_t adv_router_in_lsdb, struct ospf6_lsa_header *lsah)
{
    u_int16_t lsalen;
    struct ospf6_intra_prefix_lsa *intra_prefix_lsa;
    struct in6_addr in6;
    ...
    lsalen = OSPF6_LSA_SIZE (lsah);
    intra_prefix_lsa = (struct ospf6_intra_prefix_lsa *) ((caddr_t) (lsah) + OSPF6_LSA_HEADER_SIZE);
    struct ospf6_prefix *current = (struct ospf6_prefix *) ((caddr_t) intra_prefix_lsa + OSPF6_INTRA_PREFIX_LSA_MIN_SIZE);
    unsigned length = lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_INTRA_PREFIX_LSA_MIN_SIZE;
    ...
    while (length)
    {
        ...
        inet_ntop (AF_INET6, &in6, buf, sizeof (buf));
        zlog_debug ("%s()%d lsah , prefix:%s/%d", __func__, __LINE__, buf, current->prefix_length);

        if (current->prefix_length == prefix_length_in_lsdb)
        {
            if (ipv6_prefix_equal (in6_in_lsdb, &in6, current->prefix_length) == 1)
            {
                /*compare router id */
                if (adv_router_in_lsdb != lsah->adv_router)
                    return 1;
            }
        }
...
#ifdef TWOD
static int ospf6_compare_lsa_prefix (struct ospf6_prefix *current, unsigned length, const u_int32_t req_num_pfxs, u_int32_t adv_router, struct ospf6_lsa_header *lsah)
{
    u_char requested_pfx_bytes;
    struct in6_addr in6;
    char buf[128];
    ...
    while (length)
    {
        ...
        inet_ntop (AF_INET6, &in6, buf, sizeof (buf));
        zlog_debug ("%s()%d lsa header  prefix:%s/%d", __func__, __LINE__, buf, current->prefix_length);

        ret = ospf6_compare_lsa_prerfix_2 (&in6, current->prefix_length, adv_router, lsah);
        if (ret == 1)
            return ret;
...
#ifdef TWOD
static int ospf6_lsa_get_internal (struct ospf6_lsa *lsa, struct ospf6_lsa_header *lsah)
{
    int ret;
    u_int16_t lsalen;
    struct ospf6_intra_prefix_lsa *intra_prefix_lsa;
    lsalen = OSPF6_LSA_SIZE (lsa->header);
    u_int32_t adv_router = lsa->header->adv_router;
#if 1
    printf ("print lsa in lsdb header\n");
    ospf6_lsa_header_print_raw (lsa->header);
    printf ("print lsah header\n");
    ospf6_lsa_header_print_raw (lsah);

#endif

    intra_prefix_lsa = (struct ospf6_intra_prefix_lsa *) ((caddr_t) (lsa->header) + OSPF6_LSA_HEADER_SIZE);
    ret =
        ospf6_compare_lsa_prefix ((struct ospf6_prefix *) ((caddr_t) intra_prefix_lsa + OSPF6_INTRA_PREFIX_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_INTRA_PREFIX_LSA_MIN_SIZE,
                                  ntohs (intra_prefix_lsa->prefix_num) /* 16 bits */ , adv_router, lsah);
...
{
    struct ospf6_lsa *lsa;
    struct ospf6_lsa_header *lsah_new = lsah;
    int ret = 0;
    ...
    while (lsa)
    {
        ret = ospf6_lsa_get_internal (lsa, lsah_new);
        if (ret == 1)
            return ret;
...
#define OSPF6_LSDB_GET_LEVEL_DETAIL 1
static int ospf6_find_lsa_prefix_in_lsdb (struct ospf6_lsa_header *lsah)
{
    ...
    struct ospf6_interface *oi;
    struct ospf6_lsa_header *lsah_one = lsah;

    type = lsah->type;
    level = OSPF6_LSDB_GET_LEVEL_DETAIL;
    switch (OSPF6_LSA_SCOPE (type))
    {
    case OSPF6_SCOPE_AREA:
        printf (">>>> OSPF6_SCOPE_AREA<<<\n");
        for (ALL_LIST_ELEMENTS_RO (o->area_list, i, oa))
        {
            ret = ospf6_lsdb_get_prefix (level, &type, oa->lsdb, lsah_one);
            if (ret == 1)
                return ret;
        }
        break;
    ...
    case OSPF6_SCOPE_LINKLOCAL:
        printf (">>>>>OSPF6_SCOPE_LINKLOCAL<<<<<\n");
        for (ALL_LIST_ELEMENTS_RO (o->area_list, i, oa))
        {
            for (ALL_LIST_ELEMENTS_RO (oa->if_list, j, oi))
            {
                ret = ospf6_lsdb_get_prefix (level, &type, oi->lsdb, lsah_one);
                if (ret == 1)
                    return ret;
            }
        }
    ...
    case OSPF6_SCOPE_AS:
        printf (">>>>>>OSPF6_SCOPE_AS<<<<<<<");
        ret = ospf6_lsdb_get_prefix (level, &type, o->lsdb, lsah_one);
        break;
...
static int ospf6_get_lsa_header_type (struct ospf6_lsa_header *lsah, struct prefix_ipv6 *p)
{
    int ret = 0;
    u_int16_t lsatype;
    u_int16_t lsalen;
    ...
    case OSPF6_LSTYPE_ROUTER:
#ifdef OSPF6_DEBUG
        zlog_debug ("[%d]%s() lsa header type is OSPF6_LSTYPE_ROUTER", __LINE__, __func__);
#endif
        break;
    case OSPF6_LSTYPE_NETWORK:
#ifdef OSPF6_DEBUG
        zlog_debug ("[%d]%s() lsa header type is OSPF6_LSTYPE_NETWORK", __LINE__, __func__);
#endif
        break;
    case OSPF6_LSTYPE_INTER_PREFIX:
#ifdef OSPF6_DEBUG
        zlog_debug ("[%d]%s() lsa header type is OSPF6_LSTYPE_INTER_PREFIX", __LINE__, __func__);
#endif
        break;

    case OSPF6_LSTYPE_INTER_ROUTER:
#ifdef OSPF6_DEBUG
        zlog_debug ("[%d]%s() lsa header type is OSPF6_LSTYPE_INTER_ROUTER", __LINE__, __func__);
#endif
        break;
    case OSPF6_LSTYPE_AS_EXTERNAL:	/* RFC5340 A.4.7, same as A.4.8. */
    case OSPF6_LSTYPE_TYPE_7:
#ifdef OSPF6_DEBUG
        zlog_debug ("%s()%d lsa header type is OSPF6_LSTYPE_AS_EXTERNAL or OSPF6_LSTYPE_TYPE_7", __func__, __LINE__);
#endif
        as_external_lsa = (struct ospf6_as_external_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
        exp_length = OSPF6_LSA_HEADER_SIZE + OSPF6_AS_EXTERNAL_LSA_MIN_SIZE;
        /* To find out if the last optional field (Referenced Link State ID) is
        ...
        ret = ospf6_examine_lsa_prefix ((struct ospf6_prefix *) ((caddr_t) as_external_lsa + OSPF6_AS_EXTERNAL_LSA_MIN_SIZE), lsalen - exp_length, 1, p);
        break;
    case OSPF6_LSTYPE_LINK:
#ifdef OSPF6_DEBUG
        zlog_debug ("%s()%d lsa header type is OSPF6_LSTYPE_LINK", __func__, __LINE__);
#endif
        link_lsa = (struct ospf6_link_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
        ret =
            ospf6_examine_lsa_prefix ((struct ospf6_prefix *) ((caddr_t) link_lsa + OSPF6_LINK_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_LINK_LSA_MIN_SIZE,
                                      ntohl (link_lsa->prefix_num) /* 32 bits */ , p);
        break;
    case OSPF6_LSTYPE_INTRA_PREFIX:

#ifdef OSPF6_DEBUG
        /*find lsd prefix in lsdb */
        zlog_debug ("%s()%d lsa header type is OSPF6_LSTYPE_INTRA_PREFIX", __func__, __LINE__);
#endif
        intra_prefix_lsa = (struct ospf6_intra_prefix_lsa *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
        ret =
            ospf6_examine_lsa_prefix ((struct ospf6_prefix *) ((caddr_t) intra_prefix_lsa + OSPF6_INTRA_PREFIX_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_INTRA_PREFIX_LSA_MIN_SIZE,
                                      ntohs (intra_prefix_lsa->prefix_num) /* 16 bits */ , p);
        break;
    }
...
static void get_oi_v6_address (struct ospf6_interface *oi, struct prefix_ipv6 *ipv6)
{
    ...
    for (ALL_LIST_ELEMENTS_RO (ifp->connected, node, connected))
    {
        if ((connected->address->family == AF_INET6))
        {
            ...
            inet_ntop (AF_INET6, p->prefix.s6_addr, buf, sizeof (buf));
            printf ("%s()%d the interface:%s address is:%s/%d\n", __func__, __LINE__, ifp->name, buf, p->prefixlen);
            memcpy (ipv6, p, sizeof (struct prefix_ipv6));
            return;
        }
    }
    return;
...
int ospf6_check_lsa_maxage_prefix (struct ospf6_interface *oi, struct ospf6_lsa *lsa)
{
    int ret = 0;
    ...
    if (OSPF6_LSA_IS_MAXAGE (lsa))
    {
        ...
        memset (&ipv6, 0, sizeof (struct prefix_ipv6));
        get_oi_v6_address (oi, &ipv6);

        ret = ospf6_get_lsa_header_type (lsa->header, &ipv6);

        //zlog_debug ("%s()%d maxage lsa:%s, ret:%d", __func__, __LINE__, lsa->name, ret);
        return ret;
    }
...
int ospf6_lsupdate_send_interface (struct thread *thread)
{
    ...
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSUPDATE, SEND))
            zlog_debug ("Quit to send LSUpdate to interface %s state %s", oi->interface->name, ospf6_interface_state_str[oi->state]);
        return 0;
    }
    ...
    for (lsa = ospf6_lsdb_head (oi->lsupdate_list); lsa; lsa = ospf6_lsdb_next (lsa))
    {
        /* MTU check */
        if ((p - sendbuf + ((unsigned int) OSPF6_LSA_SIZE (lsa->header))) > ospf6_packet_max (oi))
        {
            ospf6_lsa_unlock (lsa);
            break;
        }
        ...
        if (OSPF6_LSA_IS_MAXAGE (lsa))
        {
            ret = ospf6_check_lsa_maxage_prefix (oi, lsa);
        }
    ...
    if (oi->state == OSPF6_INTERFACE_DR || oi->state == OSPF6_INTERFACE_BDR)
        ospf6_send (oi->linklocal_addr, &allspfrouters6, oi, oh);
    else
    ...
    if (oi->lsupdate_list->count > 0)
    {
        oi->thread_send_lsupdate = thread_add_event (master, ospf6_lsupdate_send_interface, oi, 0);
    }
...
int ospf6_lsack_send_neighbor (struct thread *thread)
{
    ...
    if (on->state < OSPF6_NEIGHBOR_EXCHANGE)
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSACK, SEND))
            zlog_debug ("Quit to send LSAck to neighbor %s state %s", on->name, ospf6_neighbor_state_str[on->state]);
        return 0;
    }
    ...
    for (lsa = ospf6_lsdb_head (on->lsack_list); lsa; lsa = ospf6_lsdb_next (lsa))
    {
        /* MTU check */
        if (p - sendbuf + sizeof (struct ospf6_lsa_header) > ospf6_packet_max (on->ospf6_if))
        {
            ...
            on->thread_send_lsack = thread_add_event (master, ospf6_lsack_send_neighbor, on, 0);
...
int ospf6_lsack_send_interface (struct thread *thread)
{
    ...
    {
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSACK, SEND))
            zlog_debug ("Quit to send LSAck to interface %s state %s", oi->interface->name, ospf6_interface_state_str[oi->state]);
        return 0;
    }
    ...
    for (lsa = ospf6_lsdb_head (oi->lsack_list); lsa; lsa = ospf6_lsdb_next (lsa))
    {
        /* MTU check */
        if (p - sendbuf + sizeof (struct ospf6_lsa_header) > ospf6_packet_max (oi))
        {
            ...
            oi->thread_send_lsack = thread_add_event (master, ospf6_lsack_send_interface, oi, 0);
...
ALIAS (debug_ospf6_message,	   debug_ospf6_message_sendrecv_cmd,
...
int config_write_ospf6_debug_message (struct vty *vty)
{
    ...
    if (!IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, SEND) && !IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
    ...
    for (i = 1; i < 6; i++)
    {
        if (IS_OSPF6_DEBUG_MESSAGE (i, SEND) && IS_OSPF6_DEBUG_MESSAGE (i, RECV))
            ...
            vty_out (vty, "debug ospf6 message %s send%s", type_str[i], VNL);
        else if (IS_OSPF6_DEBUG_MESSAGE (i, RECV))
            vty_out (vty, "debug ospf6 message %s recv%s", type_str[i], VNL);
    }
