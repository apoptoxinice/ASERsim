    ...
    if (oi->reference_count > 0)
    {
        ...
        if (lsa_bak == NULL)
        {
            ...
            if (ospf6_check_lsa_maxage_prefix (oi, lsa) == 0)
            {
                ...
                {
                    zlog_debug ("%s()%d The interface %s has %d neighbor in leaving, then install lsa to new lsas list, type:%u", __func__, __LINE__, oi->interface->name, oi->reference_count, lsatype);
