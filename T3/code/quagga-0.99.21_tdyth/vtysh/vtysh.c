...
/*added by zhangzhibo 2013.8.12 for show arp*/
#include <sys/stat.h>
#include <unistd.h>
#include <stdio.h>
#include <stdlib.h>
...
#define MAX_PAYLOAD 102400  /* maximum payload size*/
#define HAVE_DHCPV4
...
typedef struct
{
    ...
    unsigned char HWtype[21];
    unsigned char FlagsMask[5];
    unsigned char Iface[16];
    unsigned char HWaddress[7];
    unsigned long Expire;
    unsigned long LastRand;
} __attribute__((packed)) ArpTable;

//add for encrypt enable password by zhangzhibo
...
//added by wangyl for return to linux terminal
    if(vty->node == ENABLE_NODE)
    {
        if(strncmp (line, "Aux-bitway", strlen ("Aux-bitway")) == 0)
            exit(0);
...
//added end
    ...
    if (ret == CMD_SUCCESS || ret == CMD_SUCCESS_DAEMON || ret == CMD_WARNING)
    {
        if ((saved_node == BGP_VPNV4_NODE || saved_node == BGP_IPV4_NODE
                ...
                || saved_node == BGP_IPV6M_NODE || saved_node == BGP_IP4OVER6_NODE)/*added by wangyl 2013.3.19 for 4over6*/
                && (tried == 1))
        {
    ...
    */
    ...
    while (fgets (vty->buf, VTY_BUFSIZ, fp))
    {
        if (vty->buf[0] == '!' || vty->buf[1] == '#')
            continue;
        if(vty->buf[0]=='e'&& vty->buf[1]=='n'&& vty->buf[2]=='a')
        {
            char *p=strtok(vty->buf," 	");
            p=strtok(NULL," 	");
            p=strtok(NULL," 	\n");
            host.enable=malloc(100*sizeof(char));
            strcpy(host.enable,p);
            continue;
        }
...
//added by wangyl for dhcp 20130901
static struct cmd_node dhcp_node =
{
    DHCP_NODE,
    "%s(config-dhcp)# ",
};
//added end
#if 1//sangmeng add for ipv4 filter
static struct cmd_node ipv4_access_node =
{
    IPV4_ACCESS_NODE,
    "%s(config-ip-acl)# ",
};
#endif
#if 1//sangmeng add for ipv6 filter
static struct cmd_node ipv6_access_node =
{
    IPV6_ACCESS_NODE,
    "%s(config-ipv6-acl)# ",
};
...
static struct cmd_node tunnel_node =
{
    TUNNEL_NODE,
    "%s(config-tunnel)# ",
};
...
/*added by wangyl 2013.3.19 for 4over6*/
static struct cmd_node bgp_ipv4over6_node =
{
    BGP_IP4OVER6_NODE,
    "%s(config-router-af)# "
};
...
#endif
/*sangmeng add for ipv4 filter*/
#if 1
DEFUNSH (VTYSH_ZEBRA,
         ip_access_listnumber,
         ip_access_listnumber_cmd,
         "ip access-list WORD",
         IP_STR
         "List IP access lists\n"
         "Access-list name\n")
{
    vty->node = IPV4_ACCESS_NODE;
...
/*sangmeng add for ipv6 filter*/
#if 1
DEFUNSH (VTYSH_ZEBRA,
         ipv6_access_listnumber,
         ipv6_access_listnumber_cmd,
         "ipv6 access-list WORD",
         IPV6_STR
         "List IPV6 access lists\n"
         "IPV6 access-list name\n")
{
    vty->node = IPV6_ACCESS_NODE;
...
/*added by wangyl 2013.3.19 for 4over6*/
DEFUNSH (VTYSH_BGPD,
         address_family_ipv4over6,
         address_family_ipv4over6_cmd,
         "address-family ipv4 4over6",
         "Enter Address Family command mode\n"
         "Address family\n"
         "Address Family Modifier\n")
{
    vty->node = BGP_IP4OVER6_NODE;
    return CMD_SUCCESS;
}
...
void get_cmd_line(char *cmd, int n)
{
    int i = 0;
    char c;
    terminal_echo_off(STDIN_FILENO);
       ...
       "Turn on privileged mode command\n")
{
    ...
    memset(a,'\0',sizeof(a));
    int i=0;
    if(vtysh_passwd_control.vtysh_enable_pass==1)
        vty->node = ENABLE_NODE;
    else
//vty->node = AUTH_ENABLE_NODE;
    {
        vty_out(vty,"Password:");
        get_cmd_line(a,sizeof(a));
        if((strcmp(zencrypt(a),host.enable))==0)
            vty->node=ENABLE_NODE;
        else
        {
#if 1
...
vtysh_exit (struct vty *vty)
{
    ...
    case ENABLE_NODE:
        //exit (0);
        system("killall -9 in.telnetd 2>&1 | tee > /dev/null");
        break;
    case CONFIG_NODE:
        vty->node = ENABLE_NODE;
        break;
    case INTERFACE_NODE:
    case TUNNEL_NODE://add by ccc for tunnel
    case ZEBRA_NODE:
    case BGP_NODE:
    case RIP_NODE:
    case RIPNG_NODE:
    ...
    case RMAP_NODE:
    case VTY_NODE:
        //added by wangyl for dhcp 20130801
    case DHCP_NODE:
    case IPV4_ACCESS_NODE: //sangmeng add for ipv4 filter
    case IPV6_ACCESS_NODE: //sangmeng add for ipv6 filter
    case KEYCHAIN_NODE:
        vtysh_execute("end");
        vtysh_execute("configure terminal");
        vty->node = CONFIG_NODE;
...
ALIAS (vtysh_exit_ipv4_acl,
       vtysh_quit_ipv4_acl_cmd,
       "quit",
       "Exit current mode and down to previous mode\n")
ALIAS (vtysh_exit_ipv6_acl,
       vtysh_quit_ipv6_acl_cmd,
       "quit",
       "Exit current mode and down to previous mode\n")
...
DEFUNSH(VTYSH_ALL,
		...
		vtysh_tunnel_cmd,
		"interface tunnel <0-32>",
		"select one tunnel to configure\n"
		"tunnel num\n"
		)
{
	vty->node = TUNNEL_NODE;
...
/*add by zhangzhibo 2013.8.28 for show ipv6 neighbor*/
DEFUN (vtysh_show_ipv6_neighbor,
       vtysh_show_ipv6_neighbor_cmd,
       "show ipv6 neighbor",
       ...
       "ARP information\n")
{
    ...
    ArpTable RecvArp[((ArpTable *)NLMSG_DATA(nlh))->Num];
    memset(RecvArp,0,sizeof(RecvArp));
    memcpy(RecvArp,NLMSG_DATA(nlh),(((ArpTable *)NLMSG_DATA(nlh))->Num)*sizeof(ArpTable));
    int i;
    if(((ArpTable *)NLMSG_DATA(nlh))->Num!=0)
        vty_out(vty,"%-20s%-22s%-19s%-21s%-16s\n","Address","HWtype","HWaddress","Age(s)","Iface");
    for(i=0; i<(((ArpTable *)NLMSG_DATA(nlh))->Num); i++)
    {
        if((RecvArp[i].FlagsMask[0]=='M')||(RecvArp[i].FlagsMask[1]=='M')||(RecvArp[i].FlagsMask[2]=='M')||(RecvArp[i].FlagsMask[3]=='M')||(RecvArp[i].FlagsMask[4]=='M'))
            vty_out(vty,"%-20s%-22s%02x:%02x:%02x:%02x:%02x:%02x  %-21s%-16s\n",RecvArp[i].Address,RecvArp[i].HWtype,RecvArp[i].HWaddress[0],RecvArp[i].HWaddress[1],RecvArp[i].HWaddress[2],RecvArp[i].HWaddress[3],RecvArp[i].HWaddress[4],RecvArp[i].HWaddress[5]," - ",RecvArp[i].Iface);
        else
            vty_out(vty,"%-20s%-22s%02x:%02x:%02x:%02x:%02x:%02x  %-21lu%-16s\n",RecvArp[i].Address,RecvArp[i].HWtype,RecvArp[i].HWaddress[0],RecvArp[i].HWaddress[1],RecvArp[i].HWaddress[2],RecvArp[i].HWaddress[3],RecvArp[i].HWaddress[4],RecvArp[i].HWaddress[5],RecvArp[i].Expire,RecvArp[i].Iface);
...
// clock command
DEFUN (vtysh_show_clock,
       vtysh_show_clock_cmd,
       "show clock",
       "SHOW_clock_STR"
       "clock information\n"
       "clock information\n")
{
    system("date '+%Y/%m/%e %a, %H:%M:%S'");
    return CMD_SUCCESS;
}
...
DEFUN (vtysh_config_enable_password,
       ...
       "The HIDDEN 'enable' password string\n")
{
    ...
    if (argc == 1)
    {
        ...
        host.enable =XSTRDUP (MTYPE_HOST, zencrypt (argv[0]));
        return CMD_SUCCESS;
...
DEFUN (no_vtysh_config_enable_password,
       ...
       "Write configuration to the file \n")
{
    ...
    */
    ...
    FILE*fp1=fopen("/usr/local/etc/vtysh.conf","w+");

//FILE*fp2=fopen("/usr/local/etc/zebra.conf","r");
    ...
    if(host.enable)
    {
        fputs("enable password ",fp1);
        fputs(host.enable,fp1);
        fputs("\n",fp1);
    }
#if 0

    fputs("enable password ",fp1);
    while (fgets (zzbw, 512, fp2))
    {
        if (zzbw[0] == '!' || zzbw[1] == '#')
            continue;
        if(zzbw[0]=='e'&& zzbw[1]=='n'&& zzbw[2]=='a'&& zzbw[16]!='8')
        {
            //printf("1 Enter .conf save by hand\n");
            strcpy(zzbw1,zzbw);
            //printf("2 Enter .conf save by hand\n");
            sscanf(zzbw1,"%*s%*s%s",zzbw2);
            //printf("3 Enter .conf save by hand\n");
            if(host.enable==NULL) host.enable=malloc(100*sizeof(char));
            strcpy(host.enable,zzbw2);
            //printf("4 Enter .conf save by hand\n");
            fputs(host.enable,fp1);
            //printf("5 Enter .conf save by hand\n");
        }
        else if(zzbw[0]=='e'&& zzbw[1]=='n'&& zzbw[2]=='a'&& zzbw[16]=='8')
        {
            strcpy(zzbw1,zzbw);
            //sscanf(zzbw1,"%*s%*s%*s%s",zzbw2);
            strcpy(zzbw2,zzbw1+18);
            if(host.enable_encrypt==NULL) host.enable_encrypt=malloc(100*sizeof(char));
            strcpy(host.enable_encrypt,zzbw2);
            fputs("8 ",fp1);
            host.enable_encrypt[strlen(host.enable_encrypt)-1]='\0';
            fputs(host.enable_encrypt,fp1);
        }
        else
            continue;

    }
//printf("6 Enter .conf save by hand\n");
//fputs("\n",fp1);
#endif
    fclose(fp1);
//fclose(fp2);
...
DEFUN (vtysh_clock_set,
       vtysh_clock_set_cmd,
       "clock set WORD WORD WORD WORD WORD WORD",
...
//add at 2013.10.16 by zhangzhibo for ping ipv6 2000::2 length 100
DEFUN (vtysh_ping6_length,
       vtysh_ping6_length_cmd,
       "ping ipv6 WORD length <1-16384>",
       "Send echo messages with packetsize\n"
       "IPv6 echo\n"
       "Ping destination address or hostname\n"
       "IPv6 packetsize\n"
       "Length value\n")
{
    char s[20]= {"-s \0"};
    if(atoi(argv[1])>0 && atoi(argv[1])<16385)
    {
        execute_command ("ping6", 2, argv[0], strcat(s,argv[1]));
        return CMD_SUCCESS;
    }
...
DEFUN (vtysh_traceroute6,
       ...
       "Trace route to destination address or hostname\n")
{
    execute_command ("tracepath6", 1, argv[0], NULL);
    return CMD_SUCCESS;
}
#endif
...
DEFUN(vtysh_copy,
      ...
      "copy ftp: flash0:",
      "Copy files\n"
      "Copy files from ftp server, for large files(>15M)(s test)\n"
      "Copy files from ftp server to the flash0, for large files(>15M)\n"
     )
{
    char ftp_buf[1024] = {0};
    char find_ftp_buf[1024] = {0};
    sprintf(ftp_buf, "%s", "/bitway/ftp/ftp");
    sprintf(find_ftp_buf, "%s", "/bitway/ftp/find_ftp");
    execute_command (ftp_buf, 1, "get", NULL);
    execute_command (find_ftp_buf, 0, NULL, NULL);
    return CMD_SUCCESS;
}
#endif
#if 1
     ...
     )
{
    ...
    char ftp_buf[] = "/bitway/ftp/ftp";
    char cp_buf[] = "cp /root/ftp/ftp /bitway/ftp/";
    char ftp_s_buf[] = "/root/ftp/ftp";
    char rm_buf[] = "rm /root/ftp/ftp";

    execute_command (ftp_buf, 1, "get", NULL);
    printf("\n");
    if ((fp = fopen(ftp_s_buf, "r")) != NULL)
    {
        system(cp_buf);
        system(rm_buf);
        fclose(fp);
...
DEFUN(vtysh_copy_ftp,
      vtysh_copy_ftp_cmd,
      "copy flash0: ftp:",
      "Copy files\n"
      "Copy files from ftp server to the flash0, for large files(>15M)(s test)\n"
      "Copy files from ftp server, for large files(>15M)\n"
     )
{

    char buf[128] = {0};
    sprintf(buf, "%s", "/bitway/ftp/ftp");
    //execute_command ("ftp_put", 0, NULL, NULL);
    execute_command (buf, 1, "put", NULL);
    return CMD_SUCCESS;
}
#endif
...
void connect_damon(void)
{
    ...
    FD_ZERO (&wd_set);
    FD_SET (vtysh_client[0].fd, &wd_set);
    FD_SET (vtysh_client[4].fd, &wd_set);
    FD_SET (vtysh_client[5].fd, &wd_set);

    wd_set_test = wd_set;
    timeout.tv_sec = 5;
    timeout.tv_usec = 0;

    fd1 = vtysh_client[0].fd;
    fd2 = vtysh_client[4].fd;
    fd3 = vtysh_client[5].fd;


    select_r = select (MAX(fd1, fd2, fd3) + 1, NULL, &wd_set_test, NULL, &timeout);
    if (select_r < 0)
    {
    ...
    {
        zlog_debug("[%d]%s() enter here %d %d %d", __LINE__, __func__, vtysh_client[0].fd, vtysh_client[4].fd, vtysh_client[5].fd);
        if (!FD_ISSET (vtysh_client[0].fd, &wd_set_test))
        {
            vclient_close(&vtysh_client[0]);
            zlog_debug("close %s success\n", vtysh_client[0].name);
        }

        if (!FD_ISSET (vtysh_client[4].fd, &wd_set_test))
        {
            vclient_close(&vtysh_client[4]);
            zlog_debug("close %s success\n", vtysh_client[4].name);
        }
        if (!FD_ISSET (vtysh_client[5].fd, &wd_set_test))
        {
            vclient_close(&vtysh_client[5]);
            zlog_debug("close %s success\n", vtysh_client[5].name);
        }
    ...
    char *line = "test";
    int ret = 0;
    ret = write (vtysh_client[0].fd, line, strlen (line) + 1);
    if (ret <= 0)
    {
        vclient_close(&vtysh_client[0]);
    }

    ret = write (vtysh_client[4].fd, line, strlen (line) + 1);
    if (ret <= 0)
    {
        vclient_close(&vtysh_client[5]);
    }
    ret = write (vtysh_client[5].fd, line, strlen (line) + 1);
    if (ret <= 0)
    {
        vclient_close(&vtysh_client[5]);
    }
    ...
    if (vtysh_client[0].fd == -1)
    {
        if (vtysh_connect(&vtysh_client[0]) == 0)
        {
            //        zlog_debug("connect to %s success\n", vtysh_client[0].name);
        }
        else
        {
            //       zlog_debug("connect to %s failed\n", vtysh_client[0].name);
        }
    }

    if (vtysh_client[4].fd == -1)
    {
        if (vtysh_connect(&vtysh_client[4]) == 0)
        {
            //        zlog_debug("connect to %s success\n", vtysh_client[5].name);
        }
        else
    ...
    if (vtysh_client[5].fd == -1)
    {
        if (vtysh_connect(&vtysh_client[5]) == 0)
        {
            //        zlog_debug("connect to %s success\n", vtysh_client[5].name);
        }
        else
...
//add by zhangzhibo
    ...
    //added by wangyl for dhcp 20130801
    install_node(&dhcp_node,NULL);
    //added end
    //sangmeng add for ipv4 filter
    install_node (&ipv4_access_node, NULL);
    //sangmeng add for ipv6 filter
    install_node (&ipv6_access_node, NULL);
    install_node (&bgp_vpnv4_node, NULL);
    install_node (&bgp_ipv4_node, NULL);
    install_node (&bgp_ipv4m_node, NULL);
    /*added by wangyl 2013.3.19 for 4over6*/
    install_node (&bgp_ipv4over6_node, NULL);
    /* #ifdef HAVE_IPV6 */
    ...
    /*added by wangyl 2013.3.19 for 4over6*/
    vtysh_install_default (BGP_IP4OVER6_NODE);
    /*added by wangyl 2013.8.1 for dhcp*/
    vtysh_install_default (DHCP_NODE);
    /*sangmeng add for ipv4 filter*/
    vtysh_install_default (IPV4_ACCESS_NODE);
    /*sangmeng add for ipv6 filter*/
    vtysh_install_default (IPV6_ACCESS_NODE);
    ...
    /*added by wangyl 2013.3.19 for 4over6*/
    install_element (BGP_IP4OVER6_NODE, &vtysh_exit_bgpd_cmd);
    install_element (BGP_IP4OVER6_NODE, &vtysh_quit_bgpd_cmd);
    install_element (BGP_IPV6_NODE, &vtysh_exit_bgpd_cmd);
    install_element (BGP_IPV6_NODE, &vtysh_quit_bgpd_cmd);
    install_element (BGP_IPV6M_NODE, &vtysh_exit_bgpd_cmd);
    install_element (BGP_IPV6M_NODE, &vtysh_quit_bgpd_cmd);
    /*added by wangyl 20130801 dhcp*/
    install_element (DHCP_NODE, &vtysh_exit_dhcp_cmd);
    install_element (DHCP_NODE, &vtysh_quit_dhcp_cmd);
    /*added end*/
#if 1//sangmeng add for ipv4 filter
    install_element (IPV4_ACCESS_NODE, &vtysh_exit_ipv4_acl_cmd);
    install_element (IPV4_ACCESS_NODE, &vtysh_quit_ipv4_acl_cmd);
#endif
#if 1//sangmeng add for ipv6 filter
    install_element (IPV6_ACCESS_NODE, &vtysh_exit_ipv6_acl_cmd);
    install_element (IPV6_ACCESS_NODE, &vtysh_quit_ipv6_acl_cmd);
#endif
    /*add bu ccc for tunnel*/
...
// install_element (ISIS_NODE, &vtysh_exit_isisd_cmd);
    ...
    /*added by wangyl 2013.3.19 for 4over6*/
    install_element (BGP_IP4OVER6_NODE, &vtysh_end_all_cmd);
    /*added by wangyl 2013.8.4 for dhcp end*/
    install_element (DHCP_NODE, &vtysh_end_all_cmd);
    /*sangmeng add for ipv4 filter*/
    install_element (IPV4_ACCESS_NODE, &vtysh_end_all_cmd);
    /*sangmeng add for ipv6 filter*/
    install_element (IPV6_ACCESS_NODE, &vtysh_end_all_cmd);
    //add by ccc for tunnel
...
#endif
    //sangmeng add for ipv4 filter
    install_element (CONFIG_NODE, &ip_access_listnumber_cmd);
    //sangmeng add for ipv6 filter
    install_element (CONFIG_NODE, &ipv6_access_listnumber_cmd);
    install_element (BGP_NODE, &address_family_vpnv4_cmd);
    install_element (BGP_NODE, &address_family_vpnv4_unicast_cmd);
    //added for 4over6 20130409
    install_element (BGP_NODE, &address_family_ipv4_cmd);
    install_element (BGP_NODE, &address_family_ipv4_unicast_cmd);
    install_element (BGP_NODE, &address_family_ipv4_multicast_cmd);
    install_element (BGP_NODE, &address_family_ipv4over6_cmd);
#ifdef HAVE_IPV6
    ...
    /*added by wangyl 2013.3.19 for 4over6*/
    install_element (BGP_IP4OVER6_NODE, &exit_address_family_cmd);
...
//  install_element (ENABLE_NODE, &vtysh_write_cmd);
    ...
    install_element (VIEW_NODE, &vtysh_ping6_length_cmd);
    install_element (VIEW_NODE, &vtysh_traceroute6_cmd);
#endif
    install_element (VIEW_NODE, &vtysh_telnet_cmd);
#ifdef HAVE_FTP
    install_element (VIEW_NODE, &vtysh_copy_cmd);
    install_element (VIEW_NODE, &vtysh_copy_ftp_cmd);
#endif
    ...
    install_element (ENABLE_NODE, &vtysh_ping6_length_cmd);
    install_element (ENABLE_NODE, &vtysh_traceroute6_cmd);
#endif
    install_element (ENABLE_NODE, &vtysh_telnet_cmd);
#ifdef HAVE_FTP
    install_element (ENABLE_NODE, &vtysh_copy_cmd);
    install_element (ENABLE_NODE, &vtysh_copy_ftp_cmd);
#endif
    ...
    /*added by zhangzhibo 2013.10.18 for show clock*/
    install_element (ENABLE_NODE, &vtysh_show_clock_cmd);
    install_element (VIEW_NODE, &vtysh_show_clock_cmd);
    install_element (ENABLE_NODE, &vtysh_clock_set_cmd);
    install_element (VIEW_NODE, &vtysh_clock_set_cmd);
#if 1
    /*added by zhangzhibo 2013.8.28 for show ipv6 neighbor*/
    install_element (ENABLE_NODE, &vtysh_show_ipv6_neighbor_cmd);
    install_element (VIEW_NODE, &vtysh_show_ipv6_neighbor_cmd);
    /*added by zhangzhibo 2013.8.12 for show arp*/
    install_element (ENABLE_NODE, &vtysh_show_arp_cmd);
    install_element (VIEW_NODE, &vtysh_show_arp_cmd);

#endif
//install_element (INTERFACE_NODE, &vtysh_arp_cmd);


// install_element (INTERFACE_NODE, &vtysh_no_arp_cmd);
//install_element (INTERFACE_NODE, &vtysh_ipv6_nd_neighbor_cmd);
    /* Logging */
