...
struct zz_control vtysh_passwd_control= {1,1};
...
extern int bgpd_flag; /* extern from vty.c */
...
static const char *default_motd =
    "\r\n\
BWOS for " QUAGGA_PROGNAME " (version " QUAGGA_VERSION ").\r\n\
" QUAGGA_COPYRIGHT "\r\n\
\r\n";
...
config_write_host (struct vty *vty)
{
    ...
    /* Qinhz add for it does't need to display */
#if 0
                          ...
                          struct cmd_element **cmd)
{
    ...
    for (index = 0; index < vector_active (vline); index++)
        if ((command = vector_slot (vline, index)))
        {
            ...
            //fprintf(stdout,"cmd_execute =ret %dcommand %s\n",ret,command);
            if (ret == 1)
            {
                vector_free (cmd_vector);
...
#endif
                matched_count++;
                //fprintf(stdout,"matchcount = %d\n",matched_count);
                //fprintf(stdout,"real_string = %s\n",cmd_element->string);
            }
            else
            {
                incomplete_count++;
                            ...
                            struct cmd_element **cmd)
{
    ...
    enum match_type match = 0;
    char *command;
    char *neighbor_cmd_str = "neighbor (A.B.C.D|X:X::X:X|WORD) remote-as <1-4294967295>";
    ...
    if((bgpd_flag == 1) && ! strcmp(neighbor_cmd_str,cmd_element->string))
        neighbor_linklist_insert(*argv);
       ...
       "Exit current mode and down to previous mode\n")
{
    ...
    case TUNNEL_NODE: //add by ccc for tunnel
        vty->node = CONFIG_NODE;
    case IPV4_ACCESS_NODE:
        vty->node = CONFIG_NODE;
    case IPV6_ACCESS_NODE:
        vty->node = CONFIG_NODE;
    case ZEBRA_NODE:
    case BGP_NODE:
    case RIP_NODE:
    ...
    case BGP_IPV4_NODE:
    case BGP_IPV4M_NODE:
    case BGP_IP4OVER6_NODE://added for 4over6 20130402
    case BGP_IPV6_NODE:
    case BGP_IPV6M_NODE:
        vty->node = BGP_NODE;
        break;
       ...
       "End current mode and change to enable mode.")
{
    ...
    case BGP_IPV4M_NODE:
    case BGP_IP4OVER6_NODE://added for 4over6 20130402
    case DHCP_NODE://added for dhcp 20130804
...
DEFUN (show_version,
       ...
       "show version",
       SHOW_STR
       "Displays SpaceRouter6000 version\n")
{
    struct sysinfo s_info;
    long uptime = 0;

    vty_out (vty, "SpaceRouter6000 (%s).%s", "1.0.20170310", host.name?host.name:"\r\n",
             VTY_NEWLINE);
    ...
    int leftSeconds= uptime%(60*60);
    int minutes=leftSeconds/60;
    int seconds=leftSeconds%60;
    vty_out(vty,"uptime is %d hour(s), %d minute(s), %d second(s)\n",hours,minutes,seconds);
    return CMD_SUCCESS;
}
       ...
       "Write to configuration file\n")
{
    ...
    if(strcmp(config_file,"/usr/local/etc/zebra.conf")==0)
    {
        vty_out (vty, "Configuration saved to %s%s", "config",
                 VTY_NEWLINE);
    }
    /*
    if(host.enable_encrypt !=NULL)
    {
    FILE *fp3=fopen("/usr/local/etc/zebra.conf","r+");
    fputs("enable password 8 ",fp3);
    fputs(host.enable_encrypt,fp3);
    fputs("\0",fp3);
    fputs("\n",fp3);
    fputs("!",fp3);
    fclose(fp3);
       ...
       "The HIDDEN 'enable' password string\n")
{
    ...
    if (argc == 2)
    {
        if (*argv[0] == '8')
        {
            ...
            //void* digest1=(char*)malloc(200*sizeof(char));
//unsigned char*key="a";
//hmac_md5(argv[1],strlen(argv[1]),key,strlen(key),digest1);
//host.enable_encrypt = XSTRDUP (MTYPE_HOST,digest1);
//free(digest1);
//digest1=NULL;
