...
if_get_by_name_len(const char *name, size_t namelen)
{
    ...
    /* Qinhz modified for doesn't create interface */ /*remove by wangyl ,it may be make some mistake*/
...
/*added by wangyl for doesn't create interface*/
struct interface *
if_get_by_name_len_null(const char *name, size_t namelen)
{
    struct interface *ifp;

    /* Qinhz modified for doesn't create interface */
    /*
    return ((ifp = if_lookup_by_name_len(name, namelen)) != NULL) ? ifp :
     if_create(name, namelen);
    */
...
#else
    /* remove by wangyl for can't create interface
      ifp = if_get_by_name_len(argv[0], sl);
      */
    ifp = if_get_by_name_len_null(argv[0], sl);
#endif /* SUNOS_5 */

    /* Qinhz add for ifp is NULL */
    if(ifp == NULL)
    {
