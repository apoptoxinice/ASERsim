...
extern int bgp_self_reboot_flag;
...
bgp_establish (struct peer *peer)
{
    ...
    /*
     * ADDED by CPT
     * Active all sleep routing, and call update function.
     */
    for (afi = AFI_IP ; afi < AFI_MAX ; afi++)
        for (safi = SAFI_UNICAST ; safi < SAFI_RESERVED_3 ; safi++)
        {
            bgp_plus_resotre_sleep_route (peer, afi, safi);
        }
    ...
    /* add by ly */
    if(!bgp_peer_reboot_flag && !bgp_self_reboot_flag)
    {
        zlog_debug("bgp_announce_route_all_plus");
        bgp_announce_route_all_plus (peer);
    }
    else
    {
        zlog_debug("bgp_announce_route_all");
        bgp_announce_route_all(peer);
    }
    bgp_peer_reboot_flag = 0;
    bgp_self_reboot_flag = 0;

    neighbor_linklist_setflag(peer->host);

    BGP_TIMER_ON (peer->t_routeadv, bgp_routeadv_timer, 1);
    zlog_debug("bgp_establish end");
    return 0;
}
