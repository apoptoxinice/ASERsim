...
{
    zlog_debug("bgp_advertise_clean");
    struct bgp_advertise *adv;
    struct bgp_advertise_attr *baa;
    ...
    adj->adv = NULL;
    zlog_debug("aadj_dvertise_clean end");
...
void
bgp_adj_out_set_plus (struct bgp_node *rn, struct peer *peer, struct prefix *p,
                      struct attr *attr, afi_t afi, safi_t safi,
                      struct bgp_info *binfo)
{
    zlog_debug("adj_out_set_plus");
    struct bgp_adj_out *adj = NULL;
    struct bgp_advertise *adv;
...
void
bgp_adj_out_set_for_sleep_peer (struct bgp_node *rn, struct peer *peer, struct prefix *p,
                                struct attr *attr, afi_t afi, safi_t safi,
                                struct bgp_info *binfo)
{
    struct bgp_adj_out *adj = NULL;
    ...
    if (! adj)
    {
        ...
        if (rn)
        {
            adj->flags = BGP_PLUS_NEW;
            struct attr *tmp = XCALLOC (MTYPE_ATTR, sizeof (struct attr));
...
bgp_adj_out_unset (struct bgp_node *rn, struct peer *peer, struct prefix *p,
                   afi_t afi, safi_t safi)
{
    zlog_debug("bgp_adj_out_unset");
    puts("bgp_adj_out_unset");
    struct bgp_adj_out *adj;
    struct bgp_advertise *adv;
...
void
bgp_adj_out_unset_plus (struct bgp_node *rn, struct peer *peer, struct prefix *p,
                        afi_t afi, safi_t safi)
{
    struct bgp_adj_out *adj;
    struct bgp_advertise *adv;
...
void
bgp_adj_out_unset_for_sleep_peer (struct bgp_node *rn, struct peer *peer, struct prefix *p,
                                  afi_t afi, safi_t safi)
{
    struct bgp_adj_out *adj;
    struct bgp_advertise *adv;
    ...
    if (adj->flags == BGP_PLUS_ORIGIN)
    {
        adj->flags = BGP_PLUS_WITHDRAW;
    }
    else if (adj->flags == BGP_PLUS_NEW)
    {
        adj->flags = BGP_PLUS_WITHDRAW;
        //BGP_ADJ_OUT_DEL (rn, adj);
        //bgp_adj_out_free(adj);
...
/*
 * ADDED by CPT
 * Remove the route from advertise fifo if exist, but keep in the
 * rib_adj_out table.
 */
void bgp_adv_remove (struct bgp_node *rn, struct bgp_adj_out *adj,
                     struct peer *peer, afi_t afi, safi_t safi)
{
    if (adj->attr)
        bgp_attr_unintern (&adj->attr);
...
{
    puts("bgp_adj_in_unset");
    zlog_debug("bgp_adj_in_unset");
    struct bgp_adj_in *adj;
