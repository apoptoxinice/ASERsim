...
extern int bgp_peer_reboot_flag;
...
bgp_info_delete (struct bgp_node *rn, struct bgp_info *ri)
{
    ...
    printf("%s()%d enter bgp_info_elete:%s.\n", __func__, __LINE__, inet_ntop(rn->p.family, &rn->p.u.prefix, buf, SU_ADDRSTRLEN));
...
//added for 4over6 20130226
    if(new->flag_4over6 > exist->flag_4over6)
        return 1;
    if(new->flag_4over6 < exist->flag_4over6)
        return 0;
    newattr = new->attr;
                    ...
                    struct attr *attr, afi_t afi, safi_t safi)
{
                   ...
                   )))
    {
        /* NEXT-HOP Unchanged. */
             ...
             || safi == SAFI_4OVER6)//added for 4over6 20130314
    {
        /* Set IPv4 nexthop. */
        if (p->family == AF_INET)
        {
            ...
            else if (safi == SAFI_4OVER6)
            {
                memcpy (&attr->extra->mp_nexthop_global, &peer->nexthop.v6_global, IPV6_MAX_BYTELEN);
                               ...
                               struct bgp_node *rn, afi_t afi, safi_t safi)
{
    ...
    /*
     * ADDED by CPT
     * We still update adj_out list for disconnected peer.
     */
    /*if (peer->status != Established) {
      zlog_debug ("Update adj_out for sleep peer %d", peer->as);
      attr.extra = &extra;
      if (selected && bgp_announce_check (selected, peer, p, &attr, afi, safi))
      	bgp_adj_out_set_for_sleep_peer (rn, peer, p, &attr, afi, safi, selected);
      else
    bgp_adj_out_unset_for_sleep_peer (rn, peer, p, afi, safi);
      bgp_attr_flush (&attr);
      return 0;
    }*/
...
bgp_process_main (struct work_queue *wq, void *data)
{
    ...
    if (old_select && old_select == new_select)
    {
        if (! CHECK_FLAG (old_select->flags, BGP_INFO_ATTR_CHANGED))
        {
            ...
            {
                //added for 4over6 20130201
                if(safi == SAFI_4OVER6 && !bgp->name)
                    ret = bgp_zebra_announce_4over6 (p, old_select, bgp);
                else
    ...
    //added for 4over6 20130201
    if (safi == SAFI_4OVER6 && (! bgp->name && ! bgp_option_check(BGP_OPT_NO_FIB)))
    {
        if (new_select && new_select->type == ZEBRA_ROUTE_BGP && new_select->sub_type == BGP_ROUTE_NORMAL)
            ret = bgp_zebra_announce_4over6 (p, new_select, bgp);
        else
        {
            if (old_select && old_select->type == ZEBRA_ROUTE_BGP && old_select->sub_type == BGP_ROUTE_NORMAL)
                bgp_zebra_withdraw_4over6 (p, old_select);
        }
    }
                 ...
                 struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
    ...
    //added for 4over6 20130226
    if (afi == AFI_IP && safi == SAFI_4OVER6)
        new->flag_4over6 = BGP_4OVER6_ENABLE;
...
bgp_default_originate (struct peer *peer, afi_t afi, safi_t safi, int withdraw)
{
    ...
    if(safi == SAFI_4OVER6)
    {
        ...
        /* IPv6 global nexthop must be included. */
        memcpy (&ae->mp_nexthop_global, &peer->nexthop.v6_global, IPV6_MAX_BYTELEN);
        ...
        if (peer->shared_network
                && !IN6_IS_ADDR_UNSPECIFIED (&peer->nexthop.v6_local))
        {
            memcpy (&ae->mp_nexthop_local, &peer->nexthop.v6_local, IPV6_MAX_BYTELEN);
...
static void
bgp_announce_table_plus (struct peer *peer, afi_t afi, safi_t safi,
                         struct bgp_table *table, int rsclient)
{
    zlog_debug("bgp_announce_table_plus");
    struct bgp_node *rn;
    struct bgp_info *ri;
    struct attr attr;
    struct attr_extra extra;
        ...
        for (ri = rn->info; ri; ri = ri->next)
        {
            if (CHECK_FLAG (ri->flags, BGP_INFO_SELECTED) && ri->peer != peer)
            {
                ...
                if(!adj)
                {
                    ...
                    adj->flags = BGP_PLUS_ORIGIN;
                }
                if ( ((rsclient) ?
                        (bgp_announce_check_rsclient (ri, peer, &rn->p, &attr, afi, safi))
                        : (bgp_announce_check (ri, peer, &rn->p, &attr, afi, safi))) && (adj->flags == BGP_PLUS_NEW))
                {
                    bgp_adj_out_set_plus (rn, peer, &rn->p, &attr, afi, safi, ri);
                }
                else if(adj->flags == BGP_PLUS_WITHDRAW)
                {
                    zlog_debug("here in the withdraw");
                    bgp_adj_out_unset_plus (rn, peer, &rn->p, afi, safi);
                    //bgp_info_set_flag(rn, ri, BGP_INFO_REMOVED);
                    //zlog_debug("after unset and set");
                    bgp_rib_remove(rn, ri, peer, afi, safi);
                    rn->info=NULL;
                    bgp_unlock_node(rn);
                    bgp_unlock_node(rn);
                    ...
                    //		bgp_unlock_node(rn);
                    //		bgp_unlock_node(rn);
                    zlog_debug("the rn->lock is %d", rn->lock);
                }
#if 0
                bgp_attr_extra_free (&attr);
#endif
...
void
bgp_announce_route_plus (struct peer *peer, afi_t afi, safi_t safi)
{
    struct bgp_node *rn;
    struct bgp_table *table;
    ...
    if ((safi != SAFI_MPLS_VPN))
        bgp_announce_table_plus (peer, afi, safi, NULL, 0);
    else
        for (rn = bgp_table_top (peer->bgp->rib[afi][safi]); rn;
                rn = bgp_route_next(rn))
            if ((table = (rn->info)) != NULL)
                bgp_announce_table_plus (peer, afi, safi, table, 0);

    if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT))
        bgp_announce_table_plus (peer, afi, safi, NULL, 1);
}
void
...
void
bgp_announce_route_all_plus (struct peer *peer)
{
    afi_t afi;
    safi_t safi;
    ...
    for (afi = AFI_IP; afi < AFI_MAX; afi++)
        for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
            bgp_announce_route_plus (peer, afi, safi);
}
static void
...
bgp_clear_route_node (struct work_queue *wq, void *data)
{
    ...
    for (ri = rn->info; ri; ri = ri->next)
        if (ri->peer == peer || cnq->purpose == BGP_CLEAR_ROUTE_MY_RSCLIENT)
        {
            ...
            /*
             * ADDED by CPT
             * Comment & all set flag like graceful restart.
             */
            /*
            if (CHECK_FLAG (peer->sflags, PEER_STATUS_NSF_WAIT)
              ...
              bgp_rib_remove (rn, ri, peer, afi, safi);
            */
            zlog_debug("before: the flag is %d", ri->flags);
            bgp_info_set_flag (rn, ri, BGP_INFO_PLUS_SLEEP);
            bgp_info_unset_flag(rn, ri, BGP_INFO_VALID);
            //bgp_info_unset_flag(rn, ri, BGP_INFO_SELECTED);
            //bgp_info_set_flag (rn, ri, BGP_INFO_STALE);
            bgp_info_set_flag (rn, ri, BGP_INFO_HISTORY);
            //bgp_info_set_flag (rn, ri, BGP_INFO_DAMPED);
            zlog_debug("after: the flag is %d", ri->flags);
            bgp_rib_remove (rn, ri, peer, afi, safi);
            break;
        }
    return WQ_SUCCESS;
...
//            bgp_adj_out_remove (rn, aout, peer, afi, safi);
                bgp_adv_remove (rn, aout, peer, afi, safi);
//            bgp_unlock_node (rn);
                break;
            }
 ...
 */
void
bgp_plus_resotre_sleep_route (struct peer *peer, afi_t afi, safi_t safi)
{
    struct bgp_node *rn;
    struct bgp_info *ri;
    struct bgp_table *table;
    ...
    for (rn = bgp_table_top (table); rn; rn = bgp_route_next (rn))
    {
        for (ri = rn->info; ri; ri = ri->next)
        {
            if (ri->peer == peer)
            {
                if (CHECK_FLAG (ri->flags, BGP_INFO_PLUS_SLEEP))
                {
                    UNSET_FLAG (ri->flags, BGP_INFO_PLUS_SLEEP);
                    SET_FLAG (ri->flags, BGP_INFO_VALID);
                    UNSET_FLAG (ri->flags, BGP_INFO_HISTORY);
                    bgp_aggregate_increment(peer->bgp, &rn->p, ri, afi, safi);
                    bgp_process(peer->bgp, rn, afi, safi);
...
//added for 4over6 20130129
int bgp_table_ipv4_to_4over6(struct peer *peer, struct prefix *p)
{
    ...
    bgp = peer->bgp;
    rn = bgp_afi_node_get (bgp->rib[AFI_IP][SAFI_4OVER6], AFI_IP, SAFI_4OVER6, p, NULL);

    /* Check previously received route. */
    for (ri = rn->info; ri; ri = ri->next)
        if (ri->peer == peer && ri->type == ZEBRA_ROUTE_4OVER6 && ri->sub_type == BGP_ROUTE_NORMAL)
            break;

    if(p->family == AF_INET)
        inet_ntop(p->family, &(p->u.prefix), tmp_strptr, 128);
    else
        inet_ntop(p->family, &(p->u.prefix6), tmp_strptr, 128);
    ...
    /* Increment prefix */
    bgp_aggregate_increment (bgp, p, new, AFI_IP, SAFI_4OVER6);
    ...
    /* If maximum prefix count is configured and current prefix
    count exeed it. */
    if (bgp_maximum_prefix_overflow (peer, AFI_IP, SAFI_4OVER6, 0))
        return -1;

    /* Process change. */
    bgp_process (bgp, rn, AFI_IP, SAFI_4OVER6);

    /* Unintern original. */
    aspath_4over6_unintern (attr.aspath);

    return 0;
}

static int bgp_add_ipv4_to_4over6(struct peer *peer, struct prefix *p, struct in_addr *nexthop,
                                  uint32_t metric, afi_t afi, safi_t safi, int type, int sub_type,
                                  struct prefix_rd *prd)
{
    struct bgp_node *rn;
    struct bgp *bgp;
    ...
    bgp = peer->bgp;
    rn = bgp_afi_node_get (bgp->rib[afi][SAFI_4OVER6], afi, SAFI_4OVER6, p, prd);
    ...
    /* Increment prefix */
    bgp_aggregate_increment (bgp, p, new, afi, SAFI_4OVER6);
    ...
    /* If maximum prefix count is configured and current prefix
    count exeed it. */
    if (bgp_maximum_prefix_overflow (peer, afi, SAFI_4OVER6, 0))
        return -1;

    /* Process change. */
    bgp_process (bgp, rn, afi, SAFI_4OVER6);
...
static int bgp_del_ipv4_from_4over6(struct peer *peer, struct prefix *p, u_char type, int sub_type)
{
    struct bgp_node *rn;
    struct bgp *bgp;
    struct bgp_info *ri;
    ...
    /* Withdraw specified route from routing table. */
    if (ri && ! CHECK_FLAG (ri->flags, BGP_INFO_HISTORY))
        bgp_rib_withdraw (rn, ri, peer, AFI_IP, SAFI_4OVER6);
    else
        printf("bgp_del_ipv4_from_4over6 running wrong !\n");
...
static int bgp_add_4over6_to_ipv4(struct peer *peer, struct prefix *p, struct in_addr *nexthop,
                                  uint32_t metric, afi_t afi, safi_t safi, int type, int sub_type,
                                  struct prefix_rd *prd)
{
    struct bgp *bgp;
    ...
    bgp = peer->bgp;
    rn = bgp_afi_node_get (bgp->rib[afi][SAFI_UNICAST], afi, SAFI_UNICAST, p, prd);
    ...
    memset(&tmp_strptr, 0x00, sizeof(tmp_strptr));
    if(p->family == AF_INET)
        inet_ntop(p->family, &(p->u.prefix), tmp_strptr, 128);
    else
        inet_ntop(p->family, &(p->u.prefix6), tmp_strptr, 128);
    ...
    /* If maximum prefix count is configured and current prefix
    count exeed it. */
    if (bgp_maximum_prefix_overflow (peer, afi, SAFI_UNICAST, 0))
        return -1;
...
static int bgp_del_4over6_from_ipv4(struct peer *peer, struct prefix *p, u_char type, int sub_type)
{
    struct bgp_node *rn;
    struct bgp *bgp;
    struct bgp_info *ri;
    ...
    bgp = peer->bgp;
    rn = bgp_afi_node_get (bgp->rib[AFI_IP][SAFI_UNICAST], AFI_IP, SAFI_UNICAST, p, NULL);
    ...
    /* Withdraw specified route from routing table. */
    if (ri && ! CHECK_FLAG (ri->flags, BGP_INFO_HISTORY))
        bgp_rib_withdraw (rn, ri, peer, afi, SAFI_UNICAST);
    else
        printf("bgp_del_4over6_from_ipv4 running wrong !\n");
    ...
    for (; pnt < lim; pnt += psize)
    {
        ...
        if (attr)
        {
            ...
            if(ret == 0)
            {
                ...
                bgp = bgp_get_default ();
                if(CHECK_FLAG (bgp->table_config, BGP_TABLE_CONFIG_4OVER6))
                    bgp_add_ipv4_to_4over6(peer, &p, &(attr->nexthop), attr->med,
                                           packet->afi, packet->safi, ZEBRA_ROUTE_4OVER6, BGP_ROUTE_NORMAL,NULL);
            }
        }
        else
        {
            ...
            bgp_del_ipv4_from_4over6(peer, &p, ZEBRA_ROUTE_4OVER6, BGP_ROUTE_NORMAL);
        }
...
//added for 4over6 20130122
int bgp_nlri_parse_4over6 (struct peer *peer, struct attr *attr, struct bgp_nlri *packet)
{
    u_char *pnt;
    u_char *lim;
    struct prefix p;
    int psize;
    int ret;
    struct attr attrTemp;
    struct in_addr stNextHop;

    //oam_OutputToDevice(0, "bgp_nlri_parse_4over6 : get in!\n");
    /* Check peer status. */
    if (peer->status != Established)
        return 0;
    ...
    for (; pnt < lim; pnt += psize)
    {
        ...
        if ((packet->afi == AFI_IP && p.prefixlen > 32) || (packet->afi == AFI_IP6 && p.prefixlen > 128))
            return -1;
        ...
        /* Check address. */
        if (packet->afi == AFI_IP && packet->safi == BGP_SAFI_4OVER6)
        {
            if (IN_CLASSD (ntohl (p.u.prefix4.s_addr)))
            {
                zlog (peer->log, LOG_ERR, "4over6 unicast NLRI is multicast address %s", inet_ntoa (p.u.prefix4));
                return -1;
            }
        }
        ...
        if (packet->afi == AFI_IP6 && packet->safi == SAFI_UNICAST)
        {
            if (IN6_IS_ADDR_LINKLOCAL (&p.u.prefix6))
            {
                ...
                zlog (peer->log, LOG_WARNING, "IPv6 link-local NLRI received %s ignore this NLRI",
                      inet_ntop (AF_INET6, &p.u.prefix6, buf, BUFSIZ));
        ...
        if (attr)
        {
            //zlog_info("call bgp_update");
            ret = bgp_update (peer, &p, attr, packet->afi, SAFI_4OVER6, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, NULL, NULL, 0);

            if(ret == 0)
            {
                /* Make default attribute. */
                bgp_attr_default_set (&attrTemp, BGP_ORIGIN_INCOMPLETE);
                stNextHop = peer->local_id;
                bgp_add_4over6_to_ipv4(peer, &p, &stNextHop, attrTemp.med,
                                       packet->afi, packet->safi, ZEBRA_ROUTE_4OVER6, BGP_ROUTE_NORMAL,NULL);
            }
        }
        else
        {
            ret = bgp_withdraw (peer, &p, attr, packet->afi, SAFI_4OVER6, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, NULL, NULL);
            bgp_del_4over6_from_ipv4(peer, &p, ZEBRA_ROUTE_4OVER6, BGP_ROUTE_NORMAL);
        }
        /* Address family configuration mismatch or maximum-prefix count
        overflow. */
        if (ret < 0)
                ...
                afi_t afi, safi_t safi, const char *rmap, int backdoor)
{
    ...
    memset(&extra, 0, sizeof(extra));
    attr.extra = &extra;
    struct bgp_node *crn = bgp_afi_node_get(bgp->rib[afi][safi], afi, safi, &p, NULL);
    for(ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer))
    {
        if(peer->status != Established)
            bgp_adj_out_set_for_sleep_peer(crn, peer, &p, &attr, afi, safi, NULL);
    }
    bgp_attr_flush(&attr);
    return CMD_SUCCESS;
}
                  ...
                  afi_t afi, safi_t safi)
{
    ...
    if (0)
    {
        ...
        for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
        {
            ...
            {
                //todo = 1;
                bgp_adj_out_unset_for_sleep_peer (crn, peer, &p, afi, safi);
                //bgp_info_unset_flag(crn, crn->info, BGP_INFO_VALID);
                UNSET_FLAG(ri->flags, BGP_INFO_VALID);
                //bgp_info_set_flag(crn, crn->info, BGP_INFO_HISTORY);
                //bgp_rib_remove(crn, crn->info, peer, afi, safi);
                //zlog_debug("after rib_remove");
                continue;
            }
            if(!bgp_static->backdoor)
                bgp_static_withdraw(bgp, &p, afi, safi);
    ...
    if (! bgp_static->backdoor)
    {
        printf("%s()%d will call bgp_static_withdraw.\n", __func__, __LINE__);
        bgp_static_withdraw (bgp, &p, afi, safi);
    }
...
//added for 4over6 20130301
void bgp_redistribute_4over6_add (struct prefix *p, const struct in_addr *nexthop,
                                  const struct in6_addr *nexthop6, u_int32_t metric, u_char type)
{
    struct bgp *bgp;
    struct listnode *node, *nnode;
    struct bgp_info *new;
    ...
    struct attr_extra *extra = bgp_attr_extra_get(&attr);
    memset(&extra->mp_nexthop_global,0x0,sizeof(struct in6_addr));
    extra->mp_nexthop_len = 16;
#endif
    ...
    for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
        ...
        if (bgp->redist[afi][type])
        {
            ...
            if (bgp->rmap[afi][type].map)
            {
                ...
                ret = route_map_apply (bgp->rmap[afi][type].map, p, RMAP_BGP, &info);
            ...
            bn = bgp_afi_node_get (bgp->rib[afi][SAFI_4OVER6], afi, SAFI_4OVER6, p, NULL);
            ...
            for (bi = bn->info; bi; bi = bi->next)
                if (bi->peer == bgp->peer_self && bi->sub_type == BGP_ROUTE_REDISTRIBUTE)
                    break;

            if (bi)
            {
                if (attrhash_cmp (bi->attr, new_attr) && !CHECK_FLAG(bi->flags, BGP_INFO_REMOVED))
                {
                    bgp_attr_unintern (&new_attr);
                    aspath_unintern (&attr.aspath);
                    bgp_attr_extra_free (&attr);
            ...
            bgp_info_add (bn, new);
            bgp_unlock_node (bn);
            bgp_process (bgp, bn, afi, SAFI_4OVER6);
        }
    }
...
//added for 4over6 20130301
void bgp_redistribute_4over6_delete (struct prefix *p, u_char type)
{
    struct bgp *bgp;
    struct listnode *node, *nnode;
    afi_t afi;
    ...
    for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    {
        ...
        if (bgp->redist[afi][type])
        {
            rn = bgp_afi_node_get (bgp->rib[afi][SAFI_4OVER6], afi, SAFI_4OVER6, p, NULL);
            ...
            if (ri)
            {
                bgp_aggregate_decrement (bgp, p, ri, afi, SAFI_4OVER6);
                bgp_info_delete (rn, ri);
                bgp_process (bgp, rn, afi, SAFI_4OVER6);
            }
            bgp_unlock_node (rn);
...
//added for 4over6 20130301
void bgp_redistribute_4over6_withdraw (struct bgp *bgp, afi_t afi, int type)
{
    struct bgp_node *rn;
    struct bgp_info *ri;
    struct bgp_table *table;

    table = bgp->rib[afi][SAFI_4OVER6];
    ...
    for (rn = bgp_table_top (table); rn; rn = bgp_route_next (rn))
    {
        ...
        if (ri)
        {
            bgp_aggregate_decrement (bgp, &rn->p, ri, afi, SAFI_4OVER6);
            bgp_info_delete (rn, ri);
            bgp_process (bgp, rn, afi, SAFI_4OVER6);
        }
    }
...
route_vty_short_status_out (struct vty *vty, struct bgp_info *binfo)
{
    ...
    if (CHECK_FLAG (binfo->flags, BGP_INFO_REMOVED))
        vty_out (vty, "R");
    else if (CHECK_FLAG (binfo->flags, BGP_INFO_PLUS_SLEEP))
        vty_out (vty, "+");
               ...
               struct bgp_info *binfo, int display, safi_t safi)
{
    ...
    if (attr)
    {
        if (p->family == AF_INET)
        {
            ...
            //added for 4over6 20130203
            else if (safi == SAFI_4OVER6&&attr->extra)
            {
                char buf[BUFSIZ];
                memset(buf, 0, sizeof(buf));
                inet_ntop (AF_INET6, &attr->extra->mp_nexthop_global, buf, BUFSIZ);
                vty_out (vty, "%-39s", buf);
            }
            else
...
/*
 * ADDED by CPT
 * output the status of rib_adj_out list for bgp+;
 */
void
route_vty_out_for_bgp_plus (struct vty *vty, struct prefix *p,
                            struct attr *attr, safi_t safi, u_int16_t flags)
{
    if (CHECK_FLAG (flags, BGP_PLUS_NEW))
        vty_out (vty, "N");
    else if (CHECK_FLAG (flags, BGP_PLUS_WITHDRAW))
        vty_out (vty, "W");
    else
        vty_out (vty, "O");
    vty_out (vty, " ");
...
#define BGP_SHOW_SCODE_HEADER "Status codes: s suppressed, d damped, "\
    ...
    "              + BGP+ Sleep, N BGP+ New, O BGP+ Origin, W BGP+ Withdraw%s"
#define BGP_SHOW_OCODE_HEADER "Origin codes: i - IGP, e - EGP, ? - incomplete%s%s"
#define BGP_SHOW_HEADER "   Network          Next Hop            Metric LocPrf Weight Path%s"
//added for 4over6 20130203
#define BGP_SHOW_4OVER6_HEADER 	"   Network          Next Hop                                Metric LocPrf Weight Path%s"
        ...
        if (rn->info != NULL)
        {
            ...
            for (ri = rn->info; ri; ri = ri->next)
            {
                ...
                if (header)
                {
                    ...
                    //added for 4over6 20130203
                    else if(table->safi == SAFI_4OVER6)
                        vty_out (vty, BGP_SHOW_4OVER6_HEADER, VTY_NEWLINE);
                    else
                ...
                //added for 4over6 20130203
                    ...
                    route_vty_out (vty, &rn->p, ri, display, SAFI_4OVER6);
                else
...
//added for 4over6 20130202
DEFUN (show_4over6_bgp,
       show_4over6_bgp_cmd,
       "show 4over6 bgp",
       SHOW_STR
       IP_STR
       BGP_STR)
{
    return bgp_show (vty, NULL, AFI_IP, SAFI_4OVER6, bgp_show_type_normal, NULL);
}
        ...
        else
        {
            for (adj = rn->adj_out; adj; adj = adj->next)
                if (adj->peer == peer)
                {
                    ...
                    if (adj->attr)
                    {
                        route_vty_out_for_bgp_plus (vty, &rn->p, adj->attr, safi, adj->flags);
                        output_count++;
                    }
                }
        }
...
bgp_route_init (void)
{
    ...
    /* IPv4 4over6 configuration.  */
    install_element (BGP_IP4OVER6_NODE, &show_4over6_bgp_cmd);
    install_element (BGP_IP4OVER6_NODE, &bgp_network_cmd);
    install_element (BGP_IP4OVER6_NODE, &no_bgp_network_cmd);
    ...
    //added for 4over6 20130202
    install_element (VIEW_NODE, &show_4over6_bgp_cmd);
    install_element (ENABLE_NODE, &show_4over6_bgp_cmd);
    install_element (BGP_NODE, &show_4over6_bgp_cmd);
