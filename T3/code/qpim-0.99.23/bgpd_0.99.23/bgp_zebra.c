...
bgp_interface_add (int command, struct zclient *zclient, zebra_size_t length)
{
    ...
    //added by wangyl for ifp null
    if(ifp == NULL)
    {

        zlog_debug("Zebra rcvd: interface add NULL");
        return 0;
    }
...
zebra_read_ipv4 (int command, struct zclient *zclient, zebra_size_t length)
{
    ...
    if (command == ZEBRA_IPV4_ROUTE_ADD)
    {
        ...
        bgp = bgp_get_default();
        if(bgp_flag_check(bgp, BGP_FLAG_4OVER6_STARTED_AT_VIF))
            bgp_redistribute_4over6_add((struct prefix *)&p, &nexthop, NULL, api.metric, api.type);
    }
    else
    {
        if (BGP_DEBUG(zebra, ZEBRA))
        ...
        bgp = bgp_get_default();
        if(bgp_flag_check(bgp, BGP_FLAG_4OVER6_STARTED_AT_VIF))
            bgp_redistribute_4over6_delete((struct prefix *)&p, api.type);
    }
...
int bgp_zebra_announce_4over6(struct prefix *p, struct bgp_info *info, struct bgp *bgp)
{
    int flags;
    u_char distance;
    struct peer *peer;
    ...
    if ((peer_sort (peer) == BGP_PEER_EBGP && peer->ttl != 1)|| CHECK_FLAG (peer->flags, PEER_FLAG_DISABLE_CONNECTED_CHECK))
        SET_FLAG (flags, ZEBRA_FLAG_INTERNAL);

    /* resize nexthop buffer size if necessary */
    if ((oldsize = stream_get_size (bgp_nexthop_buf)) < (sizeof (struct in6_addr *) * (bgp_info_mpath_count (info) + 1)))
    {
        newsize = (sizeof (struct in6_addr *) * (bgp_info_mpath_count (info) + 1));
    ...
    if (p->family == AF_INET)
    {
        ...
        api.flags = flags;
        in6_nexthop = &info->attr->extra->mp_nexthop_global;
        stream_put (bgp_nexthop_buf, &in6_nexthop, sizeof (struct in6_addr *));
        for (mpinfo = bgp_info_mpath_first (info); mpinfo; mpinfo = bgp_info_mpath_next (mpinfo))
        {
            in6_nexthop = &info->attr->extra->mp_nexthop_global;
            stream_put (bgp_nexthop_buf, &in6_nexthop, sizeof (struct in_addr *));
        }

        api.type = ZEBRA_ROUTE_BGP;
        api.message = 0;
        api.safi = SAFI_4OVER6;
        ...
        if (BGP_DEBUG(zebra, ZEBRA))
        {
            ...
            char buf_v6[1][INET6_ADDRSTRLEN];

            zlog_debug("Zebra send: 4over6 route add %s/%d nexthop %s metric %u count %d",
                       inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])), p->prefixlen,
                       inet_ntop(AF_INET6, in6_nexthop, buf_v6[1], sizeof(buf_v6[1])), api.metric, api.nexthop_num);
        }
        zapi_4over6_route(ZEBRA_IPV4_4OVER6_ROUTE_ADD, zclient, (struct prefix_ipv4 *) p, &api);
    }
...
//added for 4over6 20130201
void bgp_zebra_withdraw_4over6(struct prefix *p, struct bgp_info *info)
{
    int flags;
    struct peer *peer;
    struct zapi_ipv6 api;
    ...
    if (BGP_DEBUG(zebra, ZEBRA))
    {
        ...
        zlog_debug("Zebra send: IPv4 route delete %s/%d nexthop %s metric %u",
                   inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])), p->prefixlen,
                   inet_ntop(AF_INET6, in6_nexthop, buf_v6[0], sizeof(buf_v6[0])), api.metric);
    }
    zapi_4over6_route(ZEBRA_IPV4_4OVER6_ROUTE_DELETE, zclient, (struct prefix_ipv4 *) p, &api);
    return;
}
...
int bgp_redistribute_rmap_set (struct bgp *bgp, afi_t afi, int type,
                               const char *name)
{
...
int bgp_redistribute_metric_set (struct bgp *bgp, afi_t afi, int type,
                                 u_int32_t metric)
{
...
int bgp_redistribute_unset (struct bgp *bgp, afi_t afi, int type)
{
    ...
    //added for 4over6 20130301
    if(bgp_flag_check(bgp, BGP_FLAG_4OVER6_STARTED_AT_VIF))
        bgp_redistribute_4over6_withdraw (bgp, afi, type);

    return CMD_SUCCESS;
}

/* Unset redistribution route-map configuration.  */
int bgp_redistribute_routemap_unset (struct bgp *bgp, afi_t afi, int type)
{
    if (! bgp->rmap[afi][type].name)
        return 0;
...
int bgp_redistribute_metric_unset (struct bgp *bgp, afi_t afi, int type)
{
    if (! bgp->redist_metric_flag[afi][type])
        return 0;
