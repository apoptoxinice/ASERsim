...
#if 1
size_t
bgp_packet_mpattr_start (struct stream *s, struct peer *peer, struct in6_addr *nexthop,
                         struct peer *from, afi_t afi, safi_t safi, struct attr *attr)
{
    size_t sizep;
    ...
    case AFI_IP:
        ...
        case SAFI_4OVER6:
        {
            ...
            if ((peer->sort == BGP_PEER_EBGP && from && from->sort  == BGP_PEER_IBGP && nexthop)
                    ||(peer->sort == BGP_PEER_IBGP && from && from->sort == BGP_PEER_EBGP && nexthop)
                    ||(peer->sort == BGP_PEER_EBGP && from && from->sort == BGP_PEER_EBGP&&nexthop))
            {
                if (attre->mp_nexthop_len == 16)
                {
#if 1
                    if((nexthop->s6_addr32[0] == 0) &&
                            (nexthop->s6_addr32[1] == 0) &&
                            (nexthop->s6_addr32[2] == 0) &&
                            (nexthop->s6_addr32[3] == 0))
                        if((attre->mp_nexthop_global.s6_addr32[0]==0) &&
                                (attre->mp_nexthop_global.s6_addr32[1]==0) &&
                                (attre->mp_nexthop_global.s6_addr32[2]==0) &&
                                (attre->mp_nexthop_global.s6_addr32[3]==0))
                            stream_put (s, &peer->nexthop.v6_global, 16);
                        else
                            stream_put (s, &attre->mp_nexthop_global, 16);
                    else
#endif
                        stream_put (s, nexthop, 16);
                }
...
bgp_packet_attribute (struct bgp *bgp, struct peer *peer,
                      ...
                      struct prefix *p, struct in6_addr *nexthop,  afi_t afi, safi_t safi,
...
#endif
        mpattrlen_pos = bgp_packet_mpattr_start(s, peer, nexthop, from, afi, safi, attr);
