    ...
    while (adv)
    {
        ...
        if (stream_empty (s))
        {
            ...
            if(ri)
                pstAttr = ri->attr;
            if(pstAttr)
                nexthop = &(pstAttr->extra->mp_nexthop_global);
...
#endif
#if 1
            mpattrlen_pos = bgp_packet_mpattr_start(snlri,peer, nexthop, from ,afi, safi,
                                                    adv->baa->attr);
#endif
                         ...
                         afi_t afi, safi_t safi, struct peer *from)
{
    ...
    if (BGP_DEBUG (update, UPDATE_OUT))
    {
        char attrstr[BUFSIZ];
        char buf[INET6_BUFSIZ];
        memset(attrstr, 0x00, sizeof(attrstr));
        memset(buf, 0x00, sizeof(buf));
                        ...
                        u_char orf_type, u_char when_to_refresh, int remove)
{
    ...
    //added for 4over6 20130225
    if (safi == SAFI_4OVER6)
        safi = BGP_SAFI_4OVER6;
...
bgp_open_receive (struct peer *peer, bgp_size_t size)
{
            ...
            CHECK_FLAG (peer->flags, PEER_FLAG_OVERRIDE_CAPABILITY))
    {
        ...
        peer->afc_nego[AFI_IP][SAFI_4OVER6] = peer->afc[AFI_IP][SAFI_4OVER6];//added for 4over6 20130131
...
#define NLRI_ATTR_ARG (attr_parse_ret != BGP_ATTR_PARSE_WITHDRAW ? &attr : NULL)
    ...
    //added for 4over6 20130225
    if (peer->afc[AFI_IP][SAFI_4OVER6] && peer->afc_nego[AFI_IP][SAFI_4OVER6])
    {

        if (mp_update.length && mp_update.afi == AFI_IP && mp_update.safi == BGP_SAFI_4OVER6)
        {
            struct bgp *bgp;

            bgp = bgp_get_default ();
            if(CHECK_FLAG (bgp->table_config, BGP_TABLE_CONFIG_4OVER6))
                bgp_nlri_parse_4over6 (peer, NLRI_ATTR_ARG, &mp_update);
        }

        if (mp_withdraw.length && mp_withdraw.afi == AFI_IP && mp_withdraw.safi == BGP_SAFI_4OVER6)
            bgp_nlri_parse_4over6 (peer, NULL, &mp_withdraw);

        if (! attribute_len && ! withdraw_len)
        {
            /* End-of-RIB received */
            SET_FLAG (peer->af_sflags[AFI_IP][SAFI_4OVER6], PEER_STATUS_EOR_RECEIVED);

            /* NSF delete stale route */
            if (peer->nsf[AFI_IP][SAFI_4OVER6])
                bgp_clear_stale_route (peer, AFI_IP, SAFI_4OVER6);

            if (BGP_DEBUG (normal, NORMAL))
                zlog (peer->log, LOG_DEBUG, "rcvd End-of-RIB for IPv4 Unicast from %s", peer->host);
        }
    }
...
bgp_route_refresh_receive (struct peer *peer, bgp_size_t size)
{
    ...
    if ((afi != AFI_IP && afi != AFI_IP6)
            || (safi != SAFI_UNICAST && safi != SAFI_MULTICAST && safi != SAFI_4OVER6 //added for 4over6 20130131
                && safi != SAFI_MPLS_LABELED_VPN))
    {
        if (BGP_DEBUG (normal, NORMAL))
        {
    ...
    if (safi != SAFI_UNICAST
            ...
            && safi != BGP_SAFI_4OVER6)
    {
        zlog_info ("%s [Error] Unknown SAFI %d route refresh",peer->host, safi);
