...
// add 2020
#if 1
    oi->interface_pbi_flag = OSPF6_PBIDISABLED;
    oi->neighbor_area_id = 0;
    memset(&oi->neighbor_prefix, 0x00, sizeof(struct prefix));
    memset(&oi->neighbor_linklocal_addr, 0x00, sizeof(struct in6_addr));
#endif
    ...
    //add here for OSPF+ 16.5.10
    oi->new_lsas_list = ospf6_lsdb_create (oi);
    oi->new_lsas_list->hook_add = NULL;
    oi->new_lsas_list->hook_remove = NULL;
...
#if 1
    ospf6_lsdb_remove_all (oi->new_lsas_list);
    ospf6_lsdb_delete (oi->new_lsas_list);
    oi->reference_count = 0;
#endif
...
ospf6_interface_connected_route_update (struct interface *ifp)
{
    ...
    for (ALL_LIST_ELEMENTS (oi->interface->connected, node, nnode, c))
    {
        ...
        if ((oi->type == OSPF_IFTYPE_POINTOPOINT)
                ...
                && (!IN6_IS_ADDR_UNSPECIFIED(&oi->neighbor_linklocal_addr)))
        {
...
dr_election (struct ospf6_interface *oi)
{
            ...
            oi->bdrouter != (bdrouter ? bdrouter->router_id : htonl (0)))
    {
        ...
        {
            /*for leaving*/
            if ((on->state < OSPF6_NEIGHBOR_TWOWAY) || (on->state == OSPF6_NEIGHBOR_LEAVING))
                continue;
            /* Schedule AdjOK. */
