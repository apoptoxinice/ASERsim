...
ospf6_route_table_name (struct ospf6_route_table *table)
{
    ...
    case OSPF6_SCOPE_TYPE_GLOBAL:
    {
        ...
        case OSPF6_TABLE_TYPE_SPF_RESULTS:
            snprintf (name, sizeof (name), "global spf table");
            break;
#endif
        default:
...
#if 1//add 2020
void ospf6_zone_prefix2str (struct prefix *prefix, char *buf, int size)
{
    u_int32_t adv_router, id;
    char adv_router_str[16], id_str[16];
    memcpy (&adv_router, &prefix->u.prefix6.s6_addr[0], 4);
    ...
    if (ntohl (adv_router))
        snprintf (buf, size, "%s Area-ID: %s", adv_router_str, id_str);
    else
        snprintf (buf, size, "%s", id_str);
}
#endif
...
{ "Unknown", "Intra-Area", "Inter-Area", "External-1", "External-2", "Partition-route", };
...
ospf6_route_show_detail (struct vty *vty, struct ospf6_route *route)
{
    ...
    if (route->type == OSPF6_DEST_TYPE_LINKSTATE)
    {
        ospf6_linkstate_prefix2str (&route->prefix, destination,
                                    sizeof (destination));
#ifdef TWOD
        ospf6_linkstate_prefix2str (&route->s_prefix, sourceaddr,
                                    sizeof (sourceaddr));                           // add haozhiqiang 2016-2-2
#endif
    }
    else if (route->type == OSPF6_DEST_TYPE_ROUTER)
    {
        inet_ntop (route->prefix.family, &route->prefix.u.prefix,
                   destination, sizeof (destination));
#ifdef TWOD
        inet_ntop (route->s_prefix.family, &route->s_prefix.u.prefix6,
                   sourceaddr, sizeof (sourceaddr));                                    // add haozhiqiang 2016-2-2
#endif
    }
    else
        prefix2str (&route->prefix, destination, sizeof (destination));
#ifdef TWOD
    prefix2str (&route->s_prefix, sourceaddr, sizeof (sourceaddr));       // add haozhiqiang 2016-2-2
#endif
    vty_out (vty, "Destination: %s%s", destination, VNL);
#ifdef TWOD
    vty_out (vty, "Sourceaddr: %s%s", sourceaddr, VNL);                       // add haozhiqiang 2016-2-2
#endif
                        ...
                        struct ospf6_route_table *table)
{
    ...
    vty_out(vty,"\n\r\n\r%-15s %-15s %-15s %-5s %-15s\n\r","dst-prefix","src-prefix","out-interface","flag","next-hop");
    for(int col=0; col< twod_colcu; col++)
    {
        vty_out(vty,"%-15s %-15s %-15s %-5s %-15s\n\r",vty_p[col].dst_addr_pre,
                vty_p[col].sec_addr_pre,
                vty_p[col].ifiname,
                vty_p[col].flagval,
                vty_p[col].next_hop);
    }
