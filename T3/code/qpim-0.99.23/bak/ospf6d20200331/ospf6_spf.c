...
static void ospf6_spf_calculation_zone (u_int32_t router_id, struct ospf6_route_table *result_table, struct ospf6 *o);
static int
...
ospf6_vertex_create (struct ospf6_lsa *lsa)
{
...
#if 1//add 2020
    else if (ntohs (lsa->header->type) == OSPF6_LSTYPE_ZONE)
        v->type = OSPF6_VERTEX_TYPE_ZONE;
#endif
    else
        assert (0);
    ...
    /* name */
    if (v->type == OSPF6_VERTEX_TYPE_ZONE)
        ospf6_zone_prefix2str (&v->vertex_id, v->name, sizeof (v->name));
    else
...
ospf6_lsdesc_lsa_e(caddr_t lsdesc, struct ospf6_vertex *v,struct ospf6_lsdb *lsdb)
{
    struct ospf6_lsa *lsa;
    u_int16_t type = 0;
    u_int32_t id = 0, adv_router = 0;
    ...
    lsa = ospf6_lsdb_lookup(type, id, adv_router, lsdb);   //change haozhiqiang
    ...
    else if (route && route->path.cost == v->cost)
    {
                ...
                i < OSPF6_MULTI_PATH_LIMIT; i++)
        {
            for (j = 0; j < OSPF6_MULTI_PATH_LIMIT; j++)
            {
                ...
                {
                    if (ospf6_nexthop_is_same (&route->nexthop[j],  &v->nexthop[i]))
                        break;
                    else
                        continue;
                }
...
#if 0//add 2020
    if (VERTEX_IS_TYPE (ZONE, v))
    {
        route->type = OSPF6_DEST_TYPE_AREA;
        route->path.type = OSPF6_PATH_TYPE_PARTITION;
    }
    else
#endif
    {
...
//得到最短路径树，存放在oa->spf_table1  haozhiqiang
void
ospf6_spf_calculation_e(u_int32_t router_id,
                        struct ospf6_route_table *result_table,
                        struct ospf6_area *oa,
                        struct ospf6_lsdb *lsdb)
{
    ...
    /* Install the calculating router itself as the root of the SPF tree */
    /* construct root vertex 构造根节点 */
    lsa = ospf6_lsdb_lookup(htons(OSPF6_LSTYPE_ROUTER), htonl(0),
                            router_id, oa->lsdb1);     //change haohziqiang
    if(lsa == NULL)
        return;
    ...
    /* Actually insert root to the candidate-list as the only candidate */
    pqueue_enqueue (root, candidate_list);  //插入root到链表里
    ...
    while (candidate_list->size)
    {
                ...
                lsdesc + size <= OSPF6_LSA_END (v->lsa->header); lsdesc += size)
        {
            ...
            lsa = ospf6_lsdesc_lsa_e(lsdesc, v, oa->lsdb1);   // change haozhiqiang
            if (lsa == NULL)
                continue;
...
#if 1 //add 2020
    //ospf6_spf_schedule_zone (ospf6, OSPF6_SPF_FLAGS_ZONE_LSA_ADDED);
    ospf6_spf_calculation_zone (ospf6->router_id, ospf6->spf_table_zone, ospf6);
    ospf6_zone_route_calculation (ospf6);
#endif
    return 0;
}
...
//add by hzq
static int
ospf6_spf_calculation_thread_e(struct thread *t)
{
    struct ospf6_area *oa;
    struct timeval start, end, runtime;
    struct ospf6_route_table *route_table1;
    char buf[32];
    ...
    //得到最短路径树，存放在oa->spf_table1  haozhiqiang
    ospf6_spf_calculation_e(oa->ospf6->router_id, oa->spf_table1, oa,oa->lsdb1);
    ...
    if(IS_OSPF6_DEBUG_SPF(PROCESS) || IS_OSPF6_DEBUG_SPF(TIME))
    {
        zlog_debug("SPF runtime: %ld sec %ld usec",runtime.tv_sec, runtime.tv_usec);
    }

    // add by haozhiqiang  存放二维路由表
    route_table1 = Create_Router_table1(oa);

    //printf("the program excute this (2)\n");

    //得到新的二维路由，存放在route_table1。
    ospf6_intra_route_calculation_e(oa,route_table1);
...
#if 1 //add 2020
static struct ospf6_lsa *ospf6_aaidesc_lsa (caddr_t aaidesc, struct ospf6_vertex *v)
{
    struct ospf6_lsa *lsa;
    u_int16_t type = 0;
    u_int32_t id = 0, adv_router = 0;

    type = htons (OSPF6_LSTYPE_ZONE);
    id = ZONE_AAIDESC_GET_ATTACHED_AREAID (aaidesc);
    adv_router = htonl(0);

    lsa = ospf6_lsdb_lookup (type, id, adv_router, v->area->ospf6->lsdb_zone);
...
#if 0
static char *ospf6_aaidesc_backlink (struct ospf6_lsa *lsa,
                                     caddr_t aaidesc, struct ospf6_vertex *v)
{
    caddr_t backlink, found = NULL;
    int size;

    size = sizeof (struct ospf6_zone_aaidesc);

    for (backlink = OSPF6_LSA_HEADER_END (lsa->header) + 4;
            backlink + size <= OSPF6_LSA_END (lsa->header); backlink += size)
    {
        assert (! OSPF6_LSA_IS_TYPE (ZONE, lsa));

        if (ZONE_AAIDESC_GET_ATTACHED_AREAID (backlink) != ZONE_AAIDESC_GET_ATTACHED_AREAID(aaidesc))
            continue;
        found = backlink;
    }
...
static void
ospf6_zone_nexthop_calc (struct ospf6 *o, struct ospf6_vertex *w, struct ospf6_vertex *v, caddr_t aaidesc)
{
    int i;
    u_int16_t type;
    char buf[64];
    u_int32_t attached_area_id;
    char bit8;
    struct ospf6_route_table *table;
    struct ospf6_route *route;
    assert (VERTEX_IS_TYPE (ZONE, w));

    attached_area_id = ZONE_AAIDESC_GET_ATTACHED_AREAID (aaidesc);
    bit8 = (attached_area_id>>24) &0xFF;
    DG("bit8:%d.\n", bit8);
    ...
    while (route)
    {
        if ((route->path.type == type) && (route->type == OSPF6_DEST_TYPE_NETWORK))
        {
#if 1
            char destination[64];
            ...
            prefix2str (&prefix, destination, sizeof (destination));
            DG("prefix:%s.\n", destination);
#endif
            if (prefix_cmp(&route->prefix, &prefix) == 0)
            {
                DG("get same prefix in route table.\n");
                if (IS_OSPF6_DEBUG_SPF (PROCESS))
                {
                    char ifname[IFNAMSIZ];
                    memset(buf, 0x00, sizeof(buf));
                    inet_ntop (AF_INET6, &route->nexthop[i].address, buf, sizeof (buf));
                    if_indextoname (route->nexthop[i].ifindex, ifname);
                    zlog_debug ("  nexthop %s ifname %s", buf, ifname);
                }

                memcpy (&w->nexthop[i].address, &route->nexthop[i].address,
                        sizeof (struct in6_addr));
                w->nexthop[i].ifindex = route->nexthop[i].ifindex;
                //FIXME:
                w->cost += route->path.cost;
                i++;
                break;
            }
        }
...
static void ospf6_spf_calculation_zone (u_int32_t router_id, struct ospf6_route_table *result_table, struct ospf6 *o)
{
    ...
    /* Install the calculating router itself as the root of the SPF tree */
    /* construct root vertex */
    //FIXME:adv_router shoule be 0
#if 1
    char buf[16];
    memset(buf, 0x00, sizeof(buf));
    inet_ntop (AF_INET, &o->self_area_id, buf, sizeof (buf));
    DG("self area id:%s.\n", buf);
#endif
    lsa = ospf6_lsdb_lookup (htons (OSPF6_LSTYPE_ZONE), o->self_area_id,
                             htonl(0), o->lsdb_zone);
    if (lsa == NULL)
    {
        DG("lsa is null.\n");
    ...
    while (candidate_list->size)
    {
        ...
        /* For each LS description in the just-added vertex V's LSA */
        size = sizeof (struct ospf6_zone_aaidesc);
        for (aaidesc = OSPF6_LSA_HEADER_END (v->lsa->header) + 4;
                aaidesc + size <= OSPF6_LSA_END (v->lsa->header); aaidesc += size)
        {
            lsa = ospf6_aaidesc_lsa (aaidesc, v);
            if (lsa == NULL)
                continue;
#if 0
            if (! ospf6_aaidesc_backlink (lsa, aaidesc, v))
                continue;
#endif
            w = ospf6_vertex_create (lsa);
            w->area = o->zone_oa;
            ...
            if (w->hops == 1 && v->hops == 0)
                ospf6_zone_nexthop_calc (o, w, v, aaidesc);
            else
            {
                for (i = 0; ospf6_nexthop_is_set (&v->nexthop[i]) &&
                        i < OSPF6_MULTI_PATH_LIMIT; i++)
    ...
    o->zone_oa->spf_calculation++;
}

static int
ospf6_spf_calculation_zone_thread (struct thread *t)
{
    struct ospf6 *ospf6;
    struct timeval start, end, runtime;
    int areas_processed = 0;
...
void ospf6_spf_schedule_zone (struct ospf6 *ospf6, unsigned int reason)
{
    unsigned long delay, elapsed, ht;
    struct timeval now, result;
#if 1
    {
        char rbuf[32];
        ospf6_spf_reason_string(reason, rbuf, sizeof(rbuf));
        DG("SPF: calc reason:%u:%s.\n", ospf6->spf_reason, rbuf);
    }
#endif
    ospf6_set_spf_reason(ospf6, reason);
    ...
    if (ospf6 == NULL)
    {
        DG("ospf6 == NULL, spf cal resturn here.\n");
        return;
    }

    /* SPF calculation timer is already scheduled. */
    if (ospf6->t_spf_zone_calc)
    {
        if (IS_OSPF6_DEBUG_SPF(PROCESS) || IS_OSPF6_DEBUG_SPF (TIME))
            zlog_debug ("SPF: calculation timer is already scheduled: %p",
                        ospf6->t_spf_zone_calc);

        DG("spf calc is already existed, resturn here.\n");
        return;
    }
    ...
    ospf6->t_spf_zone_calc =
        thread_add_timer_msec (master, ospf6_spf_calculation_zone_thread, ospf6, delay);
}
#endif

#ifdef TWOD
// add haozhiqiang 2016-1-26
void
ospf6_spf_schedule_e(struct ospf6_area *oa)
{
    //printf("the program excute this (3)\n");
    if (oa->thread_spf_calculation)
        return;
    //printf("the program excute this (4)\n");
    oa->thread_spf_calculation =
        thread_add_event(master, ospf6_spf_calculation_thread_e, oa, 0);
}
#endif
