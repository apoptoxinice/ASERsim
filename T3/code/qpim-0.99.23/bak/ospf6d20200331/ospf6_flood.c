...
static void ospf6_zone_lsa_originate (struct ospf6_lsa *lsa, int flood_flag)
{
    ...
    if (old && ! OSPF6_LSA_IS_DIFFER (lsa, old))
    {
        ...
        DG("Suppress updating LSA: %s, return here.\n", lsa->name);
        ospf6_lsa_delete (lsa);
        return;
    }
    ...
    DG(">>>>will install zone lsa to lsdb_zone.\n");
    ospf6_install_lsa (lsa);
...
void ospf6_install_lsa_for_leaving (struct ospf6_lsa *lsa)
{
    ...
    /* actually install */
    lsa->installed = now;
    printf("will call ospf6_lsdb_add_for_leaving.\n");
                       ...
                       struct ospf6_lsa *lsa, struct ospf6_interface *oi)
{
            ...
            (oi->type == OSPF_IFTYPE_POINTOPOINT))
    {
        ...
        //DG("copy lsa:%s to lsupdate_list, count:%d.\n", lsa->name, oi->lsupdate_list->count);
#endif
        if (oi->thread_send_lsupdate == NULL)
        {
            //DG("will call ospf6_lsupdate_send_interface.\n");
                  ...
                  struct ospf6_lsa *lsa, struct ospf6_area *oa)
{
    ...
    for (ALL_LIST_ELEMENTS (oa->if_list, node, nnode, oi))
    {
        ...
        {
            printf("will add new lsa to new_lsas_list.\n");
...
static u_char ospf6_drouter_area_eletcion(struct ospf6 *o, u_int32_t adv_router)
{
    if (ntohl (o->drouter_id) > ntohl (adv_router))
    {
        if (drouter_is_self(o))
        {
            ...
            //FIXME:purge all zone lsa
            type = ntohs (OSPF6_LSTYPE_ZONE);
            adv_router = o->drouter_id;
            for (lsa = ospf6_lsdb_type_router_head (type, adv_router, o->lsdb_zone); lsa;
...
int ospf6_process_pbr_router_lsa (struct ospf6_lsa *lsa, struct ospf6_area *oa)
{
    ...
    int nad_num = 0; /*neighbor aread id num*/
            ...
            current += sizeof (struct ospf6_router_lsdesc))
    {
        ...
        {
            ospf6_add_path_num_to_list(lsa->header->adv_router, lsdesc->neighbor_interface_id);
    ...
    DG("nad num:%d.\n", nad_num);
    if (nad_num == 1) /*Filter out just added neighbor aread id*/
        ospf6_maybe_del_attached_num_from_list(lsa->header->adv_router, neighbor_area_id);
    else if (nad_num == 0) /*This is a router lsa with the B flag set, but there is no attached aread id*/
                   ...
                   struct ospf6_lsa_header *lsa_header)
{
    ...
    if (old == NULL || ismore_recent < 0)
    {
        ...
        if (new->header->adv_router != from->ospf6_if->area->ospf6->router_id)
        {
            ...
            //if ((drouter_is_self(from->ospf6_if->area->ospf6)) && (new->header->type == htons(OSPF6_LSTYPE_LINK)))
