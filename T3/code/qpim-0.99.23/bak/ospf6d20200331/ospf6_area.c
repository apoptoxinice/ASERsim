...
ospf6_area_show (struct vty *vty, struct ospf6_area *oa)
{
    struct listnode *i;
    struct ospf6_interface *oi;
    char abuf[16], pbuf[64], lbuf[64];

    vty_out (vty, " Area %s%s", oa->name, VNL);
    vty_out (vty, "     Number of Area scoped LSAs is %u%s",
             oa->lsdb->count, VNL);

    vty_out (vty, "     Interface attached to this area:%s", VNL);
    for (ALL_LIST_ELEMENTS_RO (oa->if_list, i, oi))
    {
                ...
                && (oi->interface_pbi_flag == OSPF6_PBIENABLED))
        {
            ...
            {
                vty_out(vty, "%s", "down%s", VNL);
                break;
            }
            memset(abuf, 0x00, sizeof(abuf));
            memset(pbuf, 0x00, sizeof(pbuf));
            memset(lbuf, 0x00, sizeof(lbuf));
            inet_ntop (AF_INET, &oi->neighbor_area_id, abuf, sizeof (abuf));

            prefix2str (&oi->neighbor_prefix, pbuf, sizeof (pbuf));

            inet_ntop (AF_INET6, &oi->neighbor_linklocal_addr, lbuf, sizeof (lbuf));
            //neighbor area id, neighbor area prefix, neighbor linklocal addr
            vty_out(vty, "nai:%s nap:%s nla:%s ", abuf, pbuf, lbuf);
        }
#endif
        vty_out (vty, "%s", VNL);
    }
...
#if 1//add 2020
    vty_out(vty, "###################Display Zone###################%s", VNL);
    ospf6_linkstate_prefix (htonl(0), ospf6->self_area_id, &prefix);

    route = ospf6_route_lookup (&prefix, ospf6->spf_table_zone);
    if (route == NULL)
    {
        vty_out (vty, "LS entry for root not found in ospf %s", VNL);
        return CMD_WARNING;
    }
    root = (struct ospf6_vertex *) route->route_option;
    ospf6_spf_display_subtree (vty, "", 0, root);
