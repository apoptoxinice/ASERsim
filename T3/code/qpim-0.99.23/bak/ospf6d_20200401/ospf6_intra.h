...
#if 1
//add haozhiqiang 2015-12-17 源地址前缀数据格式
#define OSPF6_SOURCE_ADDR_TLV_TYPE  10
#define OSPF6_SOURCE_ADDR_TLV_LENGTH  20         // no include type and length 

struct ospf6_source_addr_prefix_TLV
{
    u_int16_t type;			/* TE_TLV_XXX (see below) */
    u_int16_t length;			/* Value portion only, in octets */
    u_int16_t tlv_metric;
    u_int8_t  tlv_length;
    u_int8_t  tlv_options;
};
#endif
...
//add 2020
#define OSPF6_ROUTER_LSDESC_PBRP2P_LINK        5
...
#if 1 //add 2020
#define ZONE_AAIDESC_GET_ATTACHED_AREAID(x)                  \
  (((struct ospf6_zone_aaidesc *)(x))->area_id)
#endif
...
#define OSPF6_ROUTER_LSA_SCHEDULE_FOR_LEAVING(oa) \
  do { \
    if (! (oa)->thread_router_lsa) \
      (oa)->thread_router_lsa = \
        thread_add_event (master, ospf6_router_lsa_originate_for_leaving, oa, 0); \
  } while (0)
...
#ifdef TWOD  //add haozhiqiang 2015-12-16
#define OSPF6_E_INTRA_PREFIX_LSA_SCHEDULE_STUB(oa) \
  do { \
    if (! (oa)->thread_intra_prefix_lsa) \
      (oa)->thread_intra_prefix_lsa = \
        thread_add_event (master, ospf6_extend_intra_prefix_lsa_originate_stub, \
                          oa, 0); \
  } while (0)
#endif
...
extern char *ospf6_network_lsdesc_lookup (u_int32_t router_id,
        struct ospf6_lsa *lsa);
extern int ospf6_router_lsa_originate_for_leaving (struct thread *);
...
extern int ospf6_extend_intra_prefix_lsa_originate_stub(struct thread *thread);//hzq
...
extern void ospf6_zone_route_calculation (struct ospf6 *o);
...
//zlw
extern int ospf6_send_del_acl_msg(char *src,char *dst,char *port);
extern int ospf6_send_acl_msg(char *src,char *dst,char *port);
#endif /* OSPF6_LSA_H */
