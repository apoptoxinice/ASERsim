...
#if 0
    zlog_debug("recv fe80:%s.", ipv6_address_string);
#endif
    for (ALL_LIST_ELEMENTS_RO (oi->neighbor_list, n, on))
    {
#if	0
        memset (ipv6_address_string, 0, sizeof (ipv6_address_string));
        inet_ntop (AF_INET6, on->linklocal_addr, ipv6_address_string, sizeof (ipv6_address_string));
        zlog_debug("in neigh, fe80:%s.\n", ipv6_address_string);
#endif
...
#if 1
static int update_neighbor_period (struct in6_addr *ipv6_addr)
{
    struct ospf6_neighbor *on;
    ...
    for (ALL_LIST_ELEMENTS_RO (iflist, i, ifp))
    {
        if (ifp->info == NULL)
        {
            zlog_debug (" OSPF not enabled on this interface.");
            continue;
        }
        else
        ...
        {
            if (!memcmp (&on->linklocal_addr, ipv6_addr, sizeof (struct in6_addr)))
            {
                THREAD_OFF (on->circle_period_timer);
                on->update_nb_period_timer_state = 1;
                zlog_debug("reset update_nb_period_timer_state to 1.\n");

                /*Speed up the sending of Hello message*/
                if (on->state == OSPF6_NEIGHBOR_LEAVING)
                {
#if 0
                    printf(">>>>>>>>>>>>will del iptables.\n");
                    char *cmd = "ip6tables -D OUTPUT -d ff02::5 -j DROP";
                    system(cmd);
#endif
                    on->first_hello_state = 0;
                    on->inactivity_timer_2_state = 0;
                    oi->hello_interval = MIN_HELLO_DETECTION_TIME;

                    if (oi->thread_send_hello)
                        THREAD_OFF (oi->thread_send_hello);

                    zlog_debug("speed up the sending of hello message:%d, add hello send thread.", oi->hello_interval);
...
static int update_neighbor_period (struct in6_addr *ipv6_addr)
{
    ...
    printf("will update neighbor period.\n");
...
static int check_stl_num_in_list (struct satellite_period *period)
{
    ...
    while (p)
    {
        ...
        zlog_debug (">>>>>>>neighbor_ipv6_address:%s", ipv6_address_string);
        ...
        if (p->stl_msg_all.stl_num == ntohs(period->stl_num))
        {
            ...
            memset (ipv6_address_string, 0, sizeof (ipv6_address_string));
            inet_ntop (AF_INET6, &p->stl_msg_all.neighbor_ipv6_address, ipv6_address_string, sizeof (ipv6_address_string));
            zlog_debug ("%s()%d The neighbor ipv6 address:%s already exist in list", __func__, __LINE__, ipv6_address_string);
            return 1;
        }
        p = p->next;
    }
...
static void insert_orbital_period_msg_to_list(struct satellite_period *period)
{

    /*first: check the msg is exist in list, and update it*/
    ...
    p = (stl_link_list) malloc (sizeof (struct satellite_predict_list));
    if (p == NULL)
    {
        zlog_err ("%s()%d malloc for satellite node failed", __func__, __LINE__);
        return;
    }
    memset (p, 0, sizeof (struct satellite_predict_list));
...
static int insert_connection_period_msg_to_list (struct satellite_msg *recv_info)
{
    stl_link_list p;
    char ipv6_address_string[128];
    ...
    while (p)
    {
        if (p->stl_msg_all.stl_num == ntohs(recv_info->stl_num))
        {
                    ...
                    ||(p->stl_msg_all.end_time != recv_info->end_time))
            {
                ...
                if (update_neighbor_period (&p->stl_msg_all.neighbor_ipv6_address) == 0)
                    return 0;

                zlog_debug("update start time, end time, connection period, now st:%f et:%f ct:%f", p->stl_msg_all.start_time, p->stl_msg_all.end_time, p->stl_msg_all.connection_period);
                return 1;
            }

            memset (ipv6_address_string, 0, sizeof (ipv6_address_string));
            inet_ntop (AF_INET6, &p->stl_msg_all.neighbor_ipv6_address, ipv6_address_string, sizeof (ipv6_address_string));
            zlog_debug ("The neighbor ipv6 address:%s in list, don't update.", ipv6_address_string);
            return 1;
        }
        p = p->next;
    }
...
#if 0  /*sangmeng*/
static int process_satellite_message (char *recv_buf)
{
    int total_msg_len;
    int msg_len;
    ...
    for (msg_len = 0; msg_len < total_msg_len;)
    {
        ...
        struct satellite_msg_all *stl_msg_all = (struct satellite_msg_all *) (msghead->data);
#ifdef OSPF6_DEBUG
        zlog_debug ("\nrecv satellite msg head len:%d ", ntohs (msghead->len));
#endif
...
static inline void recv_local (void)
{
    ...
    struct satellite_msg *recv_info[10];
    struct satellite_period *period[10];
    ...
    while (1)
    {
        ...
        {
            for (i = 0; i < ntohs(msghead->len) / sizeof (struct satellite_msg); i++)
            {
                recv_info[i] = (struct satellite_msg *) (msghead->data + (sizeof (struct satellite_msg) * i));
        ...
        {
            printf("sizeofsatellite_period=%lu\n",sizeof(struct satellite_period));

            for (i = 0; i < ntohs(msghead->len) / sizeof (struct satellite_period); i++)
            {
                period[i] = (struct satellite_period *) (msghead->data + (sizeof (struct satellite_period) * i));
#if 1
...
#if 0
static void *recv_satellite_message (void *arg)
{
...
static int thread_satellite_create (void)
{
    int code;
    pthread_t tid;
    int thread_stack_size;
    thread_stack_size = 1024 * 1024; /* 堆栈设置为1024K = 1M，thread_stack_size以字节为单位*/
    if ((code = create_pthread (recv_satellite_message, NULL, thread_stack_size, &tid)) != 0)
    {
...
static void ospf6_event (enum event event, int sock, void *arg);

static int ospf6_client_read (struct thread *thread)
{
    int sock;
    uint16_t i;
    char buff[BUFSIZE];
    ...
    if (recv (sock, buff, BUFSIZE, 0) < 0)
    {
        fprintf (stderr, "recv failed %s\n", strerror (errno));
    }

    head = (struct comm_head *) buff;
#if 0
    printf("sizeof(comm_head)=%lu\n", sizeof(struct comm_head));
    printf("sizeof(msg_head)=%lu\n", sizeof(struct msg_head));
#endif
    msghead = (struct msg_head *) (head->data);
    printf ("msghead->chrType=%d\n", msghead->chrType);
#if 0
    printf("msghead->len=%d\n",msghead->len);
    printf("msghead->len ntohs=%d\n",ntohs(msghead->len));
...
#include "network.h"
static int
ospf6_accept (struct thread *thread)
{
    int accept_sock;
    int client_sock;
    ...
    /* Reregister myself. */
    ospf6_event (OSPF6_SERV, accept_sock, NULL);
    ...
    if (client_sock < 0)
    {
        zlog_warn ("Can't accept ospf6 socket: %s", safe_strerror (errno));
        return -1;
    }
    ...
    ospf6_event (OSPF6_READ, client_sock, NULL);

    return 0;
}
/* Make ospf6's server socket. */
static void
ospf6_recv_satellite_serv (void)
{
    int ret;
    int accept_sock;
    struct sockaddr_in6 addr;

    accept_sock = socket (AF_INET6, SOCK_STREAM, 0);

    if (accept_sock < 0)
    {
        zlog_warn ("Can't create zserv stream socket: %s",
                   safe_strerror (errno));
        zlog_warn ("ospf6 can't provice full functionality due to above error");
        return;
    }
...
static void ospf6_event (enum event event, int sock, void *arg)
{
    ...
    {
    case OSPF6_SERV:
            printf("thread add socket:%d for accept.\n", sock);
        thread_add_read (master, ospf6_accept, NULL, sock);
        break;
    case OSPF6_READ:
        printf("thread add socket:%d for read.\n", sock);
        thread_add_read (master, ospf6_client_read, NULL, sock);
        break;
    }
}
#endif
...
#if 1 //add 2020
    memset(&g_global_prefix, 0x00, sizeof(g_global_prefix));
    str2prefix("2001:da8::/32", &g_global_prefix);
#endif
...
#if 0
    /* create thread for recv satellite info */
    if (thread_satellite_create () == -1)
    {
        zlog_err ("OSPF+ create thread for satellite info recv failed");
        exit (1);
    }
#endif
    ospf6_recv_satellite_serv();

#ifdef OSPF6_DEBUG
    zlog_debug ("OSPF+ create thread for satellite info recv successfully");
#endif
    /* pid file create */
    pid_output (pid_file);
