...
static struct pimsm_rpf *pimsm_CreateRpfEntry(VOS_IPV6_ADDR *pstDstAddr);
...
uint32_t pimsm_GrpHashMake(struct pimsm_grp_entry *pstGrpEntry)
{
    VOS_IPV6_ADDR *pstIpv4Addr = NULL;
    uint32_t key;
    uint32_t c;
    ...
    for(c = 0; c < sizeof(VOS_IPV6_ADDR); c++)
    {
...
static struct pimsm_grp_entry *pimsm_CreateGrpEntry(VOS_IPV6_ADDR *pstGrpAddr)
{
    ...
    memcpy(&pstGrpEntry->address, pstGrpAddr, sizeof(VOS_IPV6_ADDR));
    ...
    else
    {
        ...
        memcpy(&pstGrpEntry->rp_addr, &rp_addr, sizeof(VOS_IPV6_ADDR));
    }
...
static struct pimsm_src_entry *pimsm_CreateSrcEntry(VOS_IPV6_ADDR *src_addr)
{
    ...
    memcpy(&pstSrcEntry->address, src_addr, sizeof(VOS_IPV6_ADDR));
...
static struct pimsm_mrt_entry *pimsm_CreateStarGrpEntry(struct pimsm_grp_entry *pstGrpEntry,uint32_t flags)
{
    ...
    if ((PIMSM_SSM_RANGE != pstGrpEntry->type ) &&
            (ip_isAnyAddress(&pstGrpEntry->rp_addr) == FALSE))
    {
        ...
        if(NULL != pstRpf)
        {
            ...
            memcpy(&mrt_entry->upstream_if.up_stream_nbr, &pstRpf->rpf_nbr_addr, sizeof(VOS_IPV6_ADDR));
...
struct pimsm_mrt_entry *pimsm_CreateMrtEntry(VOS_IPV6_ADDR *src_addr,VOS_IPV6_ADDR *pstGrpAddr,uint8_t type,uint32_t flags)
{
    struct pimsm_mrt_entry *mrt_entry = NULL;
    struct pimsm_grp_entry *pstGrpEntry = NULL;
    struct pimsm_src_entry *pstSrcEntry = NULL;
...
struct pimsm_grp_entry *pimsm_SearchGrpEntry(VOS_IPV6_ADDR *pstGrpAddr)
{
    struct pimsm_grp_entry stGrpEntry;
    ...
    {
        memcpy(&(stGrpEntry.address), pstGrpAddr, sizeof(VOS_IPV6_ADDR));
        return (struct pimsm_grp_entry *)hash_search(g_group_hash, &stGrpEntry);
    }
    return NULL;
}

struct pimsm_src_entry *pimsm_SearchSrcEntry(VOS_IPV6_ADDR *src_addr)
{
    struct pimsm_src_entry stSrcEntry;
    ...
    {
        memcpy(&(stSrcEntry.address), src_addr, sizeof(VOS_IPV6_ADDR));
...
static struct pimsm_mrt_entry *pimsm_SearchStarGrpEntry(VOS_IPV6_ADDR *pstGrpAddr)
{
    //struct pimsm_mrt_entry *mrt_entry = NULL;
    struct pimsm_grp_entry *pstGrpEntry = NULL;
...
static struct pimsm_mrt_entry *pimsm_SearchSrcGrpEntry(VOS_IPV6_ADDR *src_addr,VOS_IPV6_ADDR *pstGrpAddr,uint8_t type)
{
    struct pimsm_mrt_entry *mrt_entry = NULL;
    struct pimsm_grp_entry *pstGrpEntry = NULL;
...
struct pimsm_mrt_entry *pimsm_SearchMrtEntry(VOS_IPV6_ADDR *src_addr,VOS_IPV6_ADDR *pstGrpAddr,uint8_t type)
{
    if ((PIMSM_MRT_TYPE_WC == type) ||
            (PIMSM_MRT_TYPE_SG == type) ||
            (PIMSM_MRT_TYPE_SGRPT == type))
...
static int pimsm_InterfaceAssertWinner(struct pimsm_mrt_entry *mrt_entry, uint32_t ifindex, VOS_IPV6_ADDR *pstWinnerAddr)
{
    struct pimsm_assert_if *pstAssertInterface;
    ...
    memset(pstWinnerAddr, 0, sizeof(VOS_IPV6_ADDR));
    ...
    while(NULL != pstAssertInterface)
    {
        if (ifindex == pstAssertInterface->ifindex)
        {
            memcpy(pstWinnerAddr, &(pstAssertInterface->stAssertStateMachine.stAssertWinner.address), sizeof(VOS_IPV6_ADDR));
        }
...
int pimsm_AddLocalReceiver(VOS_IPV6_ADDR *src_addr,VOS_IPV6_ADDR *pstGrpAddr,uint8_t type,uint32_t ifindex)
{
...
int pimsm_DelLocalReceiver(VOS_IPV6_ADDR *src_addr,VOS_IPV6_ADDR *pstGrpAddr,uint8_t type,uint32_t ifindex)
{
    struct pimsm_mrt_entry *pstSrcGrpEntryNext = NULL;
    struct pimsm_updownstream_if_entry *pstDownIf;
    struct pimsm_mrt_entry *src_grp_entry;
...
int pimsm_KeepAliveTimerStart(struct pimsm_mrt_entry *src_grp_entry, uint32_t dwTimerValue)
{
    ...
    {
        memcpy(&keepAlivePara->src_addr, &src_grp_entry->source->address, sizeof(VOS_IPV6_ADDR));
    }
    if(NULL != src_grp_entry->group)
    {
        memcpy(&keepAlivePara->grp_addr, &src_grp_entry->group->address, sizeof(VOS_IPV6_ADDR));
    }
    keepAlivePara->type = src_grp_entry->type;

    PIM_DEBUG("will call pimsm_TimeroutKeepAlive.\n");
...
int pimsm_MrtDelTimerStart(struct pimsm_mrt_entry *mrt_entry, uint32_t dwTimerValue)
{
    ...
    if(NULL != mrt_entry->source)
    {
        memcpy(&mrtDelPara->src_addr, &mrt_entry->source->address, sizeof(VOS_IPV6_ADDR));
    }
    if(NULL != mrt_entry->group)
    {
        memcpy(&mrtDelPara->grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
    }
...
{
    struct pimsm_mf6cctl mc;
    struct pimsm_oif *oil;

    zassert(NULL != pstHardEntry);

    PIM_DEBUG("(********important********)Enter pimsm_AddHardwareEntry, iif:%s, tocpu:%d.\n", ifindex2ifname(pstHardEntry->iif), pstHardEntry->bIsCopyToCpu);

    memset(&mc, 0, sizeof(struct pimsm_mf6cctl));

    init_sin6(&mc.mf6cc_origin);
    init_sin6(&mc.mf6cc_mcastgrp);
    if (ip_isAnyAddress(&pstHardEntry->src_addr))
    {
        PIM_DEBUG("pstHardEntry->src_addr is 0.\n");
    }
    else
    {
        memcpy(&mc.mf6cc_origin.sin6_addr, &pstHardEntry->src_addr, sizeof(VOS_IPV6_ADDR));
    }
    memcpy(&mc.mf6cc_mcastgrp.sin6_addr, &pstHardEntry->grp_addr, sizeof(VOS_IPV6_ADDR));

    char group_str[100];
    char source_str[100];
    pim_inet6_dump("<group?>",  mc.mf6cc_mcastgrp.sin6_addr, group_str, sizeof(group_str));
    pim_inet6_dump("<source?>", mc.mf6cc_origin.sin6_addr, source_str, sizeof(source_str));

    if (pstHardEntry->iif < 1)
    {
    ...
    mc.mf6cc_parent =  pstHardEntry->iif;
    oil = pstHardEntry->oil;
    while(NULL != oil)
    {
        if (oil->ifindex)
        {
            PIM_DEBUG("oil->ifindex:%d.\n", oil->ifindex);
            IF_SET(oil->ifindex, &mc.mf6cc_ifset);
        }
        oil = oil->next;
    }
#if 1
...
#endif
    mc.topimsm = pstHardEntry->bIsCopyToCpu;

    if (pimsm_mroute_add(&mc))
    {
        zlog_warn("%s %s: could not add  out interface, (S,G)=(%s,%s), iif:%d",
                  __FILE__, __PRETTY_FUNCTION__,
                  source_str, group_str, pstHardEntry->iif);

        return -1;
    }

    PIM_DEBUG("add  out interface (S,G)=(%s,%s) success, iif:%d.\n", source_str, group_str, pstHardEntry->iif);
    return VOS_OK;
}
...
static int pimsm_DelHardwareEntry(struct pimsm_mrt_hardware_entry *pstHardEntry)
{
    ...
    PIM_DEBUG("(********important********)Enter pimsm_DelHardwareEntry, iif:%s, tocpu:%d.\n", ifindex2ifname(pstHardEntry->iif), pstHardEntry->bIsCopyToCpu);
    ...
    while(NULL != oil)
    {
        ...
        {
            PIM_DEBUG("oil->ifindex:%d.\n", oil->ifindex);
            IF_CLR(oil->ifindex, &mc.mf6cc_ifset);
        }
        oil = oil->next;
    }
#if 1
...
static int pimsm_AddHardwareEntryOif(struct pimsm_mrt_hardware_entry *pstHardEntry, uint32_t dwOifIndex)
{
    ...
    PIM_DEBUG("(*******important*******)Enter pimsm_AddHardwareEntryOif, dwOifIndex:%d, iif:%s, tocpu:%d.\n", dwOifIndex, ifindex2ifname(pstHardEntry->iif), pstHardEntry->bIsCopyToCpu);
    ...
    PIM_DEBUG("will add oif:%d.\n", pimsm_ifp->mroute_vif_index);
    IF_SET(pimsm_ifp->mroute_vif_index, &mc.mf6cc_ifset);
#if 1
    /*
     * draft-ietf-pim-sm-v2-new-05.txt section 4.2 mentions iif is removed
     * at the packet forwarding phase
...
static int pimsm_DelHardwareEntryOif(struct pimsm_mrt_hardware_entry *pstHardEntry,uint32_t dwOifIndex)
{
    ...
    PIM_DEBUG("(********important************)Enter pimsm_DelHardwareEntryOif, dwOifIndex:%d, iif:%s, tocpu:%d.\n", dwOifIndex, ifindex2ifname(pstHardEntry->iif), pstHardEntry->bIsCopyToCpu);
    ...
    PIM_DEBUG("del mroute_vif_index:%d.\n", pimsm_ifp->mroute_vif_index);
    IF_CLR(pimsm_ifp->mroute_vif_index, &mc.mf6cc_ifset);
#if 1
    /*
     * draft-ietf-pim-sm-v2-new-05.txt section 4.2 mentions iif is removed
     * at the packet forwarding phase
    */
    ...
    if (pstHardEntry->oil_size < 1)
    {
        if (pimsm_mroute_del(&mc))
        {
            ...
            char group_str[100];
            char source_str[100];
            pim_inet6_dump("<group?>", mc.mfcc_mcastgrp, group_str, sizeof(group_str));
            pim_inet6_dump("<source?>", mc.mfcc_origin, source_str, sizeof(source_str));
...
static int pimsm_AddHardwareEntryToCpu(struct pimsm_mrt_hardware_entry *pstHardEntry)
{
    ...
    PIM_DEBUG("(********important********)Enter pimsm_AddHardwareEntryToCpu, iif:%s, tocpu:%d.\n", ifindex2ifname(pstHardEntry->iif), pstHardEntry->bIsCopyToCpu);
...
static int pimsm_DelHardwareEntryToCpu(struct pimsm_mrt_hardware_entry *pstHardEntry)
{
    ...
    PIM_DEBUG("(********important********)Enter pimsm_DelHardwareEntryToCpu, iif:%s, tocpu:%d.\n", ifindex2ifname(pstHardEntry->iif), pstHardEntry->bIsCopyToCpu);
...
static struct pimsm_rpf *pimsm_CreatRpfChangeRegister(VOS_IPV6_ADDR *pstDstAddr)
{
    struct pimsm_rpf *pstRpf = NULL;
    ...
    memcpy(&addr, pstDstAddr, sizeof(struct in6_addr));
    ...
    else
    {
        ...
        while(NULL != hard_list_new)
        {
            ...
            if (NULL != hard_list_old)
            {
                ...
                oil_1 = pimsm_OilSubOil(hard_list_new->oil, hard_list_old->oil);
                oil_2 = pimsm_OilSubOil(hard_list_old->oil, hard_list_new->oil);

                if ((hard_list_new->iif != hard_list_old->iif) ||
...
int pimsm_DestroyAllHardwareEntry(void)
{
    ...
    while(NULL != pstHardwareEntry)
    {
        ...
        pimsm_FreeOil(pstHardwareEntry->oil);
    ...
    while(NULL != pstHardwareEntry)
    {
        if (PIMSM_MRT_TYPE_WC == pstHardwareEntry->type)
        {
            vty_out(vty, "(*, %s) iif: %s",
                    ...
                    pimsm_GetIfName(pstHardwareEntry->iif));
        }
        ...
        if(NULL != pstGroupEntry)
        {
            ...
            if (NULL != mrt_entry)
            {
                if (mrt_entry->valid)
                {
                    ...
                    pstHardwareEntry->oil = pstImmediateOutIfList;
                    pstHardwareEntry->grp_addr = mrt_entry->group->address;
                    pstHardwareEntry->iif = mrt_entry->upstream_if.ifindex;
            ...
            for (; NULL != mrt_entry; mrt_entry = mrt_entry->samegrpnext)
            {
                ...
                pstHardwareEntry->oil = pstInheritedOutIfList;
...
static int pimsm_RpfChanged(struct pimsm_rpf *pstOldRpf, struct pimsm_rpf *pstNewRpf)
{
    ...
    struct pimsm_assert_if *pimsmassert_if;
    VOS_IPV6_ADDR stNewUpNbrAddr;
    VOS_IPV6_ADDR stOldUpNbrAddr;
    VOS_IPV6_ADDR stUpNbrAddr;
    int dwNewUpIf;
    int dwOldUpIf;
    ...
    memset(&stNewUpNbrAddr, 0, sizeof(VOS_IPV6_ADDR));
    memset(&stOldUpNbrAddr, 0, sizeof(VOS_IPV6_ADDR));
        ...
        if(NULL != pstGroupEntry)
        {
            ...
            if (0 == ip_AddressCompare(&pstGroupEntry->rp_addr, &pstOldRpf->dest_addr))
            {
                ...
                if (NULL != pstStarGrpEntry)
                {
                    ...
                    memcpy(&stOldUpNbrAddr, &pstStarGrpEntry->upstream_if.up_stream_nbr, sizeof(VOS_IPV6_ADDR));
                    pimsm_RpfNeighborAddr(pstStarGrpEntry, &stNewUpNbrAddr);

                    pstStarGrpEntry->upstream_if.ifindex = pstNewRpf->rpf_if;
                    memcpy(&pstStarGrpEntry->upstream_if.up_stream_nbr, &stNewUpNbrAddr, sizeof(VOS_IPV6_ADDR));
                    ...
                    {
                        memcpy(&stUpEventPara.old_upstream_nbr_addr, &stOldUpNbrAddr, sizeof(VOS_IPV6_ADDR));
                        memcpy(&stUpEventPara.new_upstream_nbr_addr, &stNewUpNbrAddr, sizeof(VOS_IPV6_ADDR));
            ...
            while(pstSrcGrpEntry)
            {
                        ...
                        (PIMSM_MRT_TYPE_SG == pstSrcGrpEntry->type))
                {
                    ...
                    memcpy(&stOldUpNbrAddr, &pstSrcGrpEntry->upstream_if.up_stream_nbr, sizeof(VOS_IPV6_ADDR));
                    pimsm_RpfNeighborAddr(pstSrcGrpEntry, &stNewUpNbrAddr);

                    pstSrcGrpEntry->upstream_if.ifindex = pstNewRpf->rpf_if;
                    memcpy(&pstSrcGrpEntry->upstream_if.up_stream_nbr, &stNewUpNbrAddr, sizeof(VOS_IPV6_ADDR));
...
int pimsm_UpdateRpf(struct rtmgt_to_pimsm *pstNewRpfMsg)
{
    ...
    memcpy(&stNewRpf.dest_addr, &(pstNewRpfMsg->source), sizeof(VOS_IPV6_ADDR));
    memcpy(&stNewRpf.rpf_nbr_addr, &(pstNewRpfMsg->rpf_nbr_addr), sizeof(VOS_IPV6_ADDR));
    ...
    while (NULL != pstRpfCurr)
    {
        ...
        if (pim_MatchPrefix(&pstRpfCurr->dest_addr, &stNewRpf.dest_addr, pstNewRpfMsg->prefixlen))
        {
            ...
            //sangmeng change
            memcpy(&stNewRpf.dest_addr, &pstRpfCurr->dest_addr, sizeof(VOS_IPV6_ADDR)); //2001:da8:bf::----->2001:da8:bf::1
            ...
            memcpy(&pstRpfCurr->rpf_nbr_addr, &stNewRpf.rpf_nbr_addr, sizeof(VOS_IPV6_ADDR));
            ...
            if((stOldRpf.rpf_if != stNewRpf.rpf_if) ||
                    (ip_AddressCompare(&stOldRpf.rpf_nbr_addr, &stNewRpf.rpf_nbr_addr) != 0))
            {
...
static struct pimsm_rpf *pimsm_CreateRpfEntry(VOS_IPV6_ADDR *pstDstAddr)
{
    ...
    g_rpf_list = pstRpf;
    memcpy(&(pstRpf->dest_addr), pstDstAddr, sizeof(VOS_IPV6_ADDR));

    PIM_DEBUG(" create rpf rpf_addr:%s\n", pim_InetFmt(&pstRpf->dest_addr));
    return pstRpf;
}

struct pimsm_rpf *pimsm_SearchRpf(VOS_IPV6_ADDR *pstDstAddr)
{
    struct pimsm_rpf *pstRpf = NULL;
    if (NULL == pstDstAddr)
    {
...
uint32_t pimsm_RpfInterface(VOS_IPV6_ADDR *pstDstAddr)
{
    ...
    if(ip_isLinklocalAddress(pstDstAddr))
    {
        pimsm_ifp = g_pimsm_interface_list;
        while(pimsm_ifp != NULL)
        {
            if (NULL != pimsm_SearchNeighbor(pimsm_ifp, pstDstAddr))
            {
...
int pimsm_RpfNeighborAddr(struct pimsm_mrt_entry *mrt_entry, VOS_IPV6_ADDR *pstNeighborAddr)
{
    ...
    VOS_IPV6_ADDR neighbor_addr;
    VOS_IPV6_ADDR stAssertWinner;
    ...
    memset(pstNeighborAddr, 0, sizeof(VOS_IPV6_ADDR));
    ...
    if(PIMSM_MRT_TYPE_WC == mrt_entry->type)
    {
        ...
        */
        ...
        if(pimsm_InterfaceLostAssert(mrt_entry, pstRpfToRp->rpf_if))
        {
            ...
            memcpy(pstNeighborAddr, &stAssertWinner, sizeof(VOS_IPV6_ADDR));
        }
        else
        {
            if (NULL != (pstInterface = pimsm_SearchInterface(pstRpfToRp->rpf_if)))
            {
                ...
                {
                    memcpy(pstNeighborAddr, &pstInterface->pimsm_neigh->source_addr, sizeof(VOS_IPV6_ADDR));
                }
            }
        }
    }
        ...
        else
        {
            if (NULL != (pstInterface = pimsm_SearchInterface(pstRpfToSrc->rpf_if)))
            {
                ...
                while(pstRpfNbr)
                {
                    ...
                    {
                        memcpy(pstNeighborAddr, &pstRpfNbr->source_addr, sizeof(VOS_IPV6_ADDR));
                        break;
                    }
        ...
        */
        ...
        else
        {
            if (NULL != mrt_entry->group->pstStarMrtLink)
            {
                ...
                memcpy(pstNeighborAddr, &neighbor_addr, sizeof(VOS_IPV6_ADDR));
            }
        }
    }
    else
