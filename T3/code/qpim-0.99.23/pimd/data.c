...
void k_add_ip6_mif (int socket, int ifindex)
{
    ...
    mif.mif6c_flags = 0;
    printf("vif.mif6c_mifi = %d mif6c_pifi %d\n", mif.mif6c_mifi, mif.mif6c_pifi);
...
{
    printf("in %s %d ifname = %s\n",__func__,__LINE__,p_ifname);
    ...
    p_if->type = type;
    p_if->mld_enable = 1;
    p_if->if_index = /*ifname2ifindex*/if_nametoindex(p_ifname);


    printf("create OK p_if->if_name = %s\n",p_if->if_name);
...
void show_imp_interface(void)
{
    printf("will display all imp interface\n");
    imp_interface    *p_if = imp_interface_first();

    for(; p_if; p_if = LIST_NEXT(p_if, link))
    {
        printf("p_if->if_name = %s\n",p_if->if_name);
    }

}

imp_interface* imp_interface_find_by_name(const char *ifname)
{
    ...
    {
        printf("p_if->if_name = %s\n",p_if->if_name);
        if(!strcmp(p_if->if_name,ifname))
        {
            printf("find imp interface by name \n");
            return p_if;
        }
    }
    return NULL;
...
imp_interface* imp_interface_find_by_ifindex(uint8_t ifindex)
{
    printf("in %s %d ifindex = %d\n",__func__,__LINE__,ifindex);
    imp_interface    *p_if = imp_interface_first();

    for(; p_if; p_if = LIST_NEXT(p_if, link))
    {
        printf("p_if->if_name = %s\n",p_if->if_name);
        if(p_if->if_index == ifindex)
        {
            printf("find imp interface by ifindex\n");
            return p_if;
        }
    }
    return NULL;
    ...
    for(p_if = LIST_FIRST(&ifp_head); p_if; p_if = ifp_next)
    {
        ...
        else
        {
            ...
            if(imp_get_ip6_addr(p_if->if_name, &in6) == STATUS_NOK)
            {
                ...
                printf("interface %s don't have ipv6 address\n", p_if->if_name);
                goto error;
            }
            imp_build_piaddr(AF_INET6, &in6, &p_if->if_addr);
            printf("%s's ipv6 address is %s\n", p_if->if_name, imp_pi_ntoa(&p_if->if_addr));
        }
...
error:
            ...
            imp_interface_find(AF_INET, INTERFACE_DOWNSTREAM) == NULL)
    {
        ...
        if (p_mproxy->igmp_version != IM_DISABLE)
            printf("Disable IGMP, there is not igmp upstream interface or"
            ...
            imp_interface_find(AF_INET6, INTERFACE_DOWNSTREAM) == NULL)
    {
        ...
        if(p_mproxy->mld_version != IM_DISABLE)
            printf("Disable MLD, there is not mld upstream interface or"
                            ...
                            pa_list* p_src_list, group_type type, im_version version)
{
    ...
    printf("in %s %d p_gp->if_index = %d\n",__func__,__LINE__,p_gp->if_index);
    mldv2_send_msg_to_pimsm(&(p_gp->group_addr.v6.sin6_addr),p_gp->if_index,MLD_ADD_ADDR);
    return p_gp;
...
#if 0
void imp_group_print_vty(struct vty *vty,imp_interface *p_if)
{
...
imp_timer* group_timer_handler(void *data)
{
    ...
    else
    {
        ...
        printf("group %s timeout, will contive to include mode\n", imp_pi_ntoa(&pa));
...
imp_timer* group_v1_timer_handler(void* data)
{
    ...
    printf("group %s IGMPv1/MLDv1 version timer expired\n", imp_pi_ntoa(&p_gp->group_addr));
...
{
    imp_group* p_gp = data;
    printf("group %s IGMPv2 version timer expired\n", imp_pi_ntoa(&p_gp->group_addr));
...
void imp_group_cleanup(imp_group *p_gp)
{
    ...
    if(p_gp->mld_group_timer != NULL)
        mld_thread_off(&p_gp->mld_group_timer);
    ...
    mldv2_send_msg_to_pimsm(&(p_gp->group_addr.v6.sin6_addr),p_gp->if_index,MLD_CANCEL_ADDR);
    free(p_gp);
    p_gp = NULL;
}
#if 0
...
imp_timer* source_timer_handler(void *data)
{
    ...
    printf("source %s timeout, free it\n", imp_pi_ntoa(&p_is->src_addr));
