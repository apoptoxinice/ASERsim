...
void mld_v2_report(struct mld_sock *mld, struct mld_hdr *p_mldh, int buf_len)
{
    int i = 0, num = 0, cur_len = 0;
    pi_addr pig;
    int ifindex = mld->interface->ifindex;
    ...
    for (; i < num; i++)
    {
        ...
        case MLD_MODE_IS_INCLUDE:
            ...
            mcast_is_in_handler(ifindex, &pig, p_src_list, IM_IGMPv3_MLDv2);
            break;
        case MLD_MODE_IS_EXCLUDE:
            printf("MLD_MODE_IS_EXCLUDE\n");
            mcast_is_ex_hander(ifindex, &pig, p_src_list, IM_IGMPv3_MLDv2);
            break;
        case MLD_CHANGE_TO_INCLUDE:
            /*leave */
            printf("MLD_CHANGE_TO_INCLUDE\n");
            mcast_to_in_handler(ifindex, &pig, p_src_list, IM_IGMPv3_MLDv2);
            break;
        case MLD_CHANGE_TO_EXCLUDE:
            /*join*/
            printf("MLD_CHANGE_TO_EXCLUDE\n");
            mcast_to_ex_hander(ifindex, &pig, p_src_list, IM_IGMPv3_MLDv2);
            break;
        case MLD_ALLOW_NEW_SOURCES:
            printf("MLD_ALLOW_NEW_SOURCES\n");
            mcast_allow_handler(ifindex, &pig, p_src_list, IM_IGMPv3_MLDv2);
            break;
        case MLD_BLOCK_OLD_SOURCES:
            printf("MLD_BLOCK_OLD_SOURCES\n");
            mcast_block_handler(ifindex, &pig, p_src_list, IM_IGMPv3_MLDv2);
            break;
        default:
