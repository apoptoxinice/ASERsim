...
int pim_HashFunction(VOS_IPV6_ADDR *group, VOS_IPV6_ADDR *rp, int masklen);
...
static uint8_t pim_iAmBsr()
{
    ...
    return ((ip_AddressCompare(&g_bsr_addr.unicast_addr, &g_root_cbsr_id.unicast_addr) == 0) && (g_cbsr_enable));
}
...
static uint8_t pim_iBecomeBsr(void)
{
    ...
    g_bsr_hash_masklen = PIM_HASH_MSAK_LEN_V6;
    //pim_GrpRpTableClean();
    ...
    if(find)
    {
        for(i = 0; i < pstSearchEntry->rp_group.rp_count; ++i)
        {
            ...
            else if(priority == pstSearchEntry->rp_group.rp_list[i].rp_priority)
            {
                ...
                {
                    if (ip_AddressCompare(&rp_addr.unicast_addr, &pstSearchEntry->rp_group.rp_list[i].rp_addr.unicast_addr) < 0)
                    {
                        memcpy(&rp_addr, &pstSearchEntry->rp_group.rp_list[i].rp_addr, sizeof(rp_addr));
                    }
                }
...
static int pim_GrpRpTableAddEntry(struct pimsm_group_rp *pstGroupRps)
{
    ...
    if (NULL != tmp)
    {
        ...
        for(i = 0; i < pstGroupRps->rp_count; ++i)
        {
            ...
            {
                if (ip_AddressCompare(&tmp->rp_group.rp_list[j].rp_addr.unicast_addr, &pstGroupRps->rp_list[i].rp_addr.unicast_addr) == 0)
                {
                    findrp = TRUE;
                    break;
                }
            ...
            if(findrp)
            {
                ...
                memcpy(&rp_para->unicast_addr, &tmp->rp_group.rp_list[j].rp_addr.unicast_addr, sizeof(VOS_IPV6_ADDR));
            ...
            else
            {
                ...
                memcpy(&rp_para->unicast_addr, &tmp->rp_group.rp_list[iRpCount].rp_addr.unicast_addr, sizeof(VOS_IPV6_ADDR));
    ...
    else
    {
        ...
        if(pstGroupRps->rp_count != 0)
        {
            ...
            for(i = 0; i < node->rp_group.rp_count; ++i)
            {
                ...
                memcpy(&rp_para2->unicast_addr, &node->rp_group.rp_list[i].rp_addr.unicast_addr, sizeof(VOS_IPV6_ADDR));
...
static int pim_OriginateBootstrapMsg(void)
{
    ...
    p_send_bsr_buf->hash_masklen = PIM_HASH_MSAK_LEN_V6;
...
int pim_SendBootstrapMsg(VOS_IPV6_ADDR *pstDstAddr)
{
    ...
    if(pstDstAddr)
    {
        if (ip_AddressCompare(pstDstAddr, &g_stPimsm.stAllPimRouters) == 0) //multicast address
        {
...
static int pim_GenerateCrpAdvMsg(uint16_t hold_time)
{
    ...
    if(0 != wGroupCount)
    {
        ...
        while (pstCurr != NULL)
        {
            ...
            group_addr.family = ADDRF_IPV6;
            group_addr.encoding_type = ADDRT_IPV6;
            group_addr.prefixlen = pstCurr->group_masklen;
            group_addr.reserved = 0;
            group_addr.b = 0;
            group_addr.z = 0;
            memcpy(&group_addr.group_addr, &pstCurr->group_addr, sizeof(VOS_IPV6_ADDR));
...
static int pim_SendSpecificCrpAdvMsg(struct pimsm_crp_group_policy_list *pstCrpGroupPolicyEntry)
{
    ...
    group_addr.b = 0;
    group_addr.z = 0;
    memcpy(&group_addr.group_addr, &pstCrpGroupPolicyEntry->group_addr, sizeof(VOS_IPV6_ADDR));
...
int pim_ReceiveBootstrap(uint32_t ifindex,VOS_IPV6_ADDR *pstSrc,uint8_t *pim_msg,uint32_t pim_msg_size)
{
    ...
    if(stBsrAddr.family != ADDRF_IPV6)
    {
        return VOS_ERROR;
    }
    if (ip_AddressCompare(&stBsrAddr.unicast_addr, &g_root_cbsr_id.unicast_addr) == 0)
    {
        return VOS_ERROR;
    }

    if (ip_AddressCompare(&stBsrAddr.unicast_addr, &g_bsr_addr.unicast_addr) != 0)
    {
    ...
    {
        if((g_root_cbsr_priority > pstBootstrap->bsr_priority)
                || ((g_root_cbsr_priority == pstBootstrap->bsr_priority) && (ip_AddressCompare(&stBsrAddr.unicast_addr, &g_root_cbsr_id.unicast_addr) <0)))
        {
            //I become BSR
            pim_iBecomeBsr();
        }
...
int pimsm_bsr_timer(struct thread *t)
{
    ...
    case PIM_CBSR_STATE_NO_INFO:
        break;
    case PIM_CBSR_STATE_ACCEPT_PREFERED:
        pimsm_EmptyIpAddr(&g_bsr_addr.unicast_addr);
        g_bsr_hash_masklen = PIM_HASH_MSAK_LEN_V6;
...
int pim_BootstrapCbsrEnable(uint32_t ifindex, uint8_t priority)
{
    ...
    if (ip_AddressCompare(&g_root_cbsr_id.unicast_addr, &pimsm_ifp->primary_address) == 0)
    {
        ...
        if(g_root_cbsr_priority != priority)
        {
            ...
            if(!pim_iAmBsr())
            {
                if((g_bsr_priority < priority)
                        ||((g_bsr_priority == priority) && (ip_AddressCompare(&g_root_cbsr_id.unicast_addr, &g_bsr_addr.unicast_addr) > 0)))
                {
                    //I become BSR
                    pim_iBecomeBsr();
                }
    ...
    else if(ip_AddressCompare(&g_root_cbsr_id.unicast_addr, &pimsm_ifp->primary_address) != 0)
    {
        //root id init
        g_root_cbsr_id.family = ADDRF_IPV6;
        PIM_DEBUG("cp root_cbsr addr %s\n",pim_InetFmt(&pimsm_ifp->primary_address));
        memcpy(&g_root_cbsr_id.unicast_addr, &pimsm_ifp->primary_address, sizeof(VOS_IPV6_ADDR));
        g_root_cbsr_id.encoding_type = ADDRT_IPV6;
...
int pim_BootstrapCbsrDisable(void)
{
    ...
    if(ip_AddressCompare(&g_bsr_addr.unicast_addr, &g_root_cbsr_id.unicast_addr) == 0)
    {
    ...
    g_root_cbsr_id.family = ADDRF_IPV6;
    pimsm_EmptyIpAddr(&g_root_cbsr_id.unicast_addr);
    g_root_cbsr_id.encoding_type = ADDRT_IPV6;
    g_root_cbsr_priority = 0;
    g_bsr_hash_masklen = PIM_HASH_MSAK_LEN_V6;
...
int pim_BootstrapCrpEnable(uint32_t ifindex, uint8_t priority)
{
    ...
    if(g_crp_enable && (ip_AddressCompare(&g_root_crp_id.unicast_addr, &pimsm_ifp->primary_address) != 0))
    {
    ...
    g_root_crp_id.family = ADDRF_IPV6;
    g_root_crp_id.encoding_type = ADDRT_IPV6;
    memcpy(&g_root_crp_id.unicast_addr, &pimsm_ifp->primary_address, sizeof(VOS_IPV6_ADDR));
...
int pim_CrpGroupPolicyAdd(VOS_IPV6_ADDR *pstGrpAddr, uint32_t dwMaskLen)
{
    ...
    else /* 不存在,新建 */
    {
        ...
        else
        {
            ...
            memcpy(&(pstEntry->group_addr), pstGrpAddr, sizeof(VOS_IPV6_ADDR));
...
int pimsm_bootstrapconfig(struct vty *vty)
{
    ...
    while (NULL != pstCurr)
    {
        memset(&stGroupMask,0,sizeof(VOS_IPV6_ADDR));
