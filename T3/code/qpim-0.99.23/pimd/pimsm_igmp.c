...
)
{
    ...
    {
        memcpy(&src_addr, pstSrcAddr, sizeof(VOS_IPV6_ADDR));
    }
...
static PIMSM_IGMP_MRT_RESOURCE_T *pimsm_IgmpMrtResEntryAdd(VOS_IPV6_ADDR *pstSrcAddr,VOS_IPV6_ADDR *pstGrpAddr,uint8_t type,uint32_t ifindex)
{
    ...
    if (NULL == pstIgmpMrtResEntry)
    {
        ...
        memcpy(&pstIgmpMrtResEntry->src_addr, &src_addr, sizeof(VOS_IPV6_ADDR));
        memcpy(&pstIgmpMrtResEntry->grp_addr, &grp_addr, sizeof(VOS_IPV6_ADDR));
...
static int pimsm_IgmpMrtResEntryDel(VOS_IPV6_ADDR *pstSrcAddr,VOS_IPV6_ADDR *pstGrpAddr,uint8_t type,uint32_t ifindex)
{
    PIMSM_IGMP_MRT_RESOURCE_T *pstIgmpMrtResEntry;
...
static int pimsm_IgmpMrtResProcMemberAdd(uint32_t ifindex,VOS_IPV6_ADDR *pstSrcAddr, VOS_IPV6_ADDR *pstGrpAddr,PIMSM_RECORD_TYPE_T emType)
{
    PIMSM_IGMP_MRT_RESOURCE_T *pstIgmpMrtResNext;
    ...
    memcpy(&grp_addr, pstGrpAddr, sizeof(VOS_IPV6_ADDR));
...
#if 0
static int pimsm_IgmpMrtResProcMemberDel(uint32_t ifindex,VOS_IPV6_ADDR *src_addr,VOS_IPV6_ADDR *pstGrpAddr,PIMSM_RECORD_TYPE_T emType)
{
#if 0
    PIMSM_IGMP_MRT_RESOURCE_T *pstIgmpMrtRes;
    PIMSM_IGMP_MRT_RESOURCE_T *pstIgmpMrtResNext;
    ...
    else if (NULL == pstResourceOld)
    {
        while(pstResourceNew)
        {
            ...
            while(pstOif)
            {
                ...
                PIM_DEBUG("src:%s.  grp:%s \n",pim_InetFmt(&pstResourceNew->src_addr),pim_InetFmt(&pstResourceNew->grp_addr));
...
static int pimsm_IGMPv3GrpMemberAdd(struct pimsm_interface_entry *pimsm_ifp,PIMSM_RECORD_LIST_T *pstRecordListEntry,PIMSM_RECORD_TYPE_T emType)
{
    ...
    PIM_DEBUG("grp:%s\n",pim_InetFmt(&pstRecordListEntry->grp_addr));
    if(pstRecordListEntry->pstSourListHead)
        PIM_DEBUG("src:%s\n",pim_InetFmt(&pstRecordListEntry->pstSourListHead->addr));
...
#define igmp_proc
#if 1
int pimsm_IgmpGrpMemberProc(uint32_t ifindex,void *p, PIMSM_MEMBER_UPDATE_TYPE_T emType)
{
    ...
    PIM_DEBUG("grp:%s\n",pim_InetFmt(&pstRecordListHead->grp_addr));
    if(pstRecordListHead->pstSourListHead)
        PIM_DEBUG("src:%s\n",pim_InetFmt(&pstRecordListHead->pstSourListHead->addr));
