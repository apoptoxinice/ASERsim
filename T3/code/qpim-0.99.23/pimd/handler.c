...
#define FALSE 0
#if 1
static int imp_group_clean_timer(struct thread *t);
#endif
                        ...
                        pa_list* p_src_list, im_version version)
{
    ...
    if (p_gp != NULL)
    {
        ...
        if (p_gp->version < version && version == IM_IGMPv3_MLDv2)
        {
            ...
            printf("Group %s Group Compatibility Mode" \
        ...
        printf("%s %d off p_gp->mld_group_timer = %p\n",__func__,__LINE__,p_gp->mld_group_timer);
        mld_thread_timer_msec_on( &p_gp->mld_group_timer,imp_group_clean_timer, p_gp, TIMER_GMI*1000);
        printf("%s %d on p_gp->mld_group_timer = %p\n",__func__,__LINE__,p_gp->mld_group_timer);
...
#if 0
            THREAD_TIMER_MSEC_ON(master, NULL,
                                 NULL,
                                 NULL, 10);
#endif
    ...
    if (p_gp == NULL)
    {
        printf("imp_group_create failed");
        return;
    }
    mld_thread_off(&p_gp->mld_group_timer);
    mld_thread_timer_msec_on( &p_gp->mld_group_timer,imp_group_clean_timer, p_gp, TIMER_GMI*1000);
...
static int imp_group_clean_timer(struct thread *t)
{
    ...
    //mld_thread_off(p_gp->mld_group_timer);
    if(p_gp != NULL && p_gp->mld_group_timer != NULL)
    {
        mld_thread_off(&p_gp->mld_group_timer);
...
int mcast_to_ex_hander(imp_interface *p_if, pi_addr *p_ga,
                       pa_list* p_src_list, im_version version)
{
    imp_group *p_gp = NULL;
    pa_list *p_node = NULL;;
    printf("%s %d p_if->if_name = %s\n",__func__,__LINE__,p_if->if_name);
    p_gp = imp_group_find(p_if, p_ga);
    if (p_gp != NULL)
    {
                         ...
                         pa_list* p_src_list, im_version version)
{
    ...
    if (p_gp != NULL)
    {
        ...
        if (p_gp->version < version)
        {
            ...
            printf("%s's Group Compatibility Mode is IGMPv1.\n",
                   imp_pi_ntoa(p_ga));
            return;
        }
    ...
    printf("Can't find group %s, will create it\n", imp_pi_ntoa(p_ga));
