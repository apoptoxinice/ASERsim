...
#define ADDRF_IPV6      2   /* family  1: ipv4  2: ipv6 */
#define ADDRT_IPV6      0   /* type */

#define PIM_IP_VERSION 4

#define SINGLE_ADDR_MSKLEN_IPV6       128 /* the single address mask length for ipv6 */
...
#define GET_ESADDR(esa, cp)  /* XXX: hard coding */  \
        do {                                    \
            ...
            (esa)->flags       = *(cp)++;       \
            (esa)->masklen     = *(cp)++;       \
            memcpy(&(esa)->src_addr, (cp), sizeof(VOS_IPV6_ADDR)); \
...
#define GET_EGADDR(ega, cp) /* XXX: hard coding */ \
        do {                                    \
            ...
            (ega)->flags   = *(cp)++;       \
            (ega)->mask_len = *(cp)++;       \
            memcpy(&(ega)->grp_addr, (cp), sizeof(VOS_IPV6_ADDR)); \
            (cp) += sizeof(VOS_IPV6_ADDR);    \
        } while(0)
...
#define PUT_EGADDR(addr, masklen, reserved, cp) \
         do {                                    \
             ...
             *(cp)++ = (masklen);                \
             for (i = 0; i < sizeof(VOS_IPV6_ADDR); i++, (cp)++) \
                 *(cp) = maskaddr.u8_addr[i] & (addr).u8_addr[i]; \
         } while(0)
...
#define GET_EUADDR(eua, cp)  /* XXX hard conding */    \
        do {                                    \
         ...
         memcpy(&(eua)->unicast_addr, (cp), sizeof(VOS_IPV6_ADDR)); \
         (cp) += sizeof(VOS_IPV6_ADDR);   \
        } while(0)
...
#define PUT_EUADDR(addr, cp) \
         do {                                    \
             *(cp)++ = ADDRF_IPV6; /* family */  \
             *(cp)++ = ADDRT_IPV6; /* type   */  \
             memcpy((cp), &(addr), sizeof(VOS_IPV6_ADDR)); \
             (cp) += sizeof(VOS_IPV6_ADDR); \
         } while(0)
...
#define IPV6_IS_UNSPECIFIC_ADDRESS(pAddress,bl)  \
{\
    if ( ( (pAddress)->u32_addr[0] == 0 ) && ( (pAddress)->u32_addr[1] == 0 ) \
        && ( (pAddress)->u32_addr[2] == 0 ) && ( (pAddress)->u32_addr[3] == 0 ) )\
       (bl) = TRUE;\
    else\
       (bl) = FALSE;\
}
typedef struct
{
    ...
    uint8_t byAddrFamily; //IPv4 or IPv6
} PIMSM_IP_ADDR_T;
#if 0
typedef union
{
...
#define PIMSM_ADDR_VALID_LENGTH(pAddr) ((pAddr)->byAddrFamily == ADDRF_IPV6 ? PIMSM_ADDR_LENGTH_IPV6 : PIMSM_ADDR_LENGTH_IPV4)
//////////////////////////////////////////////////////////////////////////////////
struct pimsm_rpf_entry
{
    struct pimsm_rpf_entry *next;
    struct pimsm_rpf_entry *prev;
    VOS_IPV6_ADDR stDest;
    VOS_IPV6_ADDR stRpfNbr;
    uint32_t rpf_if;
    uint32_t preference;
    uint32_t metric;
};
...
typedef struct pimsm_global_t
{
    ...
    VOS_IPV6_ADDR stAllPimRouters;   /* All_PIM_ROUTERS */
    VOS_IPV6_ADDR stSockAddr6d;       /* for (*,*,RP) */
...
typedef struct stu_pimsm_assert_metric
{
    ...
    VOS_IPV6_ADDR   stIpv6Addr;     /*/ipv6地址*/
} PIM_ASSERT_METRIC;
...
typedef struct stu_pimsm_iif
{
    ...
    VOS_IPV6_ADDR       stUpstreamNbr; /* upstream neighbor address */
...
typedef struct stu_pimsm_grpentry
{
    ...
    VOS_IPV6_ADDR                  stRp;       /* The IPv6 address of the RP */
...
typedef struct stu_pimsm_mrtentry
{
    ...
    VOS_IPV6_ADDR   stPmbrAddr;  /* The PMBR address (for interop) */
