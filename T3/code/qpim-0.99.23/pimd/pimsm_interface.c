...
int pimsm_DRChoose(struct pimsm_interface_entry *pimsm_ifp, VOS_IPV6_ADDR *pstDrAddr);
struct pimsm_interface_member *pimsm_FindIfMember(struct pimsm_interface_entry *pimsm_ifp,VOS_IPV6_ADDR *src_addr,VOS_IPV6_ADDR *pstGrpAddr,PIMSM_RECORD_TYPE_T emType);
...
int pimsm_zebra_if_address_add(int command, struct zclient *zclient,zebra_size_t length)
{
    ...
    if ( IN6_IS_ADDR_LINKLOCAL(&p->u.prefix6))
        UNSET_FLAG (c->flags, ZEBRA_IFA_SECONDARY);
    else
    ...
    // if (PIM_DEBUG_ZEBRA)
    {
        ...
        PIM_DEBUG("%s: %s connected IPv6 address %s flags %u %s.\n",
                  __PRETTY_FUNCTION__,
    ...
    if (!CHECK_FLAG(c->flags, ZEBRA_IFA_SECONDARY))
    {
        ...
        struct prefix_ipv6 *p_tmp = NULL;
        struct in6_addr primary_addr;
        p_tmp = pimsm_find_primary_addr(c->ifp);
        memcpy(&primary_addr, &p_tmp->prefix, sizeof(struct in6_addr));

        if (memcmp(&primary_addr, &p->u.prefix6, sizeof(struct in6_addr)) != 0)
        {
            ...
            prefix2str(p, buf, BUFSIZ);
            pim_inet6_dump("<old?>", primary_addr, old, sizeof(old));
    ...
    PIM_DEBUG("will call pimsm_if_addr_add.\n");
    pimsm_if_addr_add(c);

    ifindex = c->ifp->ifindex;

    imp_if = imp_interface_find_by_ifindex(ifindex);
    if(imp_if != NULL)
    {
        imp_if->if_index = ifindex;

        struct in6_addr ifaddr;
        memset(&ifaddr,0,sizeof(struct in6_addr));
        memcpy(&ifaddr, &(c->address->u.prefix6),sizeof(struct in6_addr));
        printf("\n---------%s------------\n",inet6_ntoa(ifaddr));

        if(ip_isLinklocalAddress((VOS_IPV6_ADDR *)&(c->address->u.prefix6)))
        {
            memcpy(&(imp_if->if_addr.v6.sin6_addr),&(c->address->u.prefix6),sizeof(struct in6_addr));
            pim6_if_addr_add(c);
        }
    }
    ...
    /* IPv6 Address */
    memcpy(&addr, &p->u.prefix6, sizeof(VOS_IPV6_ADDR));
    prefixlen = p->prefixlen;

    addr_type = pim_addr_type(&addr);
    if (( 0 == (addr_type & PIMSM_IP_ADDR_LOOPBACK)) &&
            (0 == (addr_type & PIMSM_IP_ADDR_ANY)) &&
            (0 == (addr_type & PIMSM_IP_ADDR_MULTICAST)))
    {
#if 0
        memcpy(&pimsm_ifp->primary_address, &addr, sizeof(VOS_IPV6_ADDR));
...
int pimsm_zebra_if_address_del(int command, struct zclient *client,zebra_size_t length)
{
    ...
    */
    ...
    if (PIM_DEBUG_ZEBRA)
    {
        char buf[BUFSIZ];
        prefix2str(p, buf, BUFSIZ);
        zlog_debug("%s: %s disconnected IPv6 address %s flags %u %s",
                   __PRETTY_FUNCTION__,
                   c->ifp->name, buf, c->flags,
                   CHECK_FLAG(c->flags, ZEBRA_IFA_SECONDARY) ? "secondary" : "primary");
...
{
    struct pimsm_prefix_ipv6_list *pstAddrPrefixToFree;
    struct pimsm_prefix_ipv6_list *pstAddrPrefix;
    ...
    if (NULL != g_group_hash)
    {
        ...
        if (NULL != mrt_entry)
        {
            ...
            {
                memset(&mrt_entry->upstream_if.up_stream_nbr, 0x00, sizeof(VOS_IPV6_ADDR));
            }
        }
...
uint8_t pimsm_IsLocalAddress(VOS_IPV6_ADDR *pstAddr)
{
    struct pimsm_interface_entry *pimsm_ifp = NULL;
    ...
    while (NULL != pimsm_ifp)
    {
        ...
        for (pstScdAddr = pimsm_ifp->pstSecondaryAddr; pstScdAddr; pstScdAddr = pstScdAddr->next)
        {
            if (0 == ip_AddressCompare(pstAddr, &pstScdAddr->addr))
                return TRUE;
...
{
    struct pimsm_prefix_ipv6_list *pstScndaryAddrList = NULL;
    struct pimsm_neighbor_entry *pimsm_neigh = NULL;
    struct pimsm_interface_entry *pimsm_ifp = NULL;
...
uint32_t pimsm_DirectIfSearch(VOS_IPV6_ADDR *pstAddr)
{
    struct pimsm_interface_entry *pimsm_ifp = NULL;
    struct pimsm_prefix_ipv6_list *pstScdAddr;
    ...
    while (NULL != pimsm_ifp)
    {
        ...
        if(!ip_isAnyAddress(&pimsm_ifp->primary_address) && (pimsm_ifp->prefixlen != 0))
        {
        ...
        for (pstScdAddr = pimsm_ifp->pstSecondaryAddr; pstScdAddr; pstScdAddr = pstScdAddr->next)
        {
            if(!ip_isAnyAddress(&pstScdAddr->addr) && (pstScdAddr->prefixlen != 0))
            {
                if (pim_MatchPrefix(pstAddr, &pstScdAddr->addr, pstScdAddr->prefixlen))
                {
                    return (pimsm_ifp->ifindex);
                }
            }
...
uint8_t pimsm_DRisBetter(struct pimsm_neighbor_entry *pstNbrA,struct pimsm_neighbor_entry *pstNbrB,struct pimsm_interface_entry *pimsm_ifp)
{
    ...
    if (pimsm_AllNbrPriorityPresent(pimsm_ifp))
    {
        ...
        {
            iBigOrNot = memcmp(&(pstNbrA->source_addr), &(pstNbrB->source_addr), sizeof(VOS_IPV6_ADDR));
            if(iBigOrNot > 0)
            {
                return TRUE;
            }
...
int pimsm_DRChoose(struct pimsm_interface_entry *pimsm_ifp, VOS_IPV6_ADDR *pstDrAddr)
{
    ...
    memcpy(pstDrAddr, &stDR.source_addr, sizeof(VOS_IPV6_ADDR));
...
struct pimsm_interface_member *pimsm_CreateIfMember(struct pimsm_interface_entry *pimsm_ifp, VOS_IPV6_ADDR *src_addr, VOS_IPV6_ADDR *pstGrpAddr,PIMSM_RECORD_TYPE_T emType)
{
    ...
    if (NULL == pstIfMember)
    {
        ...
        memcpy(&pstIfMember->grp_addr, pstGrpAddr, sizeof(VOS_IPV6_ADDR));
        ...
        if (((PIMSM_IF_INCLUDE == emType) || (PIMSM_IF_EXCLUDE == emType)) && (NULL != src_addr))
        {
            ...
            memcpy(&pstSrcNode->addr, src_addr, sizeof(VOS_IPV6_ADDR));
