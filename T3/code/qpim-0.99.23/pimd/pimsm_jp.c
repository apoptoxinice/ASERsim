...
{
    VOS_IPV6_ADDR stRpfNbrToSrc;
    VOS_IPV6_ADDR stRpfNbrToRp;
...
uint8_t pimsm_RPTJoinDesired(VOS_IPV6_ADDR *pstGrpAddr)
{
...
int pimsm_SendJoin(struct pimsm_mrt_entry *mrt_entry,VOS_IPV6_ADDR *pstUpstreamNbrAddr)
{
    ...
    VOS_IPV6_ADDR stSrcGrpRptNbrAddr;
    VOS_IPV6_ADDR stStarGrpNbrAddr;
    VOS_IPV6_ADDR stSrcGrpNbrAddr;
    ...
    memcpy(&grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
    ...
    {
        PIM_DEBUG("pimsm_SendJoin: Need to send a Join(*,G) message to %s with G=%s, Prune(S,G,rpt) exist or not.\n",
        ...
        memset(&src_addr, 0, sizeof(VOS_IPV6_ADDR));
        memcpy(&src_addr, &mrt_entry->group->rp_addr, sizeof(VOS_IPV6_ADDR));
        ...
        while(NULL != src_grp_entry)
        {
            if(PIMSM_MRT_TYPE_SG == src_grp_entry->type && pimsm_SptBit(&src_grp_entry->source->address, &src_grp_entry->group->address))
            {
                ...
                if(ip_AddressCompare(&stStarGrpNbrAddr, &stSrcGrpNbrAddr) != 0)
                {
                    /* add Prune(S,G,rpt) to compound message */
                    wPruneSrcNum++;
                    memcpy(&src_addr, &src_grp_entry->source->address, sizeof(VOS_IPV6_ADDR));
            ...
            else if(PIMSM_MRT_TYPE_SGRPT == src_grp_entry->type)
            {
                if(NULL == pimsm_SearchMrtEntry(&src_grp_entry->source->address, &src_grp_entry->group->address, PIMSM_MRT_TYPE_SG))
                {
                    ...
                    else
                    {
                        ...
                        if(ip_AddressCompare(&stStarGrpNbrAddr, &stSrcGrpRptNbrAddr) != 0)
                        {
                            /* add Prune(S,G,rpt) to compound message */
                            wPruneSrcNum++;
                            memcpy(&src_addr, &src_grp_entry->source->address, sizeof(VOS_IPV6_ADDR));
            ...
            else
            {
                ...
                if(NULL != pstSrcGrpRptEntry)
                {
                    if (NULL == pimsm_InheritedOlist(pstSrcGrpRptEntry, FALSE))
                    {
                        /* add Prune(S,G,rpt) to compound message */
                        wPruneSrcNum++;
                        memcpy(&src_addr, &pstSrcGrpRptEntry->source->address, sizeof(VOS_IPV6_ADDR));
                        PUT_ESADDR(src_addr, SINGLE_SRC_MSK6LEN, (PIM_USADDR_S|PIM_USADDR_RP), pucBuf);
                    }
                    else
                    {
    ...
    else if(PIMSM_MRT_TYPE_SG == mrt_entry->type)
    {
        ...
        memcpy(&src_addr, &mrt_entry->source->address, sizeof(VOS_IPV6_ADDR));
    ...
    pimsm_SendPimPacket(g_stPimsm.pucSendBuf,
                        mrt_entry->upstream_if.ifindex,
                        NULL,
                        (VOS_IPV6_ADDR *)&qpim_all_pim6_routers_addr,
                        PIM_TYPE_JOIN_PRUNE,
                        pim_msg_size);
...
static int pimsm_UpstreamIfStateChange(struct pimsm_mrt_entry *mrt_entry,PIMSM_UPSTREAM_STATE_T emStateFrom,PIMSM_UPSTREAM_STATE_T emStateTo)
{
    ...
    memset(&src_addr, 0, sizeof(VOS_IPV6_ADDR));
    memset(&grp_addr, 0, sizeof(VOS_IPV6_ADDR));
    memcpy(&grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
...
static int pimsm_StarGrpUpstreamEventProc(struct pimsm_mrt_entry *mrt_entry,uint8_t byEventType,struct pimsm_updown_state_machine_event_para *event_para)
{
    ...
    memset(&stJoinPara->src_addr, 0, sizeof(VOS_IPV6_ADDR));
    memcpy(&stJoinPara->grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
    ...
    case PimsmUpstreamStateJoined:
        ...
        else if(PimsmRpfaStarGrpChangesNotAssertEvent == byEventType)
        {
            ...
            PIM_DEBUG("%s()%d will call pimsm_sendJoin here, up stream nbr:%s.\n", __func__, __LINE__, pim_InetFmt(&event_para->new_upstream_nbr_addr));
...
static int pimsm_SrcGrpUpstreamEventProc(struct pimsm_mrt_entry *mrt_entry,uint8_t byEventType,struct pimsm_updown_state_machine_event_para *event_para)
{
    ...
    memcpy(&stJoinPara->src_addr, &mrt_entry->source->address, sizeof(VOS_IPV6_ADDR));
    memcpy(&stJoinPara->grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
...
static int pimsm_StarGrpDownstreamEventProc(struct pimsm_mrt_entry *mrt_entry,struct pimsm_updownstream_if_entry *downstream_if,uint8_t byEventType,struct pimsm_updown_state_machine_event_para *event_para)
{
    ...
    memset(&prune_pending_para->src_addr, 0, sizeof(VOS_IPV6_ADDR));
    memcpy(&prune_pending_para->grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
    prune_pending_para->type = PIMSM_MRT_TYPE_WC;
    prune_pending_para->ifindex = downstream_if->ifindex;

    memset(&expiry_para->src_addr, 0, sizeof(VOS_IPV6_ADDR));
    memcpy(&expiry_para->grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
...
static int pimsm_SrcGrpDownstreamEventProc(struct pimsm_mrt_entry *mrt_entry,struct pimsm_updownstream_if_entry *downstream_if,uint8_t byEventType,struct pimsm_updown_state_machine_event_para *event_para)
{
    ...
    memcpy(&prune_pending_para->src_addr, &mrt_entry->source->address, sizeof(VOS_IPV6_ADDR));
    memcpy(&prune_pending_para->grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
    ...
    memcpy(&expiry_para->src_addr, &mrt_entry->source->address, sizeof(VOS_IPV6_ADDR));
    memcpy(&expiry_para->grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
