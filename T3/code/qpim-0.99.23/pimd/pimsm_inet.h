...
#define PIMSM_IP_ADDR_ANY 0x0001
#define PIMSM_IP_ADDR_UNSPEC PIMSM_IP_ADDR_ANY
#define PIMSM_IP_ADDR_UNICAST 0x0002
#define PIMSM_IP_ADDR_MULTICAST 0x0004
#define PIMSM_IP_ADDR_LOOPBACK 0x0010
#define PIMSM_IP_ADDR_LINKLOCAL 0x0020
#define PIMSM_IP_ADDR_SITELOCAL 0x0040

#define GET_IP_VER(v) (((v) & 0xf0) >> 4)
#define SET_IP_VER(p,v) ((p) = ((v) << 4))

extern uint8_t ip_isAnyAddress(VOS_IPV6_ADDR  *pAddress);
extern uint8_t ip_isLinklocalAddress(VOS_IPV6_ADDR  *pAddress);
extern int 	ip_AddressCompare(VOS_IPV6_ADDR  *pAddress1, VOS_IPV6_ADDR  *pAddress2);
extern int ip_printAddress(VOS_IPV6_ADDR  *pAddress, char *buf, int buf_len);
extern int pimsm_BitMapIndex(uint32_t data, int index[32], int *count);
extern int ip_GetPrefixFromAddr(VOS_IPV6_ADDR *pAddr, int iPrefixLen);
extern uint32_t pim_addr_type(VOS_IPV6_ADDR *pAddr);
extern uint8_t pim_MatchPrefix(VOS_IPV6_ADDR *ar1, VOS_IPV6_ADDR *ar2, uint16_t prefixlen);
extern uint8_t pimsm_IsSsmRangeAddr(VOS_IPV6_ADDR *pstAddr);
