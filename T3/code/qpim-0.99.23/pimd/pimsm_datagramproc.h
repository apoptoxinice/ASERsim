...
typedef struct stu_pim_ip_header
{
    ...
    VOS_IPV6_ADDR 	ip_src;		/* IP address of source			*/
    VOS_IPV6_ADDR	ip_dst;		/* IP address of destination		*/
...
struct pimsm_encoded_unicast_addr
{
    ...
    VOS_IPV6_ADDR unicast_addr; /* Unicast Address */
...
struct pimsm_encoded_grp_addr
{
    ...
    VOS_IPV6_ADDR grp_addr; /* group multicast address */
} __attribute__((packed));
...
struct pimsm_encoded_src_addr
{
    ...
    uint8_t flags;   /* S、W、R */
    uint8_t mask_len; /* Mssk Len */
    VOS_IPV6_ADDR src_addr; /* source address */
} __attribute__((packed));
...
#define PIMSM_OPTION_TYPE_ADDRESS_LIST 24
