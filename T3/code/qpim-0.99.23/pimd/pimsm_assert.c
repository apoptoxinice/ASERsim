...
struct pimsm_assert_metric pimsm_SptAssertMetric(VOS_IPV6_ADDR *src_addr, uint32_t ifindex);
struct pimsm_assert_metric pimsm_RptAssertMetric(VOS_IPV6_ADDR *pstGrpAddr, uint32_t ifindex);
...
int pimsm_MetricCompare(struct pimsm_assert_metric *pstMetric1, struct pimsm_assert_metric *pstMetric2)
{
    ...
    if (0 > memcmp((const void *)(&(pstMetric1->address)), (const void *)(&(pstMetric2->address)), sizeof(VOS_IPV6_ADDR)))
    {
        return -1;
    }
    else
...
struct pimsm_assert_metric pimsm_SptAssertMetric(VOS_IPV6_ADDR *src_addr, uint32_t ifindex)
{
    ...
    */
    ...
    memcpy(&stAssertMetric.address, &pimsm_ifp->primary_address, sizeof(VOS_IPV6_ADDR));
...
struct pimsm_assert_metric pimsm_InfiniteAssertMetric(void)
{
    ...
    */
    ...
    memset(&stAssertMetric.address, 0, sizeof(VOS_IPV6_ADDR));
...
static int pimsm_SendAssert(struct pimsm_mrt_entry *mrt_entry,uint32_t ifindex,uint8_t blAssertCancel)
{
    ...
    memcpy(&(stIpAddr), &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
            ...
            (PIMSM_MRT_TYPE_SGRPT == mrt_entry->type))
    {
        ...
        memset(&stIpAddr, 0, sizeof(VOS_IPV6_ADDR));
        PUT_EUADDR(stIpAddr, pucBuf);
    }
    else
    {
        /* metric和preference向S方向 */
        memcpy(&stIpAddr, &mrt_entry->source->address, sizeof(VOS_IPV6_ADDR));
        PUT_EUADDR(stIpAddr, pucBuf);
    }
...
static int pimsm_AssertActionSA1(struct pimsm_mrt_entry *mrt_entry,struct pimsm_assert_if *pimsmassert_if,struct pimsm_assert_state_machine_event_para *event_para)
{
    ...
    memcpy(&assert_para->src_addr, &mrt_entry->source->address, sizeof(VOS_IPV6_ADDR));
    memcpy(&assert_para->grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
...
static int pimsm_AssertActionGA1(struct pimsm_mrt_entry *mrt_entry,struct pimsm_assert_if *pimsmassert_if,struct pimsm_assert_state_machine_event_para *event_para)
{
    ...
    memset(&assert_para->src_addr, 0, sizeof(VOS_IPV6_ADDR));
    memcpy(&assert_para->grp_addr, &mrt_entry->group->address, sizeof(VOS_IPV6_ADDR));
