...
int pimsm_TimeroutKeepAlive(struct thread *t)
{
    ...
    init_sin6(&sgreq.src);
    init_sin6(&sgreq.grp);
    memcpy(&sgreq.src.sin6_addr, &pstKeepAlivePara->src_addr, sizeof(struct in6_addr));
    memcpy(&sgreq.grp.sin6_addr, &pstKeepAlivePara->grp_addr, sizeof(struct in6_addr));

    pim_inet6_dump("<group?>", sgreq.grp.sin6_addr, group_str, sizeof(group_str));
    pim_inet6_dump("<source?>", sgreq.src.sin6_addr, source_str, sizeof(source_str));

    if (ioctl(qpim_mroute_socket_fd, SIOCGETSGCNT_IN6, &sgreq))
    {
        int e = errno;
    ...
    if (sgreq.pktcnt != mrt_entry->packet_match_count)
    {
        ...
        //reset timer
        PIM_DEBUG("reset pimsm_KeepAliveTimerStart, %d, sgreq pktcnt:%lu, mrt pktcnt:%u.\n", PIMSM_TIMER_KEEPALIVE_PERIOD, sgreq.pktcnt, mrt_entry->packet_match_count);
        pimsm_KeepAliveTimerStart(mrt_entry, PIMSM_TIMER_KEEPALIVE_PERIOD);
    }
    else
    {
        PIM_DEBUG("Entry(%s, %s) timer out, delete, sgreq pktcnt:%lu, mrt pktcnt:%u!!!\n",
                  pim_InetFmt(&pstKeepAlivePara->src_addr),
                  pim_InetFmt(&pstKeepAlivePara->grp_addr), sgreq.pktcnt, mrt_entry->packet_match_count);
...
int pimsm_AssertEvent5Func(PIM_MRTENTRY *mrt_entry, PIM_INTERFACE *pimsm_ifp, VOS_IPV6_ADDR *pstTheNbrPriAddr)
{
    int           ret;
    int           iRetval2;
    PIM_OIF        *pstAssertLoserOif = NULL;
    int             iEqOrNot;
    VOS_IPV6_ADDR   stRpfStrokePre;
    VOS_IPV6_ADDR   stRpfStrokeCur;
...
int pimsm_AssertEvent5(PIM_INTERFACE *pimsm_ifp, VOS_IPV6_ADDR *pstTheNbrPriAddr)
{
