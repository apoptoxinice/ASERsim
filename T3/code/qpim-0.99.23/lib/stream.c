...
#ifdef HAVE_IPV6
#if  1/*sangmeng add fro pim6 20190606*/
/* Put long word to the stream. */
int
stream_put_in6_addr (struct stream *s, struct in6_addr *addr)
{
    STREAM_VERIFY_SANE(s);

    if (STREAM_WRITEABLE (s) < sizeof (struct in6_addr))
    {
        STREAM_BOUND_WARN (s, "put");
        return 0;
    }

    memcpy (s->data + s->endp, addr, sizeof (struct in6_addr));
    s->endp += sizeof (struct in6_addr);

    return sizeof (struct in6_addr);
}

/* Get next  from the stream. */
struct in6_addr stream_get_ipv6 (struct stream *s)
{
    ...
    if (STREAM_READABLE (s) < sizeof(struct in6_addr))
    {
        STREAM_BOUND_WARN (s, "get ipv6");
        return addr;
    }

    memcpy (&addr, s->data + s->getp, sizeof(struct in6_addr));
    s->getp += sizeof(struct in6_addr);
