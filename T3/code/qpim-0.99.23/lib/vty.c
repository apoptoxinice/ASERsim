...
{
    char cMot[]="SpaceRouter6000(verison 2.0.20191209)\nCopyright (c) 2013-2019, All Right Reserved.\nTsinghua University.\n";
    if (host.motdfile)
    {
        FILE *f;
        char buf[4096];
...
vty_stop_input (struct vty *vty)
{
    ...
    case DHCP_NODE:
    case IPV4_ACCESS_NODE: //sangmeng add for ipv4 filter
    case IPV6_ACCESS_NODE: //sangmeng add for ipv4 filter
    case RMAP_NODE:
    case OSPF_NODE:
    case OSPF6_NODE:
    case ISIS_NODE:
                 ...
                 char *config_default_dir)
{
    ...
    else
    {
        ...
        if(!strcmp("/usr/local/etc/bgpd.conf",config_default_dir))
            bgpd_flag = 1;
