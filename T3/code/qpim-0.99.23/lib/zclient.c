...
zclient_stop (struct zclient *zclient)
{
    ...
    /* Callback to optionally notify application about broken
       connection */
...
int zapi_4over6_route (u_char cmd, struct zclient *zclient, struct prefix_ipv4 *p, struct zapi_ipv6 *api)
{
    int i;
    int psize;
    struct stream *s;
