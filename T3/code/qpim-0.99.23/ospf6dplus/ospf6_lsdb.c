...
ospf6_lsdb_add_for_expire (struct ospf6_lsa *lsa, struct ospf6_lsdb *lsdb)
{
    ...
    zlog_debug("%s()%d will add lsa to lsdb.", __func__, __LINE__);
    ...
    {
        lsdb->count++;
        printf("1>>>>lsdb->count:%d\n", lsdb->count);

        if (lsdb->hook_add)
        {
            zlog_debug("%s()%d will call hook add.", __func__, __LINE__);
            (*lsdb->hook_add) (lsa);
        }
    }
    else
    {
        printf("2>>>>lsdb->count:%d\n", lsdb->count);
        if (OSPF6_LSA_IS_CHANGED (old, lsa))
        {
            if (lsdb->hook_remove)
            {
                zlog_debug("%s()%d will call hook remove.", __func__, __LINE__);
                (*lsdb->hook_remove) (old);
            }
            if (lsdb->hook_add)
            {
                   ...
                   struct ospf6_lsdb *lsdb)
{
    ...
    if (lsdb->table == NULL)
    {
        zlog_debug("%s()%d lsdb->table is null.", __func__, __LINE__);
        return NULL;
    }
    node = route_node_lookup (lsdb->table, (struct prefix *) &key);
