                         ...
                         struct ospf6_neighbor *on)
{
    ...
    case OSPF6_NEIGHBOR_EXCHANGE:
        ...
        if (!memcmp (dbdesc, &on->dbdesc_last, sizeof (struct ospf6_dbdesc)))
        {
            ...
            printf("^^^^%s()%d here will call dbdesc send.", __func__, __LINE__);
            on->thread_send_dbdesc =
...
ospf6_dbdesc_send (struct thread *thread)
{
    ...
    if (! CHECK_FLAG (on->dbdesc_bits, OSPF6_DBDESC_IBIT))
    {
                ...
                lsa = ospf6_lsdb_next (lsa))
        {
            ...
            printf("%s(0%d will copy lsa->header to p.\n", __func__, __LINE__);
...
int ospf6_dbdesc_send_newone_for_leaving_to_exchange (struct thread *thread)
{
    ...
    for (lsa = ospf6_lsdb_head (on->summary_list); lsa; lsa = ospf6_lsdb_next (lsa))
    {
        ...
        printf("%s()%d will add lsa to dbdesc_list.\n", __func__, __LINE__);
    ...
    printf("^^^^%s()%d here will call dbdesc send.\n", __func__, __LINE__);
...
int ospf6_dbdesc_send_newone (struct thread *thread)
{
    ...
    for (lsa = ospf6_lsdb_head (on->summary_list); lsa; lsa = ospf6_lsdb_next (lsa))
    {
        ...
        printf("%s()%d will call ospf6_lsdb_add, add lsa to dbdesc_list.\n", __func__, __LINE__);
        ospf6_lsdb_add (ospf6_lsa_copy (lsa), on->dbdesc_list);
        ospf6_lsdb_remove (lsa, on->summary_list);
        size += sizeof (struct ospf6_lsa_header);
    }
