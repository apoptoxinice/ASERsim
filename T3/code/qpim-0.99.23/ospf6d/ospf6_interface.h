...
//add 2020
#if 1
#define OSPF6_PBIDISABLED 0x00
#define OSPF6_PBIENABLED  0x01
#endif

#define MIN_HELLO_DETECTION_TIME 2
#define DEAD_INTERVAL_2  10
#define HELLO_ADVANCE_PACKET_DETECTION_TIME 20
/* Debug option */
...
struct ospf6_interface
{
...
#if 1 //add 2020
    /*partition boundary interface*/
    u_char interface_pbi_flag;
    /*Area id of other partitions below this interface*/
    u_int32_t neighbor_area_id;
    /*neigh prefix*/
    struct prefix neighbor_prefix;
    /* linklocal address of p2p neighbor I/F */
    struct in6_addr neighbor_linklocal_addr;
#endif
    ...
    /* add for ospf+*/
    /*
    * if the interface has a neighbor in the leaving state, the reference count +1
    * if it changed from leaving to down, the reference count -1;
    * if it from leaving to exchange, and exchange is completed, the reference count -1
    * when the reference count is greater than 0, began to save the other neighbor receives lsa header
    * when the reference is 0, and the new lsas list is not empty, emty the new lsas list
    * */
    uint16_t reference_count; /**< count of this interface in the leaving state*/
    struct ospf6_lsdb *new_lsas_list;/**< the new lsas header*/
};
