...
ospf6_router_lsa_show (struct vty *vty, struct ospf6_lsa *lsa)
{
            ...
            current += sizeof (struct ospf6_router_lsdesc))
    {
        ...
        else if (lsdesc->type == OSPF6_ROUTER_LSDESC_PBRP2P_LINK)
            snprintf (name, sizeof (name), "PBR Point-To-Point");
        else
...
#if 1
static char *
ospf6_zone_lsa_get_aa_id (struct ospf6_lsa *lsa, char *buf, int buflen,
                          int pos)
{
...
#endif
static int
ospf6_zone_lsa_show (struct vty *vty, struct ospf6_lsa *lsa)
{
    char *start, *end, *current;
    char buf[32];
    struct ospf6_zone_lsa *zone_lsa;
    ...
    vty_out(vty,"bits:%d%s", zone_lsa->bits, VNL);

    start = (char *) zone_lsa + sizeof (struct ospf6_zone_lsa);
    end = (char *) lsa->header + ntohs (lsa->header->length);
    for (current = start; current + sizeof (struct ospf6_zone_aaidesc) <= end;
            current += sizeof (struct ospf6_zone_aaidesc))
    {
        aaidesc = (struct ospf6_zone_aaidesc *) current;

        vty_out (vty, "    Attached Area ID: %s%s",
                 inet_ntop (AF_INET, &aaidesc->area_id,
                            buf, sizeof (buf)), VNL);
    }
    return 0;
...
#if 0
#ifdef OSPF6_DEBUG
        zlog_debug ("%s()%d add lsa:%s to oa->lsdb_bak", __func__, __LINE__, lsa->name);
#endif
        lsa_bak = ospf6_lsa_copy (lsa);
        lsa_bak->lsdb = oa->lsdb_bak;
        zlog_debug("%s()%d add lsa to lsdb_bak.", __func__, __LINE__);
...
int ospf6_router_lsa_originate (struct thread *thread)
{
    ...
    if (ospf6_is_router_abr (ospf6))
        ...
        UNSET_FLAG (router_lsa->bits, OSPF6_ROUTER_BIT_B);
#if 1//add 2020
    if (ospf6_is_router_pbr (ospf6))
    {
        //DG(">>>set router lsa B bit.\n");
        SET_FLAG (router_lsa->bits, OSPF6_ROUTER_BIT_B);
    }
    ...
    for (ALL_LIST_ELEMENTS (oa->if_list, node, nnode, oi))
    {
        ...
        if (oi->type == OSPF_IFTYPE_POINTOPOINT)
        {
            for (ALL_LIST_ELEMENTS_RO (oi->neighbor_list, j, on))
            {
...
#if 1 //add 2020
                if (on->neighbor_pbr_flag == 1)
                {
                    //DG("set type to OSPF6_ROUTER_LSDESC_PBRP2P_LINK.\n");
                    lsdesc->type = OSPF6_ROUTER_LSDESC_PBRP2P_LINK;
                }
                else
#endif
                    lsdesc->type = OSPF6_ROUTER_LSDESC_POINTTOPOINT;
                ...
                if (on->neighbor_pbr_flag == 1)
                {
                    //DG("set neighbor interface id to neighbor_area_id.\n");
                    lsdesc->neighbor_interface_id = oi->neighbor_area_id;
                }
                else
#endif
                    lsdesc->neighbor_interface_id = htonl (on->ifindex);
...
#if 1//add 2020
    if (ospf6_is_router_pbr(oa->ospf6))
    {
        type = ntohs (OSPF6_LSTYPE_ROUTER);
        router = oa->ospf6->router_id;
        for (lsa = ospf6_lsdb_type_router_head (type, router, oa->lsdb); lsa;
                lsa = ospf6_lsdb_type_router_next (type, router, lsa))
        {
            DG("self originate router lsa, will process B-bit lsa.\n");
            ospf6_process_pbr_router_lsa(lsa, oa);
        }
    }
#endif
    return 0;
...
#ifdef OSPF6_DEBUG
    zlog_debug("%s()%d lsa:%s, age:%u", __func__, __LINE__, lsa->name, ospf6_lsa_age_current(lsa));
#endif
    /* Originate */
                                       ...
                                       int buflen, int pos)
{
    ...
    if (lsa)
    {
        ...
        do
        {
            ...
            {
                cnt++;
                current = start + cnt*OSPF6_PREFIX_SIZE(prefix);
            ...
            else
            {
                ...
                //DG(">>>>current:%p, length:%d, buf:%s\n", current, prefix->prefix_length ,buf);
                return (buf);
            }
        }
        while (current <= end);
...
int ospf6_extend_intra_prefix_lsa_originate_stub (struct thread *thread)
{
    ...
    ospf6_lsa_twod_del_node (oa, &twod);	//zlw 2016-4-7
    if (twod == NULL)
    {
    ...
    op->prefix_options = twod->operate;
...
ospf6_intra_prefix_lsa_originate_stub (struct thread *thread)
{
    ...
    //DG("Originate Intra-Area-Prefix-LSA for area %s's stub prefix.\n",oa->name);
    /* prepare buffer */
    memset (buffer, 0, sizeof (buffer));
    lsa_header = (struct ospf6_lsa_header *) buffer;
    intra_prefix_lsa = (struct ospf6_intra_prefix_lsa *)
    ...
    for (ALL_LIST_ELEMENTS_RO (oa->if_list, i, oi))
    {
        ...
        for (ALL_LIST_ELEMENTS_RO (oi->neighbor_list, j, on))
            if (on->state == OSPF6_NEIGHBOR_FULL)
                full_count++;
#if 1 //add 2020
            else if ((on->neighbor_pbr_flag == 1) && (on->state == OSPF6_NEIGHBOR_LEAVING))
                leaving_flag = 1;
#endif
                ...
                route = ospf6_route_best_next (route))
        {
            ...
            DG ("1    include %s.\n", buf);
#endif
            if (leaving_flag == 1 &&(prefix_cmp(&oi->neighbor_prefix, &route->prefix) == 0))
                continue;
...
ospf6_intra_prefix_lsa_originate_transit (struct thread *thread)
{
            ...
            route = ospf6_route_best_next (route))
    {
        ...
        inet_ntop (AF_INET6, &route->prefix.u.prefix6, addr_str,
                   sizeof (addr_str));
        DG("prefix body:%s.\n", addr_str);
#endif
        op = OSPF6_PREFIX_NEXT (op);
...
#if 0
        char buf[128];
        inet_ntop(AF_INET6, &(route->prefix.u.prefix6), buf, sizeof(buf));
        DG("prefix:%s/%d, cost:%d, direct_connecth:%d.\n",buf, route->prefix.prefixlen, route->path.cost, direct_connect);
#endif
        if (direct_connect)
        {
            ...
            else
            {
                ...
                for (ALL_LIST_ELEMENTS_RO (oa->if_list, i, oi))
                {
                    if (oi->type != OSPF_IFTYPE_POINTOPOINT)
                        continue;
                    if (prefix_cmp(&oi->neighbor_prefix, &route->prefix) == 0)
                    {
                        route->nexthop[0].ifindex = oi->interface->ifindex;
                        memcpy(&route->nexthop[0].address, &oi->neighbor_linklocal_addr, sizeof(struct in6_addr));
                        break;
                    }
                }
#endif
...
#if 1//add 2020
static void ospf6_zone_prefix_lsa_add (struct ospf6_lsa *lsa)
{
    struct ospf6 *o;
    struct ospf6_intra_prefix_lsa *intra_prefix_lsa;
    struct prefix ls_prefix;
    ...
    o = OSPF6_PROCESS(lsa->lsdb->data);

    ospf6_linkstate_prefix (lsa->header->adv_router, lsa->header->id, &ls_prefix);

    ls_entry = ospf6_route_lookup (&ls_prefix, o->spf_table_zone);
    if (ls_entry == NULL)
    {
        if (IS_OSPF6_DEBUG_EXAMIN (INTRA_PREFIX))
        {
    ...
    if (lsa->header->id == o->self_area_id)
    {
        direct_connect = 1;
        return;
    }
    ...
    route->path.origin.id = lsa->header->id;
    route->path.origin.adv_router = lsa->header->adv_router;
    //FIXME: how to set prefix options
    route->path.prefix_options = 0;
    route->path.area_id = o->self_area_id;
    route->path.type = OSPF6_PATH_TYPE_PARTITION;
    route->path.metric_type = 1;
    //FIXME: how to set path.cost
    route->path.cost = ls_entry->path.cost;
    ...
    DG (">>>add include %s.\n", buf);
#endif
    ospf6_route_add (route, o->route_table_zone);
}

void ospf6_zone_route_calculation (struct ospf6 *o)
{
    struct ospf6_route *route;
    u_int16_t type;
    struct ospf6_lsa *lsa;
    ...
    hook_add = o->route_table_zone->hook_add;
    hook_remove = o->route_table_zone->hook_remove;
    o->route_table_zone->hook_add = NULL;
    o->route_table_zone->hook_remove = NULL;

    for (route = ospf6_route_head (o->route_table_zone); route;
            route = ospf6_route_next (route))
        route->flag = OSPF6_ROUTE_REMOVE;

    //FIXME:
    type = htons (OSPF6_LSTYPE_ZONE);
    for (lsa = ospf6_lsdb_type_head (type, o->lsdb_zone); lsa;
            lsa = ospf6_lsdb_type_next (type, lsa))
        ospf6_zone_prefix_lsa_add (lsa);

    o->route_table_zone->hook_add = hook_add;
    o->route_table_zone->hook_remove = hook_remove;
            ...
            route = ospf6_route_next (route))
    {
        ...
        if (CHECK_FLAG (route->flag, OSPF6_ROUTE_REMOVE))
            ospf6_route_remove (route, o->route_table_zone);
        else if (CHECK_FLAG (route->flag, OSPF6_ROUTE_ADD) ||
                 CHECK_FLAG (route->flag, OSPF6_ROUTE_CHANGE))
        {
            if (hook_add)
...
#if 1//add 2020
struct ospf6_lsa_handler zone_handler =
{
    OSPF6_LSTYPE_ZONE,
    "Zone",
    "Zone",
    ospf6_zone_lsa_show,
    ospf6_zone_lsa_get_aa_id
};
...
ospf6_intra_init (void)
{
    ...
    ospf6_install_lsa_handler (&zone_handler);
}
