                   ...
                   struct ospf6_route_table *result_table)
{
    ...
    if (v->parent)
    {
        //DG("v->parent, add v:%s to child list.\n", v->name);
...
#if 0
    char idbuf[16];
    inet_ntop (AF_INET, &id, idbuf, sizeof (idbuf));
    DG("area id:%s.\n", idbuf);
#endif
...
static void ospf6_spf_calculation_zone (u_int32_t router_id, struct ospf6_route_table *result_table, struct ospf6 *o)
{
    ...
    /* Install the calculating router itself as the root of the SPF tree */
    /* construct root vertex */
    /*MARK:adv_router should be 0, id should be area id*/
#if 0
    char buf[16];
    memset(buf, 0x00, sizeof(buf));
    ...
    while (candidate_list->size)
    {
        ...
        /* installing may result in merging or rejecting of the vertex */
        /*for zone resul_table----->ospf6->spf_route_table*/
        if (ospf6_spf_install (v, result_table) < 0)
            continue;
