...
//add 20200310
#if 1
    //DG("name:%s oi->type:%d, ipf:%d, area_id:%d id:%d npf:%d.\n", oi->interface->name, oi->type, oi->interface_pbi_flag, oh->area_id, oi->area->area_id, on->neighbor_pbr_flag);
    if ((oi->type == OSPF_IFTYPE_POINTOPOINT)
            && (oi->interface_pbi_flag == OSPF6_PBIENABLED)
            && (oh->area_id != oi->area->area_id)
            && (on->neighbor_pbr_flag == 0))
        on->neighbor_pbr_flag = 1;
#endif
                          ...
                          struct ospf6_neighbor *on)
{
...
#ifdef OSPF6_DEBUG_SEQ_MISMATCH
            ...
            p += sizeof (struct ospf6_lsa_header))
    {
        ...
        case OSPF6_SCOPE_AS:
#if 1 //add 2020
            if (ntohs(his->header->type) == OSPF6_LSTYPE_ZONE)
                lsdb = on->ospf6_if->area->ospf6->lsdb_zone;
            else
#endif
        ...
        {
#ifdef OSPF6_DEBUG
            zlog_debug("%s()%d The interface has %d neighbor in the leaving state, save lsa header to new lsas list,ifname:%s",
                       __func__, __LINE__, on->ospf6_if->reference_count,on->ospf6_if->interface->name);
#endif
                  ...
                  struct ospf6_interface *oi, struct ospf6_header *oh)
{
            ...
            p += sizeof (struct ospf6_lsreq_entry))
    {
        ...
        case OSPF6_SCOPE_AS:
#if 1 //add 2020
            if (ntohs(e->type) == OSPF6_LSTYPE_ZONE)
                lsdb = on->ospf6_if->area->ospf6->lsdb_zone;
            else
#endif
                lsdb = on->ospf6_if->area->ospf6->lsdb;
        ...
        //printf(">>>%s()%d copy lsa:%s to lsupdate_list, count:%d.\n", __func__, __LINE__, lsa->name, on->lsupdate_list->count);
    }
    ...
    //printf(">>>>%s()%d will call ospf6_lsupdate_send_neighbor.\n", __func__, __LINE__);
...
#ifdef TWOD							//add haozhiqiang 2015-12-17
...
static unsigned ospf6_lsaseq_examin
(
...
static unsigned
ospf6_abr_lsa_examin (struct ospf6_lsa_header *lsah)
{
    u_int16_t lsatype;
...
static unsigned ospf6_abr_lsaseq_examin
(
    struct ospf6_lsa_header *lsah, /* start of buffered data */
    size_t length,
    const u_char headeronly,
...
)
{
    while (length)
    {
        ...
        if (headeronly)
        {
            /* less checks here and in ospf6_abr_lsa_examin() */
            if (MSG_OK != ospf6_abr_lsa_examin (lsah))
            {
                return MSG_NG;
            }
            lsah = (struct ospf6_lsa_header *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE);
...
//sangmeng add 2020310
static int ospf6_abr_packet_examin(struct ospf6_interface *oi, struct ospf6_header *oh, const unsigned bytesonwire)
{
    ...
    case OSPF6_MESSAGE_TYPE_LSREQ:
        ...
        test = ospf6_abr_lsaseq_examin
               (
                   (struct ospf6_lsa_header *) ((caddr_t) oh + OSPF6_HEADER_SIZE + OSPF6_LS_REQ_MIN_SIZE),
    ...
    default:
        if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
            zlog_debug ("%s: invalid (%u) message type on abr router", __func__, oh->type);
        return MSG_NG;
    }
...
#endif
        //add 2020
        /*If the area id of the packet received by this interface is different from the area id of this interface, then this router is an area border router. This flag is initialized only once*/
        if (oi->area->ospf6->router_pbr_flag == OSPF6_PBRDISABLED)
            oi->area->ospf6->router_pbr_flag = OSPF6_PBRENABLED;

        /*If the area ID received by this interface is not consistent with its own area ID, then this interface is an area boundary interface*/
        /*An area border router has at most two interfaces that are area border interfaces that connect to two different areas.*/
        if (oi->interface_pbi_flag == OSPF6_PBIDISABLED)
        {
            DG("set %s interface_pbi_flag to OSPF6_PBIENABLED.\n", oi->interface->name);
            oi->interface_pbi_flag =  OSPF6_PBIENABLED;
            oi->neighbor_area_id = oh->area_id;
        }
        /*recv packet lsa type examin, filter router-lsa, network-lsa, intra-area-prefix-lsa*/
#if 1//add 2020
        if (ospf6_abr_packet_examin(oi, oh, bytesonwire) != MSG_OK)
            return MSG_NG;
#endif
    }
...
#ifdef OSPF6_DEBUG
    zlog_debug("ospf6 hello send interval:%d.", oi->hello_interval);
#endif
...
//add 20200311
#if 1
            if ((on->neighbor_pbr_flag == 1) && (ospf6_abr_lsa_examin(lsa->header) != MSG_OK))
                continue;
#endif
...
#if 1 //new 20191016 
int
ospf6_lsupdate_send_neighbor_for_new_lsas (struct thread *thread)
{
    struct ospf6_neighbor *on;
    struct ospf6_header *oh;
    struct ospf6_lsupdate *lsupdate;
    ...
    /* new_lsas_list lists those LSA which doesn't need to be
...
static int ospf6_get_lsa_header_type (struct ospf6_lsa_header *lsah, struct prefix_ipv6 *p)
{
    ...
    case OSPF6_LSTYPE_INTRA_PREFIX:
#if 0
        ret = ospf6_find_lsa_prefix_in_lsdb (lsah);
        if (ret == 1)
            return 1;
#endif
#ifdef OSPF6_DEBUG
...
ospf6_lsupdate_send_interface (struct thread *thread)
{
    ...
    //printf(">>>>%s()%d will send lsupate to interface:%s, lsupdate_list->count:%d.\n", __func__, __LINE__, oi->interface->name, oi->lsupdate_list->count);
    memset (sendbuf, 0, iobuflen);
    oh = (struct ospf6_header *) sendbuf;
