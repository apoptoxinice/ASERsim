...
ospf6_lsdb_add_for_leaving (struct ospf6_lsa *lsa, struct ospf6_lsdb *lsdb)
{
    ...
    //zlog_debug("%s()%d lsa add to new lsas list successfully, now count is:%d", __func__, __LINE__, lsdb->count);
}

void
ospf6_lsdb_add_for_expire (struct ospf6_lsa *lsa, struct ospf6_lsdb *lsdb)
{
    struct prefix_ipv6 key;
    struct route_node *current;
    struct ospf6_lsa *old = NULL;
...
ospf6_lsdb_add (struct ospf6_lsa *lsa, struct ospf6_lsdb *lsdb)
{
...
#endif
    //DG("enter ospf6_lsdb_add, name:%s, age:%d.\n", lsa->name, ntohs (lsa->header->age));
    memset (&key, 0, sizeof (key));
    ospf6_lsdb_set_key (&key, &lsa->header->type, sizeof (lsa->header->type));
    ospf6_lsdb_set_key (&key, &lsa->header->adv_router,
                        sizeof (lsa->header->adv_router));
    ...
    if (!old)
    {
        ...
        if (OSPF6_LSA_IS_MAXAGE (lsa))
        {
            ...
            if (lsdb->hook_remove)
            {
                //DG("will call hoo_remove, name:%s.\n", lsa->name);
                (*lsdb->hook_remove) (lsa);
            }
        }
        else
    ...
    {
        //DG("old is not null, name:%s, age:%d.\n", old->name, ntohs (old->header->age));
        if (OSPF6_LSA_IS_CHANGED (old, lsa))
        {
            //DG("lsa is changed.\n");
            if (OSPF6_LSA_IS_MAXAGE (lsa))
            {
                //DG("maxage, lsa name:%s.\n", lsa->name);
                if (lsdb->hook_remove)
                {
                    //DG("will call hoo_remove, old name:%s age:%d, new name:%s, age:%d.\n", old->name, ntohs (old->header->age), lsa->name, ntohs (lsa->header->age));
            ...
            else if (OSPF6_LSA_IS_MAXAGE (old))
            {
                ...
                if (lsdb->hook_add)
                {
                    //DG("will call hook_add, name:%s.\n", lsa->name);
                    (*lsdb->hook_add) (lsa);
                }
            }
            else
            {
                //DG("enter else old name:%s age:%d, new name:%s, age:%d.\n", old->name, ntohs (old->header->age), lsa->name, ntohs (lsa->header->age));
                if (lsdb->hook_remove)
                    (*lsdb->hook_remove) (old);
                if (lsdb->hook_add)
                    (*lsdb->hook_add) (lsa);
...
#if 0/* delete for don't add lsa to link list*/
    /* next link */
    nextnode = current;
    route_lock_node (nextnode);
    do
...
void
ospf6_lsdb_remove_for_expire (struct ospf6_lsa *lsa, struct ospf6_lsdb *lsdb)
{
    struct route_node *node;
    struct prefix_ipv6 key;
                 ...
                 struct ospf6_lsdb *lsdb)
{
    ...
    if (level == OSPF6_LSDB_SHOW_LEVEL_NORMAL)
        showfunc = ospf6_lsa_show_summary; //mark here 2020 ('show ipv6 ospf6 database' will use this func)
