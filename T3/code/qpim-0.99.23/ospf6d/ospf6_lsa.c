...
#if 0
    else if (type == OSPF6_LSTYPE_ZONE)
    {
        vty_out (vty, "%-4s %-15s%-15s%4hu %8lx %30s%s",
                 ospf6_lstype_short_name (lsa->header->type),
                 id, adv_router, ospf6_lsa_age_current (lsa),
...
#endif
    else if (type != OSPF6_LSTYPE_UNKNOWN)  //OSPF6_TYPE_INTRA_PREFIX mark here 2020
    {
...
struct ospf6_lsa *
ospf6_lsa_copy_for_expire (struct ospf6_lsa *lsa)
{
...
struct ospf6_lsa * ospf6_lsa_copy_for_leaving(struct ospf6_lsa *lsa, struct ospf6_interface *oi)
{
    struct ospf6_lsa *copy = NULL;
...
/* decrement reference counter of struct ospf6_lsa */
void
ospf6_lsa_unlock_for_leaving (struct ospf6_lsa *lsa)
{
    /* decrement reference counter */
    assert (lsa->lock > 0);
    lsa->lock--;
...
void
ospf6_lsa_unlock_for_expire (struct ospf6_lsa *lsa)
{
    /* decrement reference counter */
    assert (lsa->lock > 0);
    lsa->lock--;
...
#if 1 //add 2019.11.29
    //zlog_debug("%s()%d will call ospf6_install_lsa_for_expire.", __func__, __LINE__);
    ospf6_install_lsa_for_expire(lsa);
#endif
    /* reflood lsa */
    //printf("\n^^^^^^^^^^^%s()%d will call ospf6_flood^^^^^^^^^^^^^^^^^.\n", __func__, __LINE__);
