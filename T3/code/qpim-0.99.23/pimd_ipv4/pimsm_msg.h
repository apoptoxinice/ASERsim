...
{
    uint32_t   source; 		// the source for which we want iif and rpfnbr
    uint32_t   prefixlen; //sangmeng add

#define	MRTMGT_RPF_CTL_INVALID_NEIGHBOR	0
    uint32_t   rpf_nbr_addr;	// next hop towards the source
...
typedef struct
{
    ...
    uint8_t blRpEmbeded;
    VOS_IP_ADDR dwBsrRootID;
    VOS_IP_ADDR dwRpRootID;
