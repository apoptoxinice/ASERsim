...
struct pimsm_rpf
{
    ...
    VOS_IP_ADDR dest_addr;
    uint32_t rpf_if;
    VOS_IP_ADDR rpf_nbr_addr;
    uint32_t preference;
    uint32_t metric;
};
...
struct pimsm_updownstream_if_entry
{
    ...
    /* upstream */
    VOS_IP_ADDR up_stream_nbr; /* upstream neighbor address, just for upstream interface */
...
struct pimsm_grp_entry
{
    ...
    VOS_IP_ADDR		rp_addr;       /* The IP address of the RP */
...
extern struct pimsm_grp_entry *pimsm_SearchGrpEntry(VOS_IP_ADDR *pstGrpAddr);
extern struct pimsm_src_entry *pimsm_SearchSrcEntry(VOS_IP_ADDR *src_addr);
extern struct pimsm_mrt_entry *pimsm_SearchMrtEntry(VOS_IP_ADDR *src_addr, VOS_IP_ADDR *pstGrpAddr, uint8_t type);
extern struct pimsm_mrt_entry *pimsm_CreateMrtEntry(VOS_IP_ADDR *src_addr, VOS_IP_ADDR *pstGrpAddr, uint8_t type, uint32_t flags);
...
extern uint8_t pimsm_SwitchToSptDesired(VOS_IP_ADDR *pstGrp);
...
extern struct pimsm_rpf *pimsm_SearchRpf(VOS_IP_ADDR *pstDstAddr);
extern int pimsm_AddLocalReceiver(VOS_IP_ADDR *src_addr, VOS_IP_ADDR *pstGrpAddr, uint8_t type, uint32_t ifindex);
extern int pimsm_DelLocalReceiver(VOS_IP_ADDR *src_addr, VOS_IP_ADDR *pstGrpAddr, uint8_t type, uint32_t ifindex);
