...
OAM_CMD_FUNC ( oam_cmd_proc_enable_config_terminal_ip_pim_rp_address)
{
    ...
    if (strcmp(pCmdBuf->buf[0], "no") == 0)
    {
        rp_addr = *(VOS_IP_ADDR *)(pCmdBuf->buf[4]);
        if(pCmdBuf->count > 5)
        {
            group_addr = *(VOS_IP_ADDR *)(pCmdBuf->buf[5]);
            stGroupMask = *(VOS_IP_ADDR *)(pCmdBuf->buf[6]);
        }
        else
        {
    ...
    {
        rp_addr = *(VOS_IP_ADDR *)(pCmdBuf->buf[3]);
        if(pCmdBuf->count > 4)
        {
            group_addr = *(VOS_IP_ADDR *)(pCmdBuf->buf[4]);
            stGroupMask = *(VOS_IP_ADDR *)(pCmdBuf->buf[5]);
        }
        else
        {
            memset(&group_addr, 0, sizeof(group_addr));
...
OAM_CMD_FUNC ( oam_cmd_proc_enable_config_terminal_ip_pim_accept_register)
{
    ...
    if (strcmp(pCmdBuf->buf[0], "no") == 0)
    {
        stCliMsgToPimsm.u_msg.mCliToPimsm.u_cmd.stAcceptSource.stSourceAddr = *(VOS_IP_ADDR *)(pCmdBuf->buf[4]);
        stCliMsgToPimsm.u_msg.mCliToPimsm.u_cmd.stAcceptSource.stSourceMask = *(VOS_IP_ADDR *)(pCmdBuf->buf[5]);
        MakeCliToPimsmMsg(&(stCliMsgToPimsm), CLI_CMD_PIMSM_NO_ACCEPT_REGISTER, iIODevice);
    }
    else
    {
        stCliMsgToPimsm.u_msg.mCliToPimsm.u_cmd.stAcceptSource.stSourceAddr = *(VOS_IP_ADDR *)(pCmdBuf->buf[3]);
        stCliMsgToPimsm.u_msg.mCliToPimsm.u_cmd.stAcceptSource.stSourceMask = *(VOS_IP_ADDR *)(pCmdBuf->buf[4]);
...
OAM_CMD_FUNC(oam_cmd_proc_enable_config_terminal_ip_pim_crp_group_policy)
{
    ...
    if (strcmp(pCmdBuf->buf[0], "no") == 0)
    {
        stCliMsgToPimsm.u_msg.mCliToPimsm.u_cmd.stCrpGroupPolicy.group_addr = *(VOS_IP_ADDR *)(pCmdBuf->buf[5]);
        stCliMsgToPimsm.u_msg.mCliToPimsm.u_cmd.stCrpGroupPolicy.stGroupMask = *(VOS_IP_ADDR *)(pCmdBuf->buf[6]);
        MakeCliToPimsmMsg(&(stCliMsgToPimsm), CLI_CMD_PIMSM_NO_CRP_GROUP_POLICY, iIODevice);
    }
    else
    {
        stCliMsgToPimsm.u_msg.mCliToPimsm.u_cmd.stCrpGroupPolicy.group_addr = *(VOS_IP_ADDR *)(pCmdBuf->buf[4]);
        stCliMsgToPimsm.u_msg.mCliToPimsm.u_cmd.stCrpGroupPolicy.stGroupMask = *(VOS_IP_ADDR *)(pCmdBuf->buf[5]);
...
OAM_CMD_FUNC ( oam_cmd_proc_show_ip_pim_rpf)
{
    ...
    stCliMsgToPimsm.u_msg.mCliToPimsm.u_cmd.stRpfDestAddr = *(VOS_IP_ADDR *)(pCmdBuf->buf[4]);
