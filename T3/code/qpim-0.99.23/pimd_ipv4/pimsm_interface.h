...
struct pimsm_prefix_ipv4_list
{
    struct pimsm_prefix_ipv4_list *next;
    struct pimsm_prefix_ipv4_list *prev;
    VOS_IP_ADDR   addr;
    uint16_t prefixlen;
};
...
struct pimsm_neighbor_entry
{
    ...
    VOS_IP_ADDR source_addr; /* The neighbor Primary address */
    struct pimsm_prefix_ipv4_list *pstSecondaryAddrList; /* The neighbor secondary address list  */
...
struct pimsm_interface_member
{
    ...
    VOS_IP_ADDR grp_addr;  //group address
...
struct pimsm_interface_entry
{
    ...
    VOS_IP_ADDR primary_address;//the linklocal address
    uint16_t prefixlen; //the linklocal address prefix len
    struct pimsm_prefix_ipv4_list *pstSecondaryAddr; //global address list

    uint32_t ifindex;  //interface number
    char name[INTERFACE_NAMSIZ + 1];  //interface name
    int pimsm_generation_id;

    VOS_IP_ADDR pimsm_dr_addr;       //the current DR (is :: in P2P network)
...
int pimsm_ShowInterface(struct vty *vty,uint32_t ifindex);
extern uint8_t pimsm_IsLocalAddress(VOS_IP_ADDR *pstAddr);
extern uint32_t pimsm_DirectIfSearch(VOS_IP_ADDR *pstAddr);
