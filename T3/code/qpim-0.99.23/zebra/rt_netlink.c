...
static int ip_address_delete_from_kernel(struct interface *ifp)
{
...
#if 0
            printf("%s()%d ip unset interface IP address: %s.\n",
                   __func__, __LINE__, prefix_out(connected->address));
#endif
        }
    }
    ...
    {
#if 0
        printf("0....will del ipv6 address, connected->address->family:%d.\n", connected->address->family);
#endif
#if 0
        if (CHECK_FLAG (connected->conf, ZEBRA_IFC_REAL) &&
            ...
            {
                printf("1....will del ipv6 address.\n");
                ...
                if (ret < 0)
                {
                    zlog_debug ("Can't unset interface IPv6 address: %s",
                                safe_strerror(errno));
                }

#if 0
                printf ("%s()%d ipv6 unset interface IPv6 address: %s.\n",
                        __func__, __LINE__, prefix_out(connected->address));
#endif
            }
    }
...
netlink_link_change (struct sockaddr_nl *snl, struct nlmsghdr *h)
{
    ...
    if (h->nlmsg_type == RTM_NEWLINK)
    {
        ...
        else
        {
            ...
            {
                ifp->flags = ifi->ifi_flags & 0x0000fffff;
#if 1/*sangmeng add for link up and down*/
                if (if_is_up (ifp) && !if_is_running (ifp))
                {
                    //zlog_debug ("line protocol is down\n");
                    printf ("line protocol is down, will delete address from kernel\n");
                    ip_address_delete_from_kernel (ifp);
                    if_down (ifp);
                }
                else if (if_is_running (ifp))
                {
                    //zlog_debug ("line protocol is up\n");
                    printf ("line protocol is up, del addr from kernel, and recover it\n");
                    ip_address_delete_from_kernel (ifp);
                    if_refresh (ifp);
...
int
addattr_l (struct nlmsghdr *n, uint32_t maxlen, int type, void *data, int alen)
{
    int len;
    struct rtattr *rta;
...
/* Utility function comes from iproute2.
   Authors:	Alexey Kuznetsov, <kuznet@ms2.inr.ac.ru> */
int
addattr32 (struct nlmsghdr *n, uint32_t maxlen, int type, int data)
{
    int len;
    struct rtattr *rta;
    ...
    size_t req_size)
{
            ...
            || nexthop->type == NEXTHOP_TYPE_IPV4_IFINDEX)
    {
        ...
        //sangmeng close
        //if (IS_ZEBRA_DEBUG_KERNEL)
        zlog_debug("%s()%d netlink_route_multipath() (%s): "
                   "nexthop via %s if %u",
                   __func__, __LINE__,
                   routedesc,
...
#ifdef HAVE_IPV6
    int ret = 1;
    //added for 4over6 20130312
    if(nexthop->ifname)
        ret = memcmp(nexthop->ifname, "4o6_tnl", 7);
    if (ret != 0)
    {
...
)
{
    ...
    printf("%s()%d enter _netlink_route_build_multipath.\n", __func__, __LINE__);
    if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_ONLINK))
                         ...
                         int family)
{
    ...
    printf("%s()%d enter netlink_route_multipath.\n", __func__, __LINE__);
    memset (&req, 0, sizeof req - NL_PKT_BUF_SIZE);
