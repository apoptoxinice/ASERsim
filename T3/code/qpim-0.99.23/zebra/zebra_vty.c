...
{
    return zebra_static_ipv4_safi(vty, SAFI_MULTICAST, 1, argv[0], NULL, argv[1], NULL, NULL);
}
...
DEFUN (no_ip_mroute,
       ...
       "Nexthop interface name\n")
{
    return zebra_static_ipv4_safi(vty, SAFI_MULTICAST, 0, argv[0], NULL, argv[1], NULL, NULL);
}
...
static int ip_nat_pool_lookup(char *pool_name, struct nat_pool_entry *pool)
{
    struct nat_pool_entry *pLast;
    struct nat_pool_entry *pNext;
    int iRetVal = 0;
...
static int ip_nat_source_list_lookup(char *list_name, struct nat_source_list *source)
{
    struct nat_source_list *pLast;
    struct nat_source_list *pNext;
    int iRetVal = 0;
...
/*
  ...
  */
#ifdef HAVE_DHCPV4
static int sendtoserver(struct vty *vty,char buffer[1024])
{
...
static void change_ipv4address_new(const char *buf, char *newbuf)
{
    char str[50]= {0};
    const char *start=NULL,*p=buf;
    int i,address1,address2,address3,address4;

    //change adress eg: 010.01.001.1 -> 10.1.1.1
    i=0;
    start=p;
    while( *p!='.' )
    {
...
static inline void change_ipv4address(char *buf)
{
    char str[50]= {0};
...
static int compare_twoaddress(char *buf, char *buf2)//low address should be < high address
{
    char str[50]= {0};
    char *start=NULL,*p=buf;
       ...
       "High IP address\n")
{
    ...
    memset(&buf_argv0, 0x00, sizeof(buf_argv0));
    memset(&buf_argv1, 0x00, sizeof(buf_argv1));

    change_ipv4address_new(argv[0], buf_argv0);
    change_ipv4address_new(argv[1], buf_argv1);
    if( compare_twoaddress(buf_argv0, buf_argv1) == -1 )//address argv[1] should >= argv[0]
    {
       ...
       "High IP address\n")
{
    ...
    else
    {
        ...
        strcat(buf,buf_argv0);
        strcat(buf," ");
        strcat(buf,buf_argv1);
    }
      ...
      )
{
    ...
    change_ipv4address_new(argv[0], buf_argv0);
    change_ipv4address_new(argv[1], buf_argv1);
    if( (buf_argv0[0]=='0') || (buf_argv1[0] =='0') )
    {
      ...
      )
{
    ...
    memset(&buf_argv0, 0x00, sizeof(buf_argv0));
    memset(&buf_argv1, 0x00, sizeof(buf_argv1));
    memset(&buf_argv2, 0x00, sizeof(buf_argv2));

    change_ipv4address_new(argv[0], buf_argv0);
    change_ipv4address_new(argv[1], buf_argv1);
    change_ipv4address_new(argv[2], buf_argv2);
...
#endif
#ifdef HAVE_DHCPV4
static int zebra_dhcp_write_config(struct vty *vty)
{
    struct sockaddr_in my_addr;
...
static int zebra_ecn_write_config(struct vty *vty, const char *ifname)
{
    vty_out(vty,"!%s",VTY_NEWLINE);
    int i = 0;
    for(i=0; i<4; i++)
...
//end add/
static int zebra_ivi_write_config(struct vty *vty, const char *ifname)
{
#define SIOCGETPREFIX SIOCGETTUNNEL
    struct ifreq ifr;
    // struct tnl_parm ivi46;
    ...
    if(strcmp(ifname,"ivi")==0)
    {
        ...
        if(ret == -1)
        {
            //vty_out(vty,"ioctl error in ivi: %d\n",errno);
            close(socketfd);
            return -1;
        }
...
//	vty_out(vty, "come here!: %d\n", num);
    while(pNext != NULL)
    {
        ...
        if(pNext->name != NULL)
        {
            ...
            ifr.ifr_data = (__caddr_t)&set_mss;
...
static void show_configure_info(struct vty* vty, char *recv_buf)
{
    char prefix[128];
    int i;
    int count;
...
#ifdef HAVE_DNS64
static void dns64_prfix_dns_set(const char *v6p, const char *v4Dnstr)
{
    struct prefix stv6p;
...
static void dns64_prfix_dns_unset()
{
    memset(&v6prefix,0x0,sizeof(struct prefix));
    v4Dns =0;
}

static inline BYTE dns64_config_check(void)
{
...
#endif
#ifdef HAVE_DNS64
static int zebra_dns64_write_config(struct vty *vty)
{
    char pre[40];
...
/* add by s 130806 */
#ifdef HAVE_4OVER6_TCPMSS
static int zebra_4over6_write_tcp_mss_config(struct vty *vty)
{
    int socketfd;
    int ret = 0;
...
static int zebra_nat64_write_config(struct vty *vty, const char *ifname)
{
...
static int zebra_flow_sepa_write_config(struct vty *vty, const char *ifname)
{
    vty_out(vty,"!%s",VTY_NEWLINE);
...
static int zebra_header_compress_write_config(struct vty *vty, const char *ifname)
{
    vty_out(vty,"!%s",VTY_NEWLINE);
...
zebra_netwire_config (struct vty *vty)
{
    ...
    if(rc != 0)
    {
        //printf("Get ivi prefix failed: %d\n", rc);
    }
    else
        write += 1;
       ...
       "Interface name\n")
{
    ...
    struct sockaddr_in *sin = NULL, *sin1 = NULL;
...
static int static_config_ipv4_arp(struct vty *vty)
{
    int k=0;
    int write=0;
...
static int flow_check_inter_area(const char *ifname)
{
    struct inter_in_area *p= inter_area_head;
...
static int flow_add_area_list(const char *ifname,char flag)
{
    struct inter_in_area *p= inter_area_head;
    if(inter_area_head == NULL)
    {
...
static int send_control_msg_kernel(struct vty*vty)
{
    ...
    memset(msg_load, 0x00, sizeof(msg_load));
...
static int update_control_msg_to_kernel(const char *ifname,struct vty*vty)
{
    //send to kernel
...
static int flow_del_area_list(const char *ifname)
{
    struct inter_in_area *p= inter_area_head;
...
#define DEL 0
//add by ccc for ecn
static unsigned int ll_name_to_index(const char *name)
{
...
static int rtnl_talk(struct rtnl_handle *rtnl, struct nlmsghdr *n, __u32 peer,
    ...
    while (1)
    {
        ...
        for (h = (struct nlmsghdr*)buf; status >= sizeof(*h); )
        {
            ...
            if (l<0 || len>status)
            {
                ...
                fprintf(stderr, "!!!malformed message: len=%ld\n", len);
                return 1;
            }
...
static int tc_red_eval_idle_damping(unsigned char *sbuf)
{
    int clog = 0;
    memset(sbuf,0,255);
    return clog;
}
static int rtnl_dump_request(struct rtnl_handle *rth, int type, void *req, int len)
{
    struct nlmsghdr nlh;
    struct sockaddr_nl nladdr;
    struct iovec iov[2] =
...
static inline  void print_size_table(FILE *fp, const char *prefix, struct rtattr *rta)
{
...
static void print_tcstats2_attr(struct vty*vty, struct rtattr *rta, const char *prefix, struct rtattr **xstats)
{
    //SPRINT_BUF(b1);
...
static void print_tcstats_attr(struct vty*vty, struct rtattr *tb[], const char *prefix, struct rtattr **xstats)
{
    //SPRINT_BUF(b1);
    if (tb[TCA_STATS2])
    {
...
static int red_print_xstats(struct vty*vty, struct rtattr *xstats)
{
...
//add by ccc for ecn
static int my_addattr_l(struct nlmsghdr *n, size_t maxlen, int type, const void *data,
                        int alen)
{
    ...
    {
        fprintf(stderr, "addattr_l ERROR: message exceeded bound of %ld\n",maxlen);
        return -1;
    }
...
static int start_rt_talk(int cmd,const char *dev_name,int p_num)
{
    ...
    char  k[16] = "red";//red
    if (k[0])
        my_addattr_l(&req.n, sizeof(req), TCA_KIND, k, strlen(k)+1);//enable red
    ...
    my_addattr_l(&req.n, 1024, TCA_OPTIONS, NULL, 0);
    my_addattr_l(&req.n, 1024, TCA_RED_PARMS, &(red_ecn_opt[p_num-1].tc_ecn_info), sizeof(struct tc_red_qopt));
    my_addattr_l(&req.n, 1024, TCA_RED_STAB, sbuf, 256);
...
static int check_ecn_info(int num)
{
    struct if_politic_qdisc *p = if_ecn_head;
    while(p != NULL)
        if(p->politic == num)
...
static int print_qdisc(const struct sockaddr_nl *who,struct nlmsghdr *n,void *arg)
{
    //FILE *fp = (FILE*)arg;
...
static int check_ecn_info_by_name(const char *ifname)
{
    struct if_politic_qdisc *p = if_ecn_head;
    if(if_ecn_head == NULL)
        return -1;
     ...
     )
{
    ...
    if( num >= 0)
    {
        vty_out(vty,"enable red politic %d: minth %dMbps maxth %dMbps%s",
                num,red_ecn_opt[num-1].tc_ecn_info.qth_min,red_ecn_opt[num-1].tc_ecn_info.qth_max,VTY_NEWLINE);
    }
...
static int save_ecn_info(const char *ifname,int num)
{
    struct if_politic_qdisc *p = if_ecn_head;
...
static void del_ecn_info(const char *ifname)
{
    if(if_ecn_head == NULL)
    {
        return;
 ...
 * */
static int check_if_exist(const char *ifname,int flag)
{
    struct if_compress_enabled *p = if_compress_head;
...
static int write_message_if_compre_file(void)
{
    FILE *fp;
    char *pstr = NULL;
    char *pstrI = NULL;
...
static int del_if_message_at_list(const char *ifname)
{
    struct if_compress_enabled *p= if_compress_head;
...
static int add_if_compress_list(const char *ifname,int flag)
{
    struct  if_compress_enabled *p =if_compress_head ;
    if(if_compress_head == NULL)
    {
...
static int set_if_compress_list(const char *ifname,int flag)
{
    struct  if_compress_enabled *p =if_compress_head ;
    while(p != NULL)
    {
