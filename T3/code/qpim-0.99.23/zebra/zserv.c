...
extern char tnl_number[TUNNELNUMBER];

//added for nat 20130508
extern struct nat_pool_entry natPoolEntry;
extern struct nat_source_list natSourceList;
extern struct nat_source_list_pool_entry natSourceListPoolEntry;
...
#if 1/*sangmeng add for pim6 2190606*/
static int
zsend_ipv6_nexthop_lookup_mrib (struct zserv *client, struct in6_addr *addr)
{
    struct stream *s;
    struct rib *rib;
    unsigned long nump;
    ...
    /* Lookup nexthop. */
    rib = rib_match_ipv6_safi (addr, SAFI_MULTICAST, skip_bgp);
    ...
    if (!rib)
    {
        ...
        rib = rib_match_ipv6_safi (addr, SAFI_UNICAST, skip_bgp);
        if (IS_ZEBRA_DEBUG_PACKET && IS_ZEBRA_DEBUG_RECV)
...
#endif
    /* Fill in result. */
    zserv_create_header (s, ZEBRA_IPV6_NEXTHOP_LOOKUP_MRIB);
    stream_put (s, &addr, 16);

    if (rib)
    {
        printf("******************(debug zebra) rib is not null.\n");
        stream_putc (s, rib->distance);
        stream_putl (s, rib->metric);
        num = 0;
        nump = stream_get_endp(s);
        ...
        for (nexthop = rib->nexthop; nexthop; nexthop = nexthop->next)
            if (CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB))
            {
                ...
                case ZEBRA_NEXTHOP_IFNAME:
#if 0
                    printf("1111enter here.\n");
#endif
                    stream_putl (s, nexthop->ifindex);
                    break;
                default:
#if 0
                    printf("enter here, default:%d.\n", nexthop->type);
#endif
                    /* do nothing */
                    break;
                }
    ...
    {
#if 0
        printf("*************(2debug zebra) rib is null.\n");
#endif
...
/* MRIB Nexthop lookup for IPv6. */
static int
zread_ipv6_nexthop_lookup_mrib (struct zserv *client, u_short length)
{
    ...
    return zsend_ipv6_nexthop_lookup_mrib (client, &addr);
}
#endif
...
zebra_client_read (struct thread *thread)
{
    ...
    case ZEBRA_IPV4_4OVER6_ROUTE_ADD:
        zread_ipv4_4over6_add (client, length);
        break;
    case ZEBRA_IPV4_4OVER6_ROUTE_DELETE:
        zread_ipv4_4over6_delete (client, length);
        break;
#ifdef HAVE_IPV6
    ...
    case ZEBRA_IPV6_NEXTHOP_LOOKUP:
        ...
        break;
    case ZEBRA_IPV6_NEXTHOP_LOOKUP_MRIB:
        printf("%s()%denter here, type:%d.\n", __func__, __LINE__, ZEBRA_IPV6_NEXTHOP_LOOKUP_MRIB);
        zread_ipv6_nexthop_lookup_mrib (client, length);
        break;
#endif /* HAVE_IPV6 */
...
zebra_init (void)
{
    ...
    memset (&zebra4over6TunnelEntry, 0, sizeof(struct zebra_4over6_tunnel_entry));
    memset(tnl_number, 0, sizeof(tnl_number));
    thread_add_timer (zebrad.master, zebra_4over6_nexthop_check, NULL, 60);

    //added for nat 20130507
    memset (&natPoolEntry, 0, sizeof(struct nat_pool_entry));
    memset (&natSourceList, 0, sizeof(struct nat_source_list));
    memset (&natSourceListPoolEntry, 0, sizeof(struct nat_source_list_pool_entry));
