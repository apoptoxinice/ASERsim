...
vrf_alloc (const char *name)
{
    ...
    vrf_table_create (vrf, AFI_IP, SAFI_4OVER6);
...
struct rib *
rib_match_ipv6_safi (struct in6_addr *addr, safi_t safi, int skip_bgp)
{
    struct prefix_ipv6 p;
    struct route_table *table;
    struct route_node *rn;
...
rib_install_kernel (struct route_node *rn, struct rib *rib)
{
    ...
    {
    case AF_INET:
        printf("%s()%d kernel add ipv4.\n", __func__, __LINE__);
        ret = kernel_add_ipv4 (&rn->p, rib);
        break;
#ifdef HAVE_IPV6
    case AF_INET6:
        printf("%s()%d kernel add ipv6.\n", __func__, __LINE__);
...
rib_process (struct route_node *rn)
{
    ...
    RNODE_FOREACH_RIB_SAFE (rn, rib, next)
    {
        ...
        //printf("%s()%d will call nexthop_active_update.\n", __func__, __LINE__);
        if (! nexthop_active_update (rn, rib, 0))
        {
            //printf("%s()%d continue here.\n", __func__, __LINE__);
            continue;
        }
...
void _rib_dump (const char * func, union prefix46constptr pp, const struct rib * rib)
{
...
static int getInterfaceIndex(char *name)
{
    int sd;
    struct ifreq ifr;
...
/*
 * sangmeng add for 4over6 use lo interface to create tunnel
 *
 * */
static int getIp6Addr_use_lo(char *buf, const char *if_name)
{
    ...
    while (ifAddrStruct!=NULL)
    {
        if ((ifAddrStruct->ifa_addr !=NULL)&&(ifAddrStruct->ifa_addr->sa_family==AF_INET6))
        {
            ...
            inet_ntop(AF_INET6, (uint8_t *)tmpAddrPtr + 4, addressBuffer, INET6_ADDRSTRLEN);
            ...
            if(strcmp (ifAddrStruct->ifa_name, if_name) == 0)
            {
                ...
                memcpy(buf, (uint8_t *)tmpAddrPtr + 4, 16);
                //printf("%s()%d  %s get it!!!\n\n\n\n", __func__, __LINE__, ifAddrStruct->ifa_name);
                zlog_debug("%s()%d  %s get it!!!\n\n\n\n", __func__, __LINE__, ifAddrStruct->ifa_name);
                return 1;
            }
            memset(buf, 0xff, 16);
        }
...
static int getIp6Addr(char *buf, int index)
{
    struct ifaddrs * ifAddrStruct=NULL;
    void * tmpAddrPtr=NULL;
    ...
    while (ifAddrStruct!=NULL)
    {
        if ((ifAddrStruct->ifa_addr !=NULL)&&(ifAddrStruct->ifa_addr->sa_family==AF_INET6))
        {
            ...
            if(index == getInterfaceIndex(ifAddrStruct->ifa_name))
            {
                ...
                zlog_debug("%s get it!", ifAddrStruct->ifa_name);
                return 0;
            }
            memset(buf, 0xff, 16);
        }
...
static int zebra_4over6_tunnel_create(struct in6_addr remote, char *tunnel_name)
{
    struct ifreq ifr;
    ...
    if (ret_lo == 0)
    {
        ...
        RNODE_FOREACH_RIB (rn, rib)
        {
            zlog_debug("rib->flags:%u.", rib->flags);
            if(CHECK_FLAG (rib->flags, ZEBRA_FLAG_SELECTED))
                break;
        }
        if(rib == NULL)
        ...
        {
            printf("%s()%d rib->nexthop->rifindex:%d.\n", __func__, __LINE__, rib->nexthop->rifindex);
            getIp6Addr((char *)&local, rib->nexthop->rifindex);
        }
        else
        {
            printf("%s()%d rib->nexthop->rifindex:%d.\n", __func__, __LINE__, rib->nexthop->ifindex);
            getIp6Addr((char *)&local, rib->nexthop->ifindex);
        }
    }

    memset(buf, 0x00, BUFSIZ);
    inet_ntop (AF_INET6, &local, buf, BUFSIZ);
    zlog_notice("%s()%d tunnel_name: %s, local = %s", __func__, __LINE__, tunnel_name, buf);
    ...
    ifr.ifr_data = (__caddr_t)&tnl_4over6;
...
zebra_4over6_nexthop_check(struct thread *thread)
{
    ...
    while (pNext!=NULL)
    {
        if(pNext->state != 1)
        {
            ...
            ret = zebra_4over6_tunnel_create(nexthop, tunnel_name);
            if(ret == 0)
                pNext->state = 1;
        }
...
int zebra_4over6_nexthop_add_check(struct in6_addr nexthop)
{
    struct zebra_4over6_tunnel_entry *pLast;
    struct zebra_4over6_tunnel_entry *pNext;
...
static int rib_add_ipv4_4over6 (int type, int flags, struct prefix_ipv4 *p,
                                struct in6_addr *gate, unsigned int ifindex, u_int32_t vrf_id,
                                u_int32_t metric, u_char distance, safi_t safi)
{
    struct rib *rib;
    ...
    RNODE_FOREACH_RIB (rn, rib)
    {
        ...
        else if ((nexthop = rib->nexthop) && nexthop->type == NEXTHOP_TYPE_IFINDEX &&
                 nexthop->ifindex == ifindex && !CHECK_FLAG (rib->status, RIB_ENTRY_REMOVED))
        {
            rib->refcnt++;
            return 0;
        }
...
/* XXX factor with rib_delete_ipv6 */
static int rib_delete_ipv4_4over6 (int type, int flags, struct prefix_ipv4 *p,
                                   struct in6_addr *gate, unsigned int ifindex, u_int32_t vrf_id, safi_t safi)
{
    struct route_table *table;
    struct route_node *rn;
    ...
    if (! rn)
    {
        if (IS_ZEBRA_DEBUG_KERNEL)
        {
            if (gate)
                zlog_debug ("route %s/%d via %s ifindex %d doesn't exist in rib",
                            inet_ntop (AF_INET, &p->prefix, buf1, INET_ADDRSTRLEN),p->prefixlen,
                            inet_ntop (AF_INET6, gate, buf2, INET6_ADDRSTRLEN),ifindex);
            else
                zlog_debug ("route %s/%d ifindex %d doesn't exist in rib",
                            inet_ntop (AF_INET, &p->prefix, buf1, INET_ADDRSTRLEN),p->prefixlen,ifindex);
        }
        return ZEBRA_ERR_RTNOEXIST;
    }
        ...
        else
        {
            if (IS_ZEBRA_DEBUG_KERNEL)
            {
                if (gate)
                    zlog_debug ("route %s/%d via %s ifindex %d type %d doesn't exist in rib",
                                inet_ntop (AF_INET, &p->prefix, buf1, INET_ADDRSTRLEN), p->prefixlen,
                                inet_ntop (AF_INET6, gate, buf2, INET6_ADDRSTRLEN), ifindex, type);
                else
                    zlog_debug ("route %s/%d ifindex %d type %d doesn't exist in rib",
                                inet_ntop (AF_INET, &p->prefix, buf1, INET_ADDRSTRLEN),
                                p->prefixlen, ifindex, type);
            }
            route_unlock_node (rn);
            return ZEBRA_ERR_RTNOEXIST;
        }
...
int zread_ipv4_4over6_add (struct zserv *client, u_short length)
{
    ...
    zebra_4over6_nexthop_add_check(nexthop);

    if (IN6_IS_ADDR_UNSPECIFIED (&nexthop))
        rib_add_ipv4_4over6 (api.type, api.flags, &p, NULL, ifindex,
                             zebrad.rtm_table_default, api.metric, api.distance, api.safi);
    else
        rib_add_ipv4_4over6 (api.type, api.flags, &p, &nexthop, ifindex,
                             zebrad.rtm_table_default, api.metric, api.distance, api.safi);
    return 0;
}

static int zebra_4over6_tunnel_delete(char *tunnel_name)
{
    struct ifreq ifr;
    int socketfd;
    int ret=0;
...
int zebra_4over6_nexthop_del_check(struct in6_addr nexthop)
{
    ...
    if (pNext != NULL)
    {
        ...
        if(pNext->num == 0)
        {
            zebra_4over6_tunnel_delete(pNext->name);
...
int zread_ipv4_4over6_delete (struct zserv *client, u_short length)
{
    ...
    if (IN6_IS_ADDR_UNSPECIFIED (&nexthop))
        rib_delete_ipv4_4over6 (api.type, api.flags, &p, NULL, ifindex, client->rtm_table, api.safi);
    else
        rib_delete_ipv4_4over6 (api.type, api.flags, &p, &nexthop, ifindex, client->rtm_table, api.safi);
    return 0;
}
...
/* Install static route into rib. */
static int
static_install_ipv4_4over6 (struct prefix *p, struct static_ipv6 *si)
{
    struct rib *rib;
    struct route_node *rn;
    struct route_table *table;
...
static int
static_uninstall_ipv4_4over6 (struct prefix *p, struct static_ipv6 *si)
{
    struct route_node *rn;
    struct rib *rib;
    struct nexthop *nexthop;
...
int static_delete_ipv4_4over6 (struct prefix *p, u_char type, struct in6_addr *gate,
                               const char *ifname, u_char distance, u_int32_t vrf_id)
{
    struct route_node *rn;
    struct static_ipv6 *si;
    ...
    for (si = rn->info; si; si = si->next)
    {
        if (distance == si->distance && type == si->type
                && (! gate || IPV6_ADDR_SAME (gate, &si->ipv6))
                && (! ifname || strcmp (ifname, si->ifname) == 0))
            break;
    }
...
/* Add static route into static route configuration. */
int
static_add_ipv4_4over6 (struct prefix *p, u_char type, struct in6_addr *gate,
                        const char *ifname, u_char flags, u_char distance, u_int32_t vrf_id)
{
    struct route_node *rn;
    struct static_ipv6 *si;
    struct static_ipv6 *pp;
