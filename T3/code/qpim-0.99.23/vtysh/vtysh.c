...
static void vtysh_exit_ripd_only (void)
{
    if (ripd_client)
...
vtysh_execute_func (const char *line, int pager)
{
    ...
    if (ret == CMD_SUCCESS || ret == CMD_SUCCESS_DAEMON || ret == CMD_WARNING)
    {
        if ((saved_node == BGP_VPNV4_NODE || saved_node == BGP_IPV4_NODE
                || saved_node == BGP_IPV6_NODE || saved_node == BGP_IPV4M_NODE || saved_node == BGP_IP4OVER6_NODE  /*added by wangyl 2013.3.19 for 4over6*/ //sangmeng
...
//sangmeng mark
static struct cmd_node bgp_ip4over6_node =
...
#if 1//sangmeng  mark
DEFUNSH (VTYSH_BGPD,
         address_family_ipv4_4over6,
         address_family_ipv4_4over6_cmd,
...
static void terminal_echo_off(int fd)
{
...
static void terminal_echo_on(int fd)
{
    struct termios old_terminal;
...
static inline int getpasswd(char* passwd, int size)
{
    int c;
    int n = 0;
...
static void get_cmd_line(char *cmd, int n)
{
    int i = 0;
    char c;
    terminal_echo_off(STDIN_FILENO);
...
vtysh_exit (struct vty *vty)
{
    ...
    case BGP_IPV4M_NODE:
        /*added by wangyl 2013.3.19 for 4over6*/
    case BGP_IP4OVER6_NODE: //sangmeng mark
    case BGP_IPV6_NODE:
    case BGP_IPV6M_NODE:
        vty->node = BGP_NODE;
        break;
         ...
         "Exit from Address Family configuration mode\n")
{
    if (vty->node == BGP_IPV4_NODE
            || vty->node == BGP_IPV4M_NODE
            /*added by wangyl 2013.3.19 for 4over6*/
            || vty->node == BGP_IP4OVER6_NODE //sangmeng mark
            || vty->node == BGP_VPNV4_NODE
            || vty->node == BGP_IPV6_NODE
            || vty->node == BGP_IPV6M_NODE)
        vty->node = BGP_NODE;
...
static inline void get_hw_addr(u_char * buf, char *str)
{
    int i;
    unsigned int p[6];
...
void connect_damon(void)
{
    ...
    if (vtysh_client[1].fd == -1)
    {
        if (vtysh_connect(&vtysh_client[1]) == 0)
        {
            //        zlog_debug("connect to %s success\n", vtysh_client[5].name);
        }
        else
    ...
    if (vtysh_client[2].fd == -1)
    {
        if (vtysh_connect(&vtysh_client[2]) == 0)
        {
            //        zlog_debug("connect to %s success\n", vtysh_client[5].name);
        }
        else
    ...
    if (vtysh_client[3].fd == -1)
    {
        if (vtysh_connect(&vtysh_client[3]) == 0)
        {
            //        zlog_debug("connect to %s success\n", vtysh_client[5].name);
        }
        else
...
vtysh_init_vty (void)
{
    ...
    /*added by wangyl 2013.3.19 for 4over6*/
    install_node (&bgp_ip4over6_node, NULL); //sangmeng mark
    /* #ifdef HAVE_IPV6 */
    ...
    /*added by wangyl 2013.3.19 for 4over6*/
    vtysh_install_default (BGP_IP4OVER6_NODE); //sangmeng mark
...
// install_element (ISIS_NODE, &vtysh_exit_isisd_cmd);
    ...
    install_element (BGP_NODE, &address_family_ipv4_4over6_cmd);
#endif
#ifdef HAVE_IPV6
