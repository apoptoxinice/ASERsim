...
static RETSIGTYPE *vtysh_signal_set (int signo, void (*func)(int))
{
    int ret;
...
static void *socket_watch (void *arg)
{
    while(1)
    {
        sleep(5);
...
static int thread_socket_watch_create(void)
{
    ...
    if ((code = create_pthread (socket_watch, NULL, thread_stack_size, &tid)) != 0)
    {
...
main (int argc, char **argv, char **env)
{
    ...
    int echo_command = 0;
    int no_error = 0;
    char cPrograme[]="SpaceRouter6000";
    ...
    vtysh_passwd_control.vtysh_enable_pass=0;
    /* Main command loop. */
    while (vtysh_rl_gets ())
