...
static void config_add_line (struct list *config, const char *line)
{
...
static void vtysh_config_parse_line (const char *line)
{
    char c;
    ...
    case ' ':
...
#if 1
            /*added by wangyl 2013.3.19 for 4over6*/ //sangmeng mark
            else if (strncmp (line, " address-family ipv4 4over6",
                              strlen (" address-family ipv4 4over6")) == 0)
                config = config_get (BGP_IP4OVER6_NODE, line);
#endif
        ...
        break;
    default:
        if (strncmp (line,"interface tunnel",strlen("interface tunnel"))==0) //add by ccc
        {

            fprintf(stdout,"in node---\n");
            config = config_get (TUNNEL_NODE,line);
            fprintf(stdout,"in node\n");
        }
        ...
        else if (strncmp (line, "ip nat inside source list", strlen ("ip nat inside source list")) == 0)
            config = config_get (IP_NAT_NODE, line);
        else if (strncmp (line, "ip nat pool", strlen ("ip nat pool")) == 0)
            config = config_get (IP_NAT_NODE, line);
        else if (strncmp (line, "ip nat source list", strlen ("ip nat source list")) == 0)
            config = config_get (IP_NAT_NODE, line);
        else if (strncmp (line, "ip route", strlen ("ip route")) == 0)
            config = config_get (IP_NODE, line);
        else if (strncmp (line, "arp", strlen ("arp")) == 0)
            config = config_get (IP_NODE, line);
        else if (strncmp (line, "ipv6 nd neighbor", strlen ("ipv6 nd neighbor")) == 0)
            config = config_get (INTERFACE_NODE, line);
        //added for 4over6 20130409
        else if (strncmp (line, "ip 4over6 route", strlen ("ip 4over6 route")) == 0)
            config = config_get (IP_NODE, line);
        ...
        //added for ivi&nat64 20130409
        else if ((strncmp (line, "nat64",
                           strlen ("nat64")) == 0)
                 || (strncmp (line, "ivi",
                              strlen ("ivi")) == 0)
                 || (strncmp (line, "ecn",
                 ...
                 || (strncmp (line, "dns64",
                              strlen ("dns64")) == 0)
                 ||(strncmp (line,"header-compression",
                             strlen ("header-compression")) == 0) )
            config = config_get (IP_NODE, line);
        //added for tunnel4o6 tcp mss  20130909
        else if (strncmp (line, "tunnel4o6 tcp mss",
                          strlen ("tunnel4o6 tcp mss")) == 0)
            config = config_get (IP_NODE, line);
        //added for snmp cmd 20130909
        else if (strncmp (line, "snmp",strlen ("snmp")) == 0)
            config = config_get (IP_NODE, line);

//added by wangyl for ip dhcp configure 20130801
        else if ((strncmp (line, "ip dhcp",strlen ("ip dhcp")) == 0))
        {
            //fprintf (stderr, "hahahahahah\n");
            config = config_get (DHCP_NODE, line);
        }
        //added end
        /*sangmeng add for ipv4 filter*/
        else if ((strncmp (line, "ip access-list",strlen ("ip access-list")) == 0))
        {
            config = config_get (IPV4_ACCESS_NODE, line);
        }
        else if ((strncmp (line, "ipv6 access-list",strlen ("ipv6 access-list")) == 0))
        {
            config = config_get (IPV6_ACCESS_NODE, line);
        }
        ...
        /*add for pimsm*/
        else if(strncmp (line, "ip pim-sm spt-threshold infinity", strlen ("ip pim-sm spt-threshold infinity")) == 0)
            config = config_get (PIM_NODE, line);
        else if(strncmp (line, "ip pim-sm rp-address", strlen ("ip pim-sm rp-address")) == 0)
            config = config_get (PIM_NODE, line);

        else if(strncmp (line, "ip pim-sm accept-register", strlen ("ip pim-sm accept-register")) == 0)
            config = config_get (PIM_NODE, line);

        else if(strncmp (line, "ip pim-sm c-rp group-policy", strlen ("ip pim-sm c-rp group-policy")) == 0)
            config = config_get (PIM_NODE, line);
        else if(strncmp (line, "ip pim-sm c-rp ", strlen ("ip pim-sm c-rp")) == 0)
            config = config_get (PIM_NODE, line);
        else if(strncmp (line, "ip pim-sm c-bsr", strlen ("ip pim-sm c-bsr")) == 0)
            config = config_get (PIM_NODE, line);

        else if (strncmp (line, "ip multicast-routing", strlen ("ip multicast-routing")) == 0)
            config = config_get (FORWARDING_NODE, line);


        /*add for ipv6v6 pimsm*/

        else if(strncmp (line, "ipv6 pim-sm spt-threshold infinity", strlen ("ipv6 pim-sm spt-threshold infinity")) == 0)
            config = config_get (PIM_NODE, line);
        else if(strncmp (line, "ipv6 pim-sm rp-address", strlen ("ipv6 pim-sm rp-address")) == 0)
            config = config_get (PIM_NODE, line);

        else if(strncmp (line, "ipv6 pim-sm accept-register", strlen ("ipv6 pim-sm accept-register")) == 0)
            config = config_get (PIM_NODE, line);

        else if(strncmp (line, "ipv6 pim-sm c-rp group-policy", strlen ("ipv6 pim-sm c-rp group-policy")) == 0)
            config = config_get (PIM_NODE, line);
        else if(strncmp (line, "ipv6 pim-sm c-rp ", strlen ("ipv6 pim-sm c-rp")) == 0)
            config = config_get (PIM_NODE, line);
        else if(strncmp (line, "ipv6 pim-sm c-bsr", strlen ("ipv6 pim-sm c-bsr")) == 0)
            config = config_get (PIM_NODE, line);

        else if (strncmp (line, "ipv6 multicast-routing", strlen ("ipv6 multicast-routing")) == 0)
            config = config_get (FORWARDING_NODE, line);
...
((I) == ACCESS_NODE || (I) == PREFIX_NODE || (I) == IP_NAT_NODE ||(I) == IP_NODE  || (I) == DHCP_NODE|| (I) == TUNNEL_NODE\
 || (I) == IPV4_ACCESS_NODE || (I) == IPV6_ACCESS_NODE||(I) == AS_LIST_NODE  || (I) == COMMUNITY_LIST_NODE || \
 (I) == ACCESS_IPV6_NODE || (I) == PREFIX_IPV6_NODE \
 || (I) == SERVICE_NODE || (I) == FORWARDING_NODE || (I) == DEBUG_NODE \
 || (I) == AAA_NODE || (I) == PIM_NODE)
...
vtysh_config_write ()
{
    ...
    if(host.enable)
    {
        sprintf (line, "enable password %s", host.enable);
        vtysh_config_parse_line(line);
    }
